...
| `artifacts/v3_mechanism_feature_embedding_pilot_current702_20260601.json` | What happens when the audited mechanism-feature contract is actually fit on train and calibrated on calibration rows? | Current train/cal-only centroid pilot; full contract AUC 0.948491 but no-template ablation AUC 0.549698, so next work must close row-specific mechanism features rather than trust template leakage |
