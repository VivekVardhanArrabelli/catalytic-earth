...
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_source_evidence_sidecar_current702_20260601.json` | What draft source-backed evidence fills the P0 bond-change worksheet before review? | Current draft sidecar; 15/15 rows have source spans and draft events, 12/15 have Rhea equations after bounded lookup resolution, 0 rows approved or consumable |
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_source_evidence_sidecar_strict_audit_current702_20260601.json` | Does the draft P0 source-evidence sidecar pass row-alignment and guardrail checks? | Current strict audit; 15 draft rows, 0 approved rows, 0 critical violations, no feature-contract refresh |
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_source_evidence_review_queue_current702_20260601.json` | Which draft P0 source-evidence rows should be manually reviewed first? | Current manual-review queue; 3 Rhea-missing P1 rows, 5 high-complexity P2 rows, 7 standard P3 rows |
...
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_rhea_lookup_manifest_current702_20260601.json` | Which Rhea EC lookups remain after the bounded official resolution? | Current manual-only lookup manifest; 3 EC targets staged for `m_csa:11`, `m_csa:169`, and `m_csa:5`, no approval |
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_rhea_resolution_consumption_audit_current702_20260601.json` | Did the resolved Rhea equation enter only the draft sidecar while unresolved rows stayed blocked? | Current review-only strict audit; passes with 1 resolved row, 3 unresolved rows, 0 consumable rows |
...
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_reviewer_decision_matrix_current702_20260601.json` | What reviewer provenance choices can clear or hold the three unresolved P0 Rhea rows? | Current review-only decision matrix; three rows, three allowed decision options per row, zero approvals recorded and zero consumable rows |
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_feature_readiness_audit_current702_20260601.json` | Are the draft P0 bond/proton/electron events ready to enter the no-template feature contract? | Current readiness audit; 15 structurally ready drafts, 0 approved consumable rows, feature refresh blocked on reviewer provenance plus 3 Rhea rows |
| `artifacts/v3_mechanism_feature_row_specific_bond_change_p0_refresh_blocker_audit_current702_20260601.json` | Can automation safely refresh the no-template mechanism-feature contract from the P0 draft sidecar? | Current blocker audit; refresh remains blocked until reviewer provenance clears `m_csa:5`, `m_csa:11`, and `m_csa:169` |
