...
| `artifacts/v3_mechanism_feature_inorganic_cofactor_locus_schema_current702_20260601.json` | What schema should close the metal/cobalamin/radical-SAM/Fe-S cofactor locus gap? | Current review-only schema/materialization queue; no sidecar values emitted yet |
...
| `artifacts/v3_fold_augmented_family_panel_missing_primary_channel_queue_current702_20260601.json` | Which family-panel rows still need predicted geometry or predicted-fold scores? | Current review-only materialization queue; 10 secondary-probe/external rows after M-CSA repair; no label/import mutation |
| `artifacts/v3_fold_augmented_family_panel_missing_primary_channel_diagnosis_current702_20260601.json` | Do any missing-queue rows already have frozen geometry/fold scores upstream? | Current review-only diagnosis; remaining rows need source-backed sidecars and coordinate materialization |
