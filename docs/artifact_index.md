...
| `artifacts/v3_fold_augmented_confounded_deployment_closure_audit_current702_20260601.json` | What is the current deployment-closure state for the fold channel on the six cofactor-confounded heldout OOS rows? | Current validation-only synthesis; fixed threshold abstains on 5/6 confounded rows, but production closure remains blocked by five train/cal OOS surface gaps |
