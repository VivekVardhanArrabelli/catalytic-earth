#!/usr/bin/env python3
"""Source serine beta-lactamase bronze through guarded mechanism handles."""
...
from catalytic_earth.serine_beta_lactamase_sourcing import (  # noqa: E402
    FAMILIES,
    write_serine_beta_lactamase_sourcing,
)

DEFAULT_OUT = "artifacts/v3_serine_beta_lactamase_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/serine_beta_lactamase_sourcing_current702.md"
...
def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:3.5.2.6&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which families to source; default = serine_beta_lactamase",
    )
    parser.add_argument("--max-records-per-lane", type=int, default=500)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="optional subset of serine beta-lactamase lane ids to run",
    )
    parser.add_argument(
        "--include-unreviewed-tier2-lanes",
        action="store_true",
        help=(
            "prepend unreviewed UniProt active-site/reaction SBL lanes; requires "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "source only unreviewed UniProt active-site/reaction SBL lanes; requires "
            "--source-tier source_tier_2 and the stricter three-axis trust gate"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="fetch the requested UniProt window with the REST offset parameter",
    )
    parser.add_argument(
        "--use-query-cursor-pagination",
        action="store_true",
        help="fetch UniProt search records through cursor pagination",
    )
    parser.add_argument(
    ...
    if args.reuse_preview:
        ...
        audit = write_serine_beta_lactamase_sourcing(
