#!/usr/bin/env python3
"""Source HAD-like phosphatase bronze through broadened handles."""
...
from catalytic_earth.had_like_phosphatase_sourcing import (  # noqa: E402
    FAMILIES,
    write_had_like_phosphatase_sourcing,
)

DEFAULT_OUT = "artifacts/v3_had_like_phosphatase_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/had_like_phosphatase_sourcing_current702.md"


def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:3.1.3.*+AND+protein_name:HAD&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which broadened-handle families to source; default = had_like_phosphatase",
    )
    parser.add_argument("--max-records-per-lane", type=int, default=220)
    parser.add_argument(
    ...
    if args.reuse_preview:
        ...
        audit = write_had_like_phosphatase_sourcing(
