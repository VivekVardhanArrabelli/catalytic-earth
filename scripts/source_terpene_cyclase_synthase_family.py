#!/usr/bin/env python3
"""Source terpene cyclase/synthase bronze through broadened handles."""
...
from catalytic_earth.terpene_cyclase_synthase_sourcing import (  # noqa: E402
    FAMILIES,
    write_terpene_cyclase_synthase_sourcing,
)
...
def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:4.2.3.*+AND+keyword:Terpene&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which broadened-handle families to source; default = terpene_cyclase_synthase",
    )
