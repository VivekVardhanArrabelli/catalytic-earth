#!/usr/bin/env python3
"""Source Ser/Thr protein phosphatase bronze through broadened handles."""
...
from catalytic_earth.ser_thr_protein_phosphatase_sourcing import (  # noqa: E402
    FAMILIES,
    write_ser_thr_protein_phosphatase_sourcing,
)

DEFAULT_OUT = "artifacts/v3_ser_thr_protein_phosphatase_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/ser_thr_protein_phosphatase_sourcing_current702.md"
...
def _with_alarm(timeout_seconds: int, func, *args):
...
def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:3.1.3.16+AND+protein_name:%22protein%20phosphatase%22&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which broadened-handle families to source; default = ser_thr_protein_phosphatase",
    )
    ...
    if args.reuse_preview:
        ...
        query_fetcher = (
            (lambda query, size: _with_alarm(args.fetch_timeout_seconds, fetch_uniprot_query, query, size))
            if args.fetch_timeout_seconds
            else fetch_uniprot_query
        )
        entry_fetcher = (
            (lambda accession: _with_alarm(args.fetch_timeout_seconds, fetch_uniprot_entry, accession))
            if args.fetch_timeout_seconds
            else fetch_uniprot_entry
        )
        rhea_fetcher = (
            (lambda ec_number, limit: _with_alarm(args.fetch_timeout_seconds, fetch_rhea_by_ec, ec_number, limit))
            if args.fetch_timeout_seconds
            else fetch_rhea_by_ec
        )
        audit = write_ser_thr_protein_phosphatase_sourcing(
