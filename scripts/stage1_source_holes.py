#!/usr/bin/env python3
"""Run Stage-1 hole sourcing (scaling_plan_to_10k) end to end.

Sources fresh annotation-anchored bronze for the cofactor-defined holes
(`radical_sam_enzyme`, `cobalamin_radical_rearrangement`) via the existing
fetch -> cofactor/EC disambiguation -> novelty-gate pipeline, then writes a
NON-DESTRUCTIVE preview. With --apply it appends the novelty-admitted labels to
the SEPARATE expansion registry (data/registries/external_bronze_labels.json);
the frozen current702 benchmark is never written.

Requires live UniProt egress (this sandbox blocks it by default; run where the
network policy is Full/Custom-allowed). The cofactorless ser_his hole is NOT
sourced here -- use `build-ser-his-triad-locator-scan`. See
docs/stage1_hole_sourcing_runbook.md.

Usage:
    PYTHONPATH=src python scripts/stage1_source_holes.py            # preview only
    PYTHONPATH=src python scripts/stage1_source_holes.py --apply    # preview + append
"""
...
DEFAULT_OUT = "artifacts/v3_stage1_hole_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/stage1_hole_sourcing_current702.md"


def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:3.4.21.1&format=tsv&size=1",
...
def main() -> int:
    ...
    if not args.skip_egress_check and not _egress_ok():
        print(
            ...
            f"({UNIPROT_SEARCH_URL} did not return 2xx). This sandbox blocks it by "
            "default; run where the network policy allows UniProt egress, or pass "
            "--skip-egress-check to attempt anyway.",
            file=sys.stderr,
        )
        return 2

    audit = write_stage1_hole_sourcing(
    ...
    for hole, proj in audit["floor_projection"].items():
        print(
            f"  {hole}: {proj['combined_before']} -> {proj['projected_combined']} "
