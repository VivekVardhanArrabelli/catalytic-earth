#!/usr/bin/env python3
"""Source SAM methyltransferase bronze through the broadened-handle admission path.

This is a non-destructive preview unless --apply is supplied. With --apply it appends only
the novelty-admitted SAM methyltransferase labels to the separate external bronze registry;
the frozen current702 registry is never written and its sha256 is printed before/after.
"""
...
from catalytic_earth.sam_methyltransferase_sourcing import (  # noqa: E402
    FAMILIES,
    write_sam_methyltransferase_sourcing,
)

DEFAULT_OUT = "artifacts/v3_sam_methyltransferase_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/sam_methyltransferase_sourcing_current702.md"


def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:2.1.1.6&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which broadened-handle families to source; default = sam_methyltransferase",
    )
    ...
    audit = write_sam_methyltransferase_sourcing(
