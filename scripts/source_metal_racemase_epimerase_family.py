#!/usr/bin/env python3
"""Source non-PLP racemase/epimerase bronze through broadened handles."""
...
from catalytic_earth.metal_racemase_epimerase_sourcing import (  # noqa: E402
    FAMILIES,
    write_metal_racemase_epimerase_non_plp_sourcing,
)
...
DEFAULT_OUT = "artifacts/v3_metal_racemase_epimerase_non_plp_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/metal_racemase_epimerase_non_plp_sourcing_current702.md"


def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:5.1.*+AND+keyword:Isomerase&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which broadened-handle families to source; default = metal_racemase_epimerase_non_plp",
    )
    parser.add_argument("--max-records-per-lane", type=int, default=120)
    ...
    audit = write_metal_racemase_epimerase_non_plp_sourcing(
