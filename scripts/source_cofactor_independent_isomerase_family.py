#!/usr/bin/env python3
"""Source cofactor-independent isomerase bronze through the broadened-handle path."""
...
from catalytic_earth.cofactor_independent_isomerase_sourcing import (  # noqa: E402
    DEFAULT_ISOMERASE_CAP_CEILING,
    FAMILIES,
    write_cofactor_independent_isomerase_sourcing,
)
...
DEFAULT_OUT = "artifacts/v3_cofactor_independent_isomerase_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/cofactor_independent_isomerase_sourcing_current702.md"


def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:5.3.1.1&format=tsv&size=1",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="which broadened-handle families to source; default = cofactor_independent_isomerase",
    )
    parser.add_argument("--max-records-per-lane", type=int, default=80)
    parser.add_argument("--cap-ceiling", type=int, default=DEFAULT_ISOMERASE_CAP_CEILING)
    ...
    audit = write_cofactor_independent_isomerase_sourcing(
