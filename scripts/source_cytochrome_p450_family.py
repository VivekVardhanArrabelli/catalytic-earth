#!/usr/bin/env python3
"""Source cytochrome P450 monooxygenase bronze through the broadened-handle path.

This is a non-destructive preview unless --apply is supplied. With --apply it appends only
the novelty-admitted P450 labels to the separate external bronze registry; the frozen
current702 registry is never written and its sha256 is printed before/after.
"""
...
from catalytic_earth.cytochrome_p450_sourcing import (  # noqa: E402
    FAMILIES,
    write_cytochrome_p450_sourcing,
)
...
DEFAULT_OUT = "artifacts/v3_cytochrome_p450_sourcing_preview_current702.json"
DEFAULT_REPORT = "work/cytochrome_p450_sourcing_current702.md"


def _egress_ok() -> bool:
    try:
        request = Request(
            f"{UNIPROT_SEARCH_URL}?query=ec:1.14.14.1&format=tsv&size=1",
...
def main() -> int:
    ...
    audit = write_cytochrome_p450_sourcing(
