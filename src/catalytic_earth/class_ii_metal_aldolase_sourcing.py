"""Broadened-handle sourcing for class-II metal aldolases."""
...
ARTIFACT_ID = "v3_class_ii_metal_aldolase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "class_ii_metal_aldolase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 4.1.2/4.1.3 scopes reviewed C-C lyase candidates only. Membership is
# confirmed later by non-EC metal cofactor/site context, Lyase/aldolase text,
# Rhea C-C/oxoacid reaction evidence, or active-/binding-/metal-site mechanism
# evidence. PLP, ThDP, class-I Schiff-base, and side-EC boundary rows are held.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "class_ii_metal_aldolase_reviewed_metal_lyases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:4.1.2.*) OR (ec:4.1.3.*)) AND "
                "((cc_cofactor:zinc) OR (cc_cofactor:cobalt) OR "
                "((keyword:Lyase) AND (keyword:Metal-binding)))"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a class-II metal aldolase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_class_ii_metal_aldolase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "class_ii_metal_aldolase_broadened_handle_fresh_fetch",
                "path": "class_ii_metal_aldolase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_class_ii_metal_aldolase_annotation",
        "stage": "scaling_plan_to_10k:wire_class_ii_metal_aldolase_broadened_handle",
        ...
        "guardrails": {
            ...
            "class_ii_aldolase_handles_scope_admission_only": True,
            ...
            "plp_thdp_schiff_transferase_hydrolase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- class-II aldolase handles scope-admission only, never predictive: "
            ...
            "- PLP/ThDP/Schiff-base/transferase/hydrolase/side-EC boundary guards: "
            f"{audit['guardrails']['plp_thdp_schiff_transferase_hydrolase_side_ec_boundary_guard']}.",
...
def write_class_ii_metal_aldolase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_class_ii_metal_aldolase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
