"""Broadened-handle sourcing for SAM-dependent methyltransferases.

This is the 15-fingerprint follow-on to the NAD(P)-dehydrogenase + glycosyltransferase
runner. It uses the same admission machinery:
...
SAM methyltransferase scope is EC 2.1.1. Membership requires SAM/SAH methyl-donor
evidence (Rhea participant, cofactor/cosubstrate handle, binding-site evidence, or the
Methyltransferase keyword) and explicitly excludes Fe-S / radical-SAM rows. EC, keyword,
SAM/SAH participant text, protein name, and prose remain admission/excluded context only;
`predictive_evidence` stays empty.
"""
...
ARTIFACT_ID = "v3_sam_methyltransferase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "sam_methyltransferase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# Split first by a few high-signal EC leaves, then include a broad keyword fallback.
# These are fetch/scope lanes only. The authoritative admission decision is still the
# mechanism corroborator + EC 2.1.1 predicate in the disambiguation engine, and every
# admitted row is deduped against both registries and novelty-gated.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "sam_methyltransferase_ec_2_1_1_nucleic_acid",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND "
                "((ec:2.1.1.37) OR (ec:2.1.1.72) OR (ec:2.1.1.113) OR (ec:2.1.1.199)) "
                "AND (keyword:Methyltransferase)"
            ),
        },
        {
            "lane_id": "sam_methyltransferase_ec_2_1_1_small_molecule",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND "
                "((ec:2.1.1.6) OR (ec:2.1.1.68) OR (ec:2.1.1.104) OR (ec:2.1.1.140)) "
                "AND (keyword:Methyltransferase)"
            ),
        },
        {
            "lane_id": "sam_methyltransferase_ec_2_1_1_broad_keyword",
            "target_family_lane": FAMILY,
            "query": "(reviewed:true) AND (ec:2.1.1.*) AND (keyword:Methyltransferase)",
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a SAM methyltransferase family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_sam_methyltransferase_sourcing(
    *,
    ...
    """Source SAM methyltransferase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "sam_methyltransferase_broadened_handle_fresh_fetch",
                "path": "sam_methyltransferase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_sam_sah_keyword_annotation",
        "stage": "scaling_plan_to_10k:wire_sam_methyltransferase_broadened_handle",
        ...
        "guardrails": {
            ...
            "predictive_features_use_ec_name_keyword_sam_sah_or_prose": False,
            "ec_used_for_scope_assignment_only_never_predictive": True,
            "sam_sah_keyword_handles_scope_admission_only": True,
            ...
            "no_fe_s_radical_sam_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- SAM/SAH + keyword handles scope-admission only, never predictive: "
            ...
            "- No-Fe-S radical-SAM guard: "
            f"{audit['guardrails']['no_fe_s_radical_sam_guard']}.",
...
def write_sam_methyltransferase_sourcing(
    *,
    out_path: Path,
    ...
    """Build the SAM preview and write it (non-destructive: no registry is touched)."""
    expansion_path = Path(expansion_registry_path)
    audit = build_sam_methyltransferase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
