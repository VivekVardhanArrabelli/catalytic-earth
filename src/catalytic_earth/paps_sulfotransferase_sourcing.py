"""Broadened-handle sourcing for PAPS-dependent sulfotransferases (EC 2.8.2)."""
...
ARTIFACT_ID = "v3_paps_sulfotransferase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "paps_sulfotransferase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 2.8.2 scopes the reviewed candidate supply only (NOT shared with any existing fingerprint);
# counted corroboration comes from a sulfotransferase family/name plus a Rhea/reviewed sulfuryl-
# transfer reaction naming the PAPS donor (3'-phosphoadenylyl sulfate) or the PAP product
# (adenosine 3',5'-bisphosphate). Sulfur-relay sulfurtransferases (rhodanese / cysteine desulfurase,
# EC 2.8.1), ATP sulfurylase / adenylyl-sulfate enzymes, and PAPS reductase are guarded by the
# disambiguation engine's boundary signal.
SULFO_SCOPE = (
...
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "paps_sulfotransferase_reaction_sulfate",
            "target_family_lane": FAMILY,
            "query": f"{SULFO_SCOPE} AND (cc_catalytic_activity:sulfate)",
        },
        {
            "lane_id": "paps_sulfotransferase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{SULFO_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "paps_sulfotransferase_named_all",
            "target_family_lane": FAMILY,
            "query": SULFO_SCOPE,
        },
    )
...
def _lane_queries_for(
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown PAPS-sulfotransferase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_paps_sulfotransferase_sourcing(
    *,
    ...
    """Source PAPS-sulfotransferase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "paps_sulfotransferase_broadened_handle_fresh_fetch",
                "path": "paps_sulfotransferase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_paps_sulfuryl_transfer_annotation",
        "stage": "scaling_plan_to_10k:wire_paps_sulfotransferase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "paps_sulfotransferase_handles_scope_admission_only": True,
            ...
            "sulfurtransferase_atp_sulfurylase_paps_reductase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['sulfurtransferase_atp_sulfurylase_paps_reductase_boundary_guard']}.",
...
def write_paps_sulfotransferase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_paps_sulfotransferase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
