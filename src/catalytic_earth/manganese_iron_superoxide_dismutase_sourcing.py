"""Broadened-handle sourcing for Mn/Fe superoxide dismutases."""
...
ARTIFACT_ID = "v3_manganese_iron_superoxide_dismutase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "manganese_iron_superoxide_dismutase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "mn_fe_sod_reviewed_ec_1_15_1_1_guarded",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.15.1.1) AND "
                "((cc_cofactor:manganese) OR (cc_cofactor:iron) OR "
                "(protein_name:manganese) OR (protein_name:iron) OR "
                "(protein_name:Mn) OR (protein_name:Fe)) NOT "
                "((cc_cofactor:copper) OR (cc_cofactor:zinc) OR "
                '(protein_name:"Cu-Zn") OR (protein_name:"Cu/Zn") OR '
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a Mn/Fe SOD sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_manganese_iron_superoxide_dismutase_sourcing(
    *,
    ...
    """Source Mn/Fe SOD labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "mn_fe_sod_broadened_handle_fresh_fetch",
                "path": "mn_fe_sod_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_mn_fe_sod_annotation",
        "stage": "scaling_plan_to_10k:wire_manganese_iron_superoxide_dismutase_broadened_handle",
        ...
        "guardrails": {
            ...
            "sod_handles_scope_admission_only": True,
            ...
            "cu_zn_heme_superoxide_reductase_side_ec_boundary_guard": True,
        ...
        "next_action": (
            "Review floor_projection + novelty_gate. If novelty, dedup, trust-tier, "
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- SOD handles scope-admission only, never predictive: "
            ...
            "- Cu/Zn, heme/peroxidase, superoxide-reductase, and side-EC guards: "
            f"{audit['guardrails']['cu_zn_heme_superoxide_reductase_side_ec_boundary_guard']}.",
...
def write_manganese_iron_superoxide_dismutase_sourcing(
    *,
    out_path: Path,
    ...
    """Build the Mn/Fe SOD preview and write it without touching registries."""
    expansion_path = Path(expansion_registry_path)
    audit = build_manganese_iron_superoxide_dismutase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
