"""Broadened-handle sourcing for dihydrofolate reductases (EC 1.5.1.3)."""
...
ARTIFACT_ID = "v3_dihydrofolate_reductase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "dihydrofolate_reductase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 1.5.1.3 scopes the reviewed candidate supply only (NOT shared with any existing fingerprint); the
# required hard mechanism anchor is a Rhea/reviewed reaction reducing 7,8-dihydrofolate (or folate) to
# 5,6,7,8-tetrahydrofolate with NADP(H) -- a dihydrofolate/folate substrate token AND a NAD(P)
# cosubstrate token. The second corroborator is a dihydrofolate-reductase family/name OR an annotated
# active/binding site. Dihydrofolate synthase / folylpolyglutamate synthetase (EC 6.3.2),
# methylenetetrahydrofolate reductase (EC 1.5.1.20) / dehydrogenase (EC 1.5.1.5), and the thymidylate
# synthase half of a bifunctional row (non-1.5.1.3 side EC) are boundary-guarded by the disambiguation
# engine. NOTE: the NADPH hydride-transfer half-reaction is shared with the EC 1.1.1 NAD(P)
# dehydrogenases, so this family is representation-confusable with the NAD(P) hydride-transfer surface
# in the source-free feature space (cap 150); the folate-reduction reaction separates it at admission.
DIHYDROFOLATE_REDUCTASE_SCOPE = "(reviewed:true) AND (ec:1.5.1.3)"

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "dihydrofolate_reductase_reaction_folate",
            "target_family_lane": FAMILY,
            "query": f'{DIHYDROFOLATE_REDUCTASE_SCOPE} AND (cc_catalytic_activity:"dihydrofolate")',
        },
        {
            "lane_id": "dihydrofolate_reductase_active_binding",
            "target_family_lane": FAMILY,
            "query": (
                f"{DIHYDROFOLATE_REDUCTASE_SCOPE} AND "
                "((ft_act_site:*) OR (ft_binding:*))"
            ),
        },
        {
            "lane_id": "dihydrofolate_reductase_named_all",
            "target_family_lane": FAMILY,
            "query": (
                f"{DIHYDROFOLATE_REDUCTASE_SCOPE} AND "
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a dihydrofolate-reductase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown dihydrofolate-reductase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_dihydrofolate_reductase_sourcing(
    *,
    ...
    """Source dihydrofolate-reductase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "dihydrofolate_reductase_broadened_handle_fresh_fetch",
                "path": "dihydrofolate_reductase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_nadph_dihydrofolate_reduction_annotation",
        "stage": "scaling_plan_to_10k:wire_dihydrofolate_reductase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "dihydrofolate_reductase_handles_scope_admission_only": True,
            ...
            "folate_synthase_mthfr_thymidylate_synthase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['folate_synthase_mthfr_thymidylate_synthase_side_ec_boundary_guard']}.",
...
def write_dihydrofolate_reductase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_dihydrofolate_reductase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
