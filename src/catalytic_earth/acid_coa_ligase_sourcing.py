"""Broadened-handle sourcing for acid--CoA ligases / acyl-CoA synthetases (EC 6.2.1)."""
...
ARTIFACT_ID = "v3_acid_coa_ligase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "acid_coa_ligase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 6.2.1 scopes the reviewed candidate supply only (NOT shared with any existing fingerprint);
# the required hard mechanism anchor is a Rhea/reviewed reaction that activates a carboxylate with
# ATP to a CoA thioester (acid + ATP + CoA -> acyl-CoA + AMP + diphosphate) -- a CoA token AND an
# AMP/adenylate release token, so the ADP-forming succinate--CoA ligases and the CoA transferases
# (no AMP release) do not qualify. The second corroborator is a CoA-ligase / acyl-CoA-synthetase
# family/name handle OR an annotated active/binding site. CoA transferases (EC 2.3 / 2.8.3),
# biotin-dependent carboxylases (EC 6.3.4 / 6.4.1), thiolases, and acyl-CoA dehydrogenases are
# guarded by the disambiguation engine's boundary signal. NOTE: the ATP-adenylation step is shared
# with the EC 6.3 atp_amide_ligase and the CoA thioester with coa_acyltransferase, so this family is
# representation-confusable with both in the source-free feature space (cap 150).
ACID_COA_SCOPE = "(reviewed:true) AND (ec:6.2.1.*)"

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "acid_coa_ligase_reaction_coa",
            "target_family_lane": FAMILY,
            "query": f'{ACID_COA_SCOPE} AND (cc_catalytic_activity:"CoA")',
        },
        {
            "lane_id": "acid_coa_ligase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{ACID_COA_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "acid_coa_ligase_named_all",
            "target_family_lane": FAMILY,
            "query": (
                f"{ACID_COA_SCOPE} AND "
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an acid--CoA-ligase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown acid--CoA-ligase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_acid_coa_ligase_sourcing(
    *,
    ...
    """Source acid--CoA-ligase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "acid_coa_ligase_broadened_handle_fresh_fetch",
                "path": "acid_coa_ligase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_atp_coa_thioester_ligation_annotation",
        "stage": "scaling_plan_to_10k:wire_acid_coa_ligase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "acid_coa_ligase_handles_scope_admission_only": True,
            ...
            "coa_transferase_carboxylase_thiolase_dehydrogenase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['coa_transferase_carboxylase_thiolase_dehydrogenase_boundary_guard']}.",
...
def write_acid_coa_ligase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_acid_coa_ligase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
