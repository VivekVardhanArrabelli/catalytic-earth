"""Broadened-handle sourcing for ATP-dependent amide ligases."""
...
ARTIFACT_ID = "v3_atp_amide_ligase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "atp_amide_ligase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 6.3 scopes reviewed C-N ligase candidates only. Membership is confirmed later
# by non-EC ATP/ADP/phosphate or Mg context, Ligase/ATP-grasp text, Rhea amide/C-N
# ligation evidence, or active-/binding-site mechanism evidence. Biotin carboxylases
# and generic kinase/phosphotransferase boundary rows are intentionally excluded here.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "atp_amide_ligase_reviewed_ec63_ligase_non_biotin",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:6.3.*) AND (keyword:Ligase) "
                "NOT (ec:6.3.4.*) NOT (keyword:Biotin) NOT (protein_name:carboxylase)"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an ATP amide ligase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_atp_amide_ligase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "atp_amide_ligase_broadened_handle_fresh_fetch",
                "path": "atp_amide_ligase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_atp_amide_ligase_annotation",
        "stage": "scaling_plan_to_10k:wire_atp_amide_ligase_broadened_handle",
        ...
        "guardrails": {
            ...
            "atp_ligase_handles_scope_admission_only": True,
            ...
            "biotin_kinase_transferase_hydrolase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- ATP ligase handles scope-admission only, never predictive: "
            ...
            "- Biotin/kinase/transferase/hydrolase/side-EC boundary guards: "
            f"{audit['guardrails']['biotin_kinase_transferase_hydrolase_side_ec_boundary_guard']}.",
...
def write_atp_amide_ligase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_atp_amide_ligase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
