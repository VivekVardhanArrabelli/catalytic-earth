"""Broadened-handle sourcing for nucleoside diphosphate kinases."""
...
ARTIFACT_ID = "v3_nucleoside_diphosphate_kinase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "nucleoside_diphosphate_kinase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "nucleoside_diphosphate_kinase_reviewed_strict_ec_2_7_4_6",
            "target_family_lane": FAMILY,
            "query": (
                '(reviewed:true) AND (ec:2.7.4.6) AND '
                '((protein_name:"nucleoside diphosphate kinase") OR '
                '(protein_name:"nucleoside-diphosphate kinase")) NOT '
                "((ec:2.7.11.*) OR (ec:2.7.13.*) OR (ec:3.*) OR "
                "(ec:2.7.4.3) OR (ec:2.7.4.4) OR (ec:2.7.4.14) OR "
                "(ec:2.7.4.18))"
            ),
        },
    )
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an NDK sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_nucleoside_diphosphate_kinase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "nucleoside_diphosphate_kinase_broadened_handle_fresh_fetch",
                "path": "nucleoside_diphosphate_kinase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_ndk_annotation",
        "stage": "scaling_plan_to_10k:wire_nucleoside_diphosphate_kinase_broadened_handle",
        ...
        "guardrails": {
            ...
            "ndk_handles_scope_admission_only": True,
            ...
            "kinase_hydrolase_nmp_side_ec_boundary_guard": True,
        ...
        "counts": {
            ...
            "disambiguation_hold_reason_counts": disambig["counts"].get("hold_reason_counts", {}),
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- NDK handles scope-admission only, never predictive: "
            ...
            "- Protein-kinase/histidine-kinase/hydrolase/NMP-kinase side-EC boundary guards: "
            f"{audit['guardrails']['kinase_hydrolase_nmp_side_ec_boundary_guard']}.",
...
def write_nucleoside_diphosphate_kinase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_nucleoside_diphosphate_kinase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
