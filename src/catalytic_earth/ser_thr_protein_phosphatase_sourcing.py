"""Broadened-handle sourcing for Ser/Thr protein phosphatases."""
...
ARTIFACT_ID = "v3_ser_thr_protein_phosphatase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "ser_thr_protein_phosphatase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "ser_thr_protein_phosphatase_ec_3_1_3_16_named",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:3.1.3.16) OR (ec:3.1.3.48)) AND "
                '((protein_name:"protein phosphatase") OR '
                '(protein_name:"serine/threonine-protein phosphatase") OR '
                '(protein_name:calcineurin) OR (keyword:Phosphoprotein))'
            ),
        },
        {
            "lane_id": "ser_thr_protein_phosphatase_metal_context",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:3.1.3.16) OR (ec:3.1.3.48)) AND "
                '((protein_name:"protein phosphatase") OR (protein_name:calcineurin)) AND '
                "((cc_cofactor:manganese) OR (cc_cofactor:iron) OR "
                "(cc_cofactor:magnesium) OR (ft_binding:manganese) OR "
                "(ft_binding:iron) OR (ft_binding:magnesium))"
            ),
        },
        {
            "lane_id": "ser_thr_protein_phosphatase_phosphoprotein_reaction",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:3.1.3.16) OR (ec:3.1.3.48)) AND "
                '((cc_catalytic_activity:phosphoprotein) OR '
                '(cc_catalytic_activity:"phosphorylated protein") OR '
                '(cc_catalytic_activity:"protein + phosphate"))'
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a Ser/Thr protein phosphatase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_ser_thr_protein_phosphatase_sourcing(
    *,
    ...
    """Source Ser/Thr protein phosphatase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "ser_thr_protein_phosphatase_broadened_handle_fresh_fetch",
                "path": "ser_thr_protein_phosphatase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_protein_phosphatase_metal_annotation",
        "stage": "scaling_plan_to_10k:wire_ser_thr_protein_phosphatase_broadened_handle",
        ...
        "guardrails": {
            ...
            "protein_phosphatase_handles_scope_admission_only": True,
            ...
            "had_ptp_small_molecule_phosphatase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['had_ptp_small_molecule_phosphatase_boundary_guard']}.",
...
def write_ser_thr_protein_phosphatase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_ser_thr_protein_phosphatase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
