"""Broadened-handle sourcing for cofactor-independent isomerases."""
...
ARTIFACT_ID = "v3_cofactor_independent_isomerase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "cofactor_independent_isomerase"
FAMILIES: tuple[str, ...] = (FAMILY,)
DEFAULT_ISOMERASE_CAP_CEILING = 150

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "isomerase_ec_5_3_2_keto_aldose",
            "target_family_lane": FAMILY,
            "query": "(reviewed:true) AND (ec:5.3.2.*) AND (keyword:Isomerase)",
        },
        {
            "lane_id": "isomerase_ec_5_3_3_alkene",
            "target_family_lane": FAMILY,
            "query": "(reviewed:true) AND (ec:5.3.3.*) AND (keyword:Isomerase)",
        },
        {
            "lane_id": "isomerase_ec_5_3_1_broad",
            "target_family_lane": FAMILY,
            "query": "(reviewed:true) AND (ec:5.3.1.*) AND (keyword:Isomerase)",
        },
        {
            "lane_id": "isomerase_ec_5_3_broad_keyword",
            "target_family_lane": FAMILY,
            "query": "(reviewed:true) AND (ec:5.3.*) AND (keyword:Isomerase)",
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a cofactor-independent isomerase family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_cofactor_independent_isomerase_sourcing(
    *,
    ...
    cap_ceiling: int = DEFAULT_ISOMERASE_CAP_CEILING,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "cofactor_independent_isomerase_broadened_handle_fresh_fetch",
                "path": "cofactor_independent_isomerase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_isomerase_active_site_annotation",
        "stage": "scaling_plan_to_10k:wire_cofactor_independent_isomerase_broadened_handle",
        ...
        "guardrails": {
            ...
            "predictive_features_use_ec_name_keyword_isomerase_or_prose": False,
            "ec_used_for_scope_assignment_only_never_predictive": True,
            "isomerase_handles_scope_admission_only": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Isomerase handles scope-admission only, never predictive: "
            ...
            "- Non-5.3 side-EC guard: "
            f"{audit['guardrails']['non_5_3_side_ec_guard']}.",
...
def write_cofactor_independent_isomerase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_cofactor_independent_isomerase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
