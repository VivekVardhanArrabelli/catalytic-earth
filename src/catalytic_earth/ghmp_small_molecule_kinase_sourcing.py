"""Broadened-handle sourcing for GHMP small molecule kinases."""
...
ARTIFACT_ID = "v3_ghmp_small_molecule_kinase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "ghmp_small_molecule_kinase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "ghmp_small_molecule_kinase_reviewed_strict_ec_2_7_1",
            ...
            "query": (
                ...
                '(protein_name:"galactokinase")) NOT '
                ...
                '(protein_name:"acetate kinase") OR (protein_name:"glucokinase") OR '
                '(protein_name:"hexokinase") OR (protein_name:"glycerol kinase") OR '
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an GHMP sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_ghmp_small_molecule_kinase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "ghmp_small_molecule_kinase_broadened_handle_fresh_fetch",
                "path": "ghmp_small_molecule_kinase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_GHMP_annotation",
        "stage": "scaling_plan_to_10k:wire_ghmp_small_molecule_kinase_broadened_handle",
        ...
        "guardrails": {
            ...
            "ghmp_handles_scope_admission_only": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- GHMP handles scope-admission only, never predictive: "
            ...
            "- Protein-kinase/histidine-kinase/hydrolase/ASKHA/NDK/dNK/Pfk boundary guards: "
...
def write_ghmp_small_molecule_kinase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_ghmp_small_molecule_kinase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
