"""Broadened-handle sourcing for N-ribosyl hydrolases."""
...
ARTIFACT_ID = "v3_n_ribosyl_hydrolase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "n_ribosyl_hydrolase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "n_ribosyl_hydrolase_ec_3_2_2_named",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:3.2.2.*) OR "
                '(protein_name:"nucleoside hydrolase") OR '
                '(protein_name:"N-ribosylhydrolase") OR '
                '(protein_name:"N-ribosidase") OR '
                "(protein_name:nucleosidase) OR "
                '(protein_name:"N-ribohydrolase") OR '
                '(protein_name:"nucleoside N-ribohydrolase"))'
            ),
        },
        {
            "lane_id": "n_ribosyl_hydrolase_nucleosidase_synonyms",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((protein_name:nucleosidase) OR "
                '(protein_name:"uridine nucleosidase") OR '
                '(protein_name:"purine nucleosidase") OR '
                '(protein_name:"N-ribohydrolase") OR '
                '(protein_name:"nucleoside N-ribohydrolase"))'
            ),
        },
        {
            "lane_id": "n_ribosyl_hydrolase_non_ec_handles",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:3.2.2.*) OR "
                '(protein_name:"nucleoside hydrolase") OR '
                ...
                "(protein_name:nucleosidase) OR "
                '(protein_name:"N-ribohydrolase") OR '
                '(protein_name:"nucleoside N-ribohydrolase")) AND '
                '((protein_name:"nucleoside hydrolase") OR '
                '(protein_name:"N-ribosylhydrolase") OR '
                '(protein_name:"N-ribosidase") OR (keyword:Hydrolase) OR '
                "(ft_act_site:*) OR (ft_binding:*))"
            ),
        },
        {
            "lane_id": "n_ribosyl_hydrolase_n_glycosidic_reaction",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:3.2.2.*) OR "
                '(protein_name:"nucleoside hydrolase") OR '
                ...
                '(protein_name:"N-ribohydrolase") OR '
                '(protein_name:"nucleoside N-ribohydrolase")) AND '
                "((cc_catalytic_activity:ribose) OR "
                '(cc_catalytic_activity:"D-ribose") OR '
                "(cc_catalytic_activity:hypoxanthine) OR "
                "(cc_catalytic_activity:adenine) OR "
                "(cc_catalytic_activity:uracil))"
            ),
        },
    )
}
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a N-ribosyl hydrolase sourcing family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown N-ribosyl hydrolase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_n_ribosyl_hydrolase_sourcing(
    *,
    ...
    """Source N-ribosyl hydrolase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "n_ribosyl_hydrolase_broadened_handle_fresh_fetch",
                "path": "n_ribosyl_hydrolase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_n_glycosidic_hydrolysis_annotation",
        "stage": "scaling_plan_to_10k:wire_n_ribosyl_hydrolase_broadened_handle",
        ...
        "guardrails": {
            ...
            "n_ribosyl_hydrolase_handles_scope_admission_only": True,
            ...
            "o_glycosidase_phosphorylase_kinase_transferase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['o_glycosidase_phosphorylase_kinase_transferase_boundary_guard']}.",
...
def write_n_ribosyl_hydrolase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_n_ribosyl_hydrolase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
