"""Broadened-handle sourcing for aminoglycoside acetyltransferases (AAC; GNAT fold)."""
...
ARTIFACT_ID = "v3_aminoglycoside_acetyltransferase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "aminoglycoside_acetyltransferase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 2.3.1 scopes the reviewed candidate supply only; counted corroboration comes
# from aminoglycoside-acetyltransferase family/name text, acetyl-CoA/CoA cosubstrate/binding
# context, and Rhea/reviewed aminoglycoside N-acetyl transfer reaction text. Generic CoA
# acyltransferases and APH/ANT resistance enzymes are guarded by the AAC boundary signal.
AAC_SCOPE = (
    "(reviewed:true) AND (ec:2.3.1.*) AND "
    "((protein_name:aminoglycoside) OR (protein_name:gentamicin) OR "
    "(protein_name:kanamycin) OR (protein_name:tobramycin) OR "
    "(protein_name:AAC))"
)
...
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "aminoglycoside_acetyltransferase_named_acetyltransferase",
            "target_family_lane": FAMILY,
            "query": f"{AAC_SCOPE} AND ((keyword:Acetyltransferase) OR (keyword:Antibiotic) OR (keyword:\"Antibiotic resistance\"))",
        },
        {
            "lane_id": "aminoglycoside_acetyltransferase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{AAC_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*) OR (cc_cofactor:coa))",
        },
        {
            "lane_id": "aminoglycoside_acetyltransferase_reaction_coa",
            "target_family_lane": FAMILY,
            "query": (
                f"{AAC_SCOPE} AND ((cc_catalytic_activity:acetyl-CoA) OR "
                "(cc_catalytic_activity:CoA) OR (cc_catalytic_activity:acetyltransferase))"
            ),
        },
    )
}
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an aminoglycoside acetyltransferase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown AAC lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_aminoglycoside_acetyltransferase_sourcing(
    *,
    ...
    """Source aminoglycoside acetyltransferase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "aminoglycoside_acetyltransferase_broadened_handle_fresh_fetch",
                "path": "aminoglycoside_acetyltransferase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_aac_acetyl_coa_n_acetyl_transfer_annotation",
        "stage": "scaling_plan_to_10k:wire_aminoglycoside_acetyltransferase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "aminoglycoside_acetyltransferase_handles_scope_admission_only": True,
            ...
            "coa_acyltransferase_aph_ant_metal_flavin_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['coa_acyltransferase_aph_ant_metal_flavin_boundary_guard']}.",
...
def write_aminoglycoside_acetyltransferase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_aminoglycoside_acetyltransferase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
