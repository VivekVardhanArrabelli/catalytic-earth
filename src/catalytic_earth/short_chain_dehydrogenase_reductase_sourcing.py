"""Broadened-handle sourcing for short-chain dehydrogenases/reductases."""
...
ARTIFACT_ID = "v3_short_chain_dehydrogenase_reductase_sourcing_preview_current702"
...
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "short_chain_dehydrogenase_reductase_named_nad",
            "target_family_lane": FAMILY,
            "query": f"{SDR_SCOPE} AND ((keyword:NAD) OR (keyword:NADP))",
        },
        {
            "lane_id": "short_chain_dehydrogenase_reductase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{SDR_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "short_chain_dehydrogenase_reductase_reaction_nad",
            "target_family_lane": FAMILY,
            "query": (
                f"{SDR_SCOPE} AND ((cc_catalytic_activity:NAD) OR "
                "(cc_catalytic_activity:NADP) OR (cc_catalytic_activity:NADH) OR "
                "(cc_catalytic_activity:NADPH))"
            ),
        },
    )
}
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a short-chain dehydrogenase/reductase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown SDR lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_short_chain_dehydrogenase_reductase_sourcing(
    *,
    ...
    """Source SDR labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "short_chain_dehydrogenase_reductase_broadened_handle_fresh_fetch",
                "path": "short_chain_dehydrogenase_reductase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_sdr_nad_p_hydride_transfer_annotation",
        "stage": "scaling_plan_to_10k:wire_short_chain_dehydrogenase_reductase_broadened_handle",
        ...
        "guardrails": {
            ...
            "short_chain_dehydrogenase_reductase_handles_scope_admission_only": True,
            ...
            "akr_mdr_aldh_flavin_metal_redox_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['akr_mdr_aldh_flavin_metal_redox_boundary_guard']}.",
...
def write_short_chain_dehydrogenase_reductase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_short_chain_dehydrogenase_reductase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
