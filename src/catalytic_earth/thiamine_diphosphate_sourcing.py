"""Broadened-handle sourcing for ThDP-dependent ylide enzymes."""
...
ARTIFACT_ID = "v3_thiamine_diphosphate_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "thiamine_diphosphate_enzyme"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC selects reviewed ThDP candidate scope only. Admission requires non-EC ThDP/Mg,
# Rhea carbonyl/decarboxylation/ThDP participant, family keyword/domain, or active-site
# evidence. PLP, molybdopterin/flavin/heme, kinase/phosphotransferase, hydrolase, side-EC,
# and multi-fingerprint rows are held.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "thdp_reviewed_ec_2_2_1_4_1_1_1_2_4",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND ((ec:2.2.1.*) OR (ec:4.1.1.*) OR "
                "(ec:1.2.4.*)) AND ((cc_cofactor:thiamine) OR "
                "(keyword:Thiamine\\ pyrophosphate))"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a ThDP sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_thiamine_diphosphate_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "thiamine_diphosphate_broadened_handle_fresh_fetch",
                "path": "thiamine_diphosphate_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_thdp_annotation",
        "stage": "scaling_plan_to_10k:wire_thiamine_diphosphate_broadened_handle",
        ...
        "guardrails": {
            ...
            "thdp_handles_scope_admission_only": True,
            ...
            "plp_molybdopterin_flavin_heme_kinase_hydrolase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- ThDP handles scope-admission only, never predictive: "
            ...
            "- PLP/Mo/flavin/heme/kinase/hydrolase/side-EC boundary guards: "
            f"{audit['guardrails']['plp_molybdopterin_flavin_heme_kinase_hydrolase_side_ec_boundary_guard']}.",
...
def write_thiamine_diphosphate_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_thiamine_diphosphate_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
