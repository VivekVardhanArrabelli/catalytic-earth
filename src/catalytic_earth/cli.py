...
from .labels import (
    ...
    build_epk_source_free_protein_substrate_role_discriminator_audit,
    build_epk_source_free_protein_substrate_role_discriminator_stress_audit,
...
from .transfer_scope import (
    ...
    build_external_hard_negative_broader_structural_sourcing,
...
def cmd_build_predicted_geometry_robustness_audit(args: argparse.Namespace) -> int:
    audit = write_predicted_geometry_robustness_audit(
        label_manifest_path=Path(args.label_manifest),
        graph_path=Path(args.graph),
        experimental_geometry_features_path=Path(args.experimental_geometry_features),
        experimental_geometry_retrieval_path=Path(args.experimental_geometry_retrieval),
        labels_path=Path(args.labels),
        wave1_audit_path=Path(args.wave1_audit),
        out_path=Path(args.out),
        report_path=Path(args.report) if args.report else None,
    ...
    print(
        "Wrote predicted geometry robustness audit to "
        f"{args.out} ({audit.get('status')}; "
...
def cmd_build_external_source_pilot_dna_pol_x_lyase_repair_control(
    args: argparse.Namespace,
) -> int:
    artifact_payloads, artifact_lineage = _load_external_lineaged_artifacts(
        args,
        ...
        blocker_removed="dna_pol_x_lyase_repair_lane_control_staged",
    )
    with Path(args.curated_labels).open("r", encoding="utf-8") as handle:
        curated_labels = json.load(handle)
    if not isinstance(curated_labels, list):
    ...
    write_json(Path(args.out), control)
    print(
        "Wrote external source pilot DNA Pol X/5'-dRP lyase repair control to "
        f"{args.out} ({control['metadata']['candidate_count']} rows)"
    )
    return 0


def cmd_build_external_source_pilot_dna_pol_x_lyase_import_safety_adjudication(
    args: argparse.Namespace,
) -> int:
    artifact_payloads, artifact_lineage = _load_external_lineaged_artifacts(
        args,
        ...
        blocker_removed=(
            "dna_pol_x_lyase_control_integrated_into_import_safety_adjudication"
        ),
    )
    adjudication = (
        build_external_source_pilot_dna_pol_x_lyase_import_safety_adjudication(
            dna_pol_x_lyase_repair_control=artifact_payloads[
                "dna_pol_x_lyase_repair_control"
            ]
            or {},
            resolved_pilot_decisions=artifact_payloads[
                "resolved_pilot_decisions"
            ]
            or {},
            pilot_active_site_evidence_decisions=artifact_payloads[
    ...
    write_json(Path(args.out), adjudication)
    print(
        "Wrote external source pilot DNA Pol X/5'-dRP lyase import-safety "
...
def cmd_build_external_source_pilot_sugar_phosphate_isomerase_import_safety_adjudication(
    args: argparse.Namespace,
) -> int:
    artifact_payloads, artifact_lineage = _load_external_lineaged_artifacts(
        args,
    ...
    adjudication = (
        build_external_source_pilot_sugar_phosphate_isomerase_import_safety_adjudication(
            sugar_phosphate_isomerase_control=artifact_payloads[
                "sugar_phosphate_isomerase_control"
            ]
            or {},
            resolved_pilot_decisions=artifact_payloads["resolved_pilot_decisions"]
    ...
    write_json(Path(args.out), adjudication)
    print(
        "Wrote external source pilot sugar-phosphate isomerase import-safety "
        f"adjudication to {args.out} "
        f"({adjudication['metadata']['candidate_count']} rows)"
    )
    return 0
...
def cmd_build_external_source_pilot_schiff_base_lyase_control(
    args: argparse.Namespace,
) -> int:
    artifact_payloads, artifact_lineage = _load_external_lineaged_artifacts(
        args,
    ...
    write_json(Path(args.out), control)
    print(
        "Wrote external source pilot Schiff-base lyase control to "
        f"{args.out} ({control['metadata']['candidate_count']} rows)"
    )
    return 0


def cmd_build_external_source_pilot_schiff_base_lyase_import_safety_adjudication(
    args: argparse.Namespace,
) -> int:
    artifact_payloads, artifact_lineage = _load_external_lineaged_artifacts(
        args,
    ...
    adjudication = (
        build_external_source_pilot_schiff_base_lyase_import_safety_adjudication(
            schiff_base_lyase_control=artifact_payloads[
                "schiff_base_lyase_control"
            ]
            or {},
            resolved_pilot_decisions=artifact_payloads["resolved_pilot_decisions"]
    ...
    write_json(Path(args.out), adjudication)
    print(
        "Wrote external source pilot Schiff-base lyase import-safety "
        f"adjudication to {args.out} "
        f"({adjudication['metadata']['candidate_count']} rows)"
    )
    return 0
...
def cmd_build_external_hard_negative_broader_structural_sourcing(
    args: argparse.Namespace,
) -> int:
    query_manifest = read_json_object(Path(args.query_manifest))
    current_candidate_manifest = read_json_object(Path(args.current_candidate_manifest))
    ...
    prior_sourcing_paths = args.prior_new_candidate_sourcing or [
    ...
    prior_terminal_paths = args.prior_new_candidate_terminal_decisions or [
    ...
    prior_deferral_paths = args.prior_deferred_candidate_decision or [
        "artifacts/v3_external_hard_negative_p22830_cycle_deferral_1025.json",
    ]
    prior_new_candidate_sourcings = tuple(
        read_json_object(Path(path)) for path in prior_sourcing_paths
    )
    prior_new_candidate_terminal_decisions = tuple(
        read_json_object(Path(path)) for path in prior_terminal_paths
    )
    prior_deferred_candidate_decisions = tuple(
        read_json_object(Path(path)) for path in prior_deferral_paths
    )
    artifact_lineage = {
        "method": "external_transfer_artifact_path_lineage_validation",
        "slice_id": 1025,
        ...
        "artifact_paths": {
            ...
            "prior_new_candidate_sourcings": prior_sourcing_paths,
            "prior_new_candidate_terminal_decisions": prior_terminal_paths,
            "prior_deferred_candidate_decisions": prior_deferral_paths,
        },
        "blocker_removed": "broader_external_structural_sourcing",
    }
    sourcing = build_external_hard_negative_broader_structural_sourcing(
        query_manifest=query_manifest,
        current_candidate_manifest=current_candidate_manifest,
        second_tranche_terminal_decisions=second_tranche_terminal_decisions,
        ...
        min_sourced_lanes=args.min_sourced_lanes,
        artifact_lineage=artifact_lineage,
    )
    write_json(Path(args.out), sourcing)
    print(
        "Wrote broader external hard-negative structural sourcing to "
        f"{args.out} ({sourcing['metadata']['sourced_candidate_count']} "
        "sourced, "
        f"{sourcing['metadata']['sourced_lane_count']} lanes)"
    )
    return 0


def cmd_build_external_hard_negative_post_p06744_sourcing(
    args: argparse.Namespace,
) -> int:
    query_manifest = read_json_object(Path(args.query_manifest))
    current_candidate_manifest = read_json_object(Path(args.current_candidate_manifest))
    ...
    prior_terminal_paths = args.prior_new_candidate_terminal_decisions or [
        "artifacts/v3_external_hard_negative_new_candidate_terminal_decisions_1025.json",
        "artifacts/v3_external_hard_negative_next_candidate_terminal_decisions_1025.json",
        "artifacts/v3_external_hard_negative_broader_structural_terminal_decisions_1025.json",
    ]
    prior_deferral_paths = args.prior_deferred_candidate_decision or [
        "artifacts/v3_external_hard_negative_p22830_cycle_deferral_1025.json",
    ]
    ...
    artifact_lineage = {
        ...
        "blocker_removed": "post_p06744_external_sourcing",
    }
    ...
    write_json(Path(args.out), sourcing)
    print(
        "Wrote post-P06744 external hard-negative sourcing to "
        f"{args.out} ({sourcing['metadata']['sourced_candidate_count']} "
        "sourced, "
        f"{sourcing['metadata']['sourced_lane_count']} lanes)"
    )
...
def cmd_build_epk_sibling_negative_control_alternate_structure_plan(
    args: argparse.Namespace,
) -> int:
    ...
    with Path(args.graph).open("r", encoding="utf-8") as handle:
        graph = json.load(handle)
    plan = build_epk_sibling_negative_control_alternate_structure_plan(
    ...
    write_json(Path(args.out), plan)
    print(
        "Wrote ePK sibling negative-control alternate-structure plan to "
        f"{args.out} (ready_for_measurement={plan['metadata']['ready_for_future_distance_measurement_count']})"
    )
    return 0
...
def cmd_build_epk_sibling_control_repair_review(args: argparse.Namespace) -> int:
    ...
    with Path(args.epk_sibling_negative_control_alternate_structure_plan).open(
        ...
        epk_sibling_negative_control_alternate_structure_plan = json.load(handle)
    review = build_epk_sibling_control_repair_review(
    ...
    write_json(Path(args.out), review)
    print(
        "Wrote ePK sibling-control repair review to "
        f"{args.out} (status={review['metadata']['family_repair_review_status']})"
    )
    return 0
...
def cmd_build_epk_protein_substrate_acceptor_candidate_audit(
    args: argparse.Namespace,
) -> int:
    with Path(args.epk_chain_ligand_acceptor_disambiguation_audit).open(
        "r", encoding="utf-8"
        ...
        epk_chain_ligand_acceptor_disambiguation_audit = json.load(handle)
    audit = build_epk_protein_substrate_acceptor_candidate_audit(
        epk_chain_ligand_acceptor_disambiguation_audit=(
            epk_chain_ligand_acceptor_disambiguation_audit
        ),
        imported_external_entry_ids=_split_csv(args.imported_external_entry_ids),
    ...
    write_json(Path(args.out), audit)
    print(
        "Wrote ePK protein-substrate acceptor candidate audit to "
        f"{args.out} (positive_misses="
        f"{audit['metadata']['current_positive_feature_miss_count']})"
    )
    return 0


def cmd_build_epk_ligand_analog_policy_blocker_decision(
    args: argparse.Namespace,
) -> int:
    ...
    with Path(args.epk_m_csa640_alternate_gamma_geometry_review).open(
        ...
        epk_m_csa640_alternate_gamma_geometry_review = json.load(handle)
    decision = build_epk_ligand_analog_policy_blocker_decision(
    ...
    write_json(Path(args.out), decision)
    print(
        "Wrote ePK ligand-analog policy blocker decision to "
        f"{args.out} (dependencies="
        f"{decision['metadata']['ligand_analog_dependency_count']})"
    )
    return 0


def cmd_build_epk_protein_substrate_positive_source_triage(
    args: argparse.Namespace,
) -> int:
    with Path(args.active_learning_queue).open("r", encoding="utf-8") as handle:
        active_learning_queue = json.load(handle)
    ...
    with Path(args.epk_positive_fingerprint_readiness_packet).open(
        ...
        epk_positive_fingerprint_readiness_packet = json.load(handle)
    with Path(args.epk_protein_substrate_acceptor_candidate_audit).open(
        "r", encoding="utf-8"
    ) as handle:
        epk_protein_substrate_acceptor_candidate_audit = json.load(handle)
    triage = build_epk_protein_substrate_positive_source_triage(
        active_learning_queue=active_learning_queue,
        geometry_features=geometry_features,
        geometry_retrieval=geometry_retrieval,
        epk_positive_fingerprint_readiness_packet=(
            epk_positive_fingerprint_readiness_packet
        ),
        epk_protein_substrate_acceptor_candidate_audit=(
            epk_protein_substrate_acceptor_candidate_audit
    ...
    write_json(Path(args.out), triage)
    print(
        "Wrote ePK protein-substrate positive source triage to "
        f"{args.out} (candidates={triage['metadata']['candidate_row_count']})"
    )
    return 0


def cmd_build_epk_m_csa760_atp_state_repair_scan(
    args: argparse.Namespace,
) -> int:
    ...
    scan = build_epk_m_csa760_atp_state_repair_scan(
    ...
    print(
        "Wrote ePK m_csa:760 ATP-state repair scan to "
...
def cmd_build_epk_source_free_protein_substrate_role_discriminator_audit(
    args: argparse.Namespace,
) -> int:
    with Path(args.epk_protein_substrate_acceptor_candidate_audit).open(
        "r", encoding="utf-8"
    ...
    if args.epk_length_band_substrate_identity_counteraxis_audit:
        with Path(
            ...
            epk_length_band_substrate_identity_counteraxis_audit = json.load(handle)
    audit = build_epk_source_free_protein_substrate_role_discriminator_audit(
        epk_protein_substrate_acceptor_candidate_audit=(
            epk_protein_substrate_acceptor_candidate_audit
        ),
        epk_heteromeric_chain_topology_signal_audit=(
    ...
    write_json(Path(args.out), audit)
    print(
        "Wrote ePK source-free protein-substrate role discriminator audit to "
        f"{args.out} (status="
        f"{audit['metadata']['protein_substrate_role_discriminator_status']})"
    )
    return 0


def cmd_build_epk_source_free_protein_substrate_role_discriminator_stress_audit(
    args: argparse.Namespace,
) -> int:
    with Path(args.epk_source_free_protein_substrate_role_discriminator_audit).open(
        "r", encoding="utf-8"
    ...
    with Path(args.epk_heteromeric_source_expansion_peptide_role_axis_audit).open(
        "r", encoding="utf-8"
    ) as handle:
        epk_heteromeric_source_expansion_peptide_role_axis_audit = json.load(
            handle
        )
    audit = build_epk_source_free_protein_substrate_role_discriminator_stress_audit(
        epk_source_free_protein_substrate_role_discriminator_audit=(
            epk_source_free_protein_substrate_role_discriminator_audit
        ),
        epk_heteromeric_source_expansion_peptide_role_axis_audit=(
    ...
    write_json(Path(args.out), audit)
    print(
        "Wrote ePK source-free protein-substrate role discriminator stress "
        f"audit to {args.out} (status="
        f"{audit['metadata']['protein_role_source_expansion_stress_status']})"
    )
    return 0
...
def cmd_build_epk_nonready_ligand_exclusion_decision(
    args: argparse.Namespace,
) -> int:
    with Path(args.epk_nonready_ligand_repair_plan).open(
        "r", encoding="utf-8"
        ...
        epk_nonready_ligand_repair_plan = json.load(handle)
    with Path(args.epk_nonready_ligand_alternate_structure_plan).open(
        "r", encoding="utf-8"
    ) as handle:
        epk_nonready_ligand_alternate_structure_plan = json.load(handle)
    decision = build_epk_nonready_ligand_exclusion_decision(
        epk_nonready_ligand_repair_plan=epk_nonready_ligand_repair_plan,
    ...
    write_json(Path(args.out), decision)
    print(
        "Wrote ePK non-ready ligand exclusion decision to "
        f"{args.out} (excluded={decision['metadata']['excluded_nonready_row_count']})"
    )
    return 0
...
def cmd_build_epk_precount_gate_status(args: argparse.Namespace) -> int:
    ...
    with Path(args.epk_nonready_ligand_repair_plan).open(
        ...
        epk_nonready_ligand_repair_plan = json.load(handle)
    epk_nonready_ligand_exclusion_decision = None
    if args.epk_nonready_ligand_exclusion_decision:
        with Path(args.epk_nonready_ligand_exclusion_decision).open(
            "r", encoding="utf-8"
        ) as handle:
            epk_nonready_ligand_exclusion_decision = json.load(handle)
    ...
    if args.epk_negative_control_gamma_distance_distribution:
        with Path(args.epk_negative_control_gamma_distance_distribution).open(
            ...
            epk_negative_control_gamma_distance_distribution = json.load(handle)
    epk_sibling_negative_control_alternate_structure_plan = None
    if args.epk_sibling_negative_control_alternate_structure_plan:
        with Path(args.epk_sibling_negative_control_alternate_structure_plan).open(
            "r", encoding="utf-8"
        ) as handle:
            epk_sibling_negative_control_alternate_structure_plan = json.load(handle)
    ...
    if args.epk_chain_ligand_external_hard_negative_feature_screen:
        with Path(args.epk_chain_ligand_external_hard_negative_feature_screen).open(
            ...
            epk_chain_ligand_external_hard_negative_feature_screen = json.load(handle)
    epk_protein_substrate_acceptor_candidate_audit = None
    if args.epk_protein_substrate_acceptor_candidate_audit:
        with Path(args.epk_protein_substrate_acceptor_candidate_audit).open(
            "r", encoding="utf-8"
        ) as handle:
            epk_protein_substrate_acceptor_candidate_audit = json.load(handle)
    epk_ligand_analog_policy_blocker_decision = None
    if args.epk_ligand_analog_policy_blocker_decision:
        with Path(args.epk_ligand_analog_policy_blocker_decision).open(
            "r", encoding="utf-8"
        ) as handle:
            epk_ligand_analog_policy_blocker_decision = json.load(handle)
    ...
    epk_source_free_protein_substrate_role_discriminator_audit = None
    if args.epk_source_free_protein_substrate_role_discriminator_audit:
        with Path(
            args.epk_source_free_protein_substrate_role_discriminator_audit
        ).open("r", encoding="utf-8") as handle:
            epk_source_free_protein_substrate_role_discriminator_audit = json.load(
                handle
            )
    epk_source_free_protein_substrate_role_discriminator_stress_audit = None
    if args.epk_source_free_protein_substrate_role_discriminator_stress_audit:
        with Path(
            args.epk_source_free_protein_substrate_role_discriminator_stress_audit
        ).open("r", encoding="utf-8") as handle:
            epk_source_free_protein_substrate_role_discriminator_stress_audit = json.load(
                handle
            )
...
def cmd_eval_mechanism_abstention_gate(args: argparse.Namespace) -> int:
    audit = write_mechanism_abstention_gate_eval(
        esm2_150m_path=Path(args.esm2_150m_embeddings),
    ...
    print(
        "Wrote mechanism abstention gate eval to "
...
def cmd_build_predicted_atlas_geometry_novelty_operating_grid(
    args: argparse.Namespace,
) -> int:
    audit = write_predicted_atlas_geometry_novelty_operating_grid(
    ...
    print(
        "Wrote predicted-atlas geometry novelty operating grid to "
...
def cmd_build_family_panel_source_free_predicted_geometry_sidecar_manifest(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_predicted_geometry_sidecar_manifest(
        source_backed_materialization_path=Path(args.source_backed_materialization),
        predicted_geometry_atlas_path=Path(args.predicted_geometry_atlas),
        label_manifest_path=Path(args.label_manifest),
...
def cmd_build_family_panel_source_free_active_site_locator_schema(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_active_site_locator_schema(
        source_free_geometry_manifest_path=Path(args.source_free_geometry_manifest),
        out_path=Path(args.out),
        report_path=Path(args.report) if args.report else None,
    )
    print(
        "Wrote family-panel source-free active-site locator schema to "
        f"{args.out} (status: {audit.get('status')}, "
        f"target rows: {audit.get('counts', {}).get('target_rows')})"
    )
    return 0


def cmd_audit_family_panel_source_free_active_site_locator_schema(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_active_site_locator_schema_audit(
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote family-panel source-free active-site locator schema audit to "
        f"{args.out} (status: {audit.get('status')}, "
...
def cmd_build_family_panel_source_free_active_site_locator_materialization_plan(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_active_site_locator_materialization_plan(
        source_free_geometry_manifest_path=Path(args.source_free_geometry_manifest),
        locator_schema_path=Path(args.locator_schema),
        locator_schema_audit_path=Path(args.locator_schema_audit),
        locator_dir=Path(args.locator_dir),
    ...
    print(
        "Wrote family-panel source-free active-site locator materialization plan to "
        f"{args.out} (planned rows: {audit.get('counts', {}).get('planned_rows')})"
    )
    return 0


def cmd_build_family_panel_source_free_active_site_locator_template_bundle(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_active_site_locator_template_bundle(
        locator_materialization_plan_path=Path(args.locator_materialization_plan),
        template_dir=Path(args.template_dir),
        out_path=Path(args.out),
        report_path=Path(args.report) if args.report else None,
    )
    print(
        "Wrote family-panel source-free active-site locator template bundle to "
        f"{args.out} (templates: {audit.get('counts', {}).get('templates')})"
    )
    return 0


def cmd_build_family_panel_source_free_active_site_locator_candidate_audit(
    args: argparse.Namespace,
) -> int:
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote family-panel source-free active-site locator candidate audit to "
        f"{args.out} (candidate rows: {counts.get('candidate_sidecars_staged')}, "
        "ready rows: "
...
def cmd_build_family_panel_source_free_active_site_locator_review_queue(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_active_site_locator_review_queue(
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote family-panel source-free active-site locator review queue to "
        f"{args.out} (priority-1 rows: "
        f"{counts.get('ready_for_manual_forbidden_feature_review')})"
    )
    return 0


def cmd_audit_family_panel_source_free_active_site_locator_candidate_integrity(
    args: argparse.Namespace,
) -> int:
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote family-panel source-free active-site locator candidate integrity "
        f"audit to {args.out} (passed: "
        f"{counts.get('integrity_passed_sidecars')}, ready rows: "
        f"{counts.get('ready_for_predicted_geometry_scoring')})"
    )
    return 0


def cmd_build_family_panel_source_free_active_site_locator_manual_review_packet(
    args: argparse.Namespace,
) -> int:
    audit = write_family_panel_source_free_active_site_locator_manual_review_packet(
        candidate_audit_path=Path(args.candidate_audit),
        candidate_integrity_audit_path=Path(args.candidate_integrity_audit),
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote family-panel source-free active-site locator manual review "
...
def cmd_score_fold_augmented_train_cal_oos_negative_surface(args: argparse.Namespace) -> int:
    audit = write_fold_augmented_train_cal_oos_negative_surface_scores(
        negative_surface_manifest_path=Path(args.negative_surface_manifest),
        label_manifest_path=Path(args.label_manifest),
        graph_path=Path(args.graph),
        experimental_geometry_features_path=Path(args.experimental_geometry_features),
        threshold_contract_path=Path(args.threshold_contract),
        ...
        selected_organic_cofactor_sidecar_path=Path(args.selected_organic_cofactor_sidecar),
        coordinate_root=Path(args.coordinate_root),
        train_cal_oos_foldseek_tsv=Path(args.train_cal_oos_foldseek_tsv),
        out_path=Path(args.out),
        report_path=Path(args.report) if args.report else None,
        foldseek_binary=args.foldseek_binary,
        alphafold_version=args.alphafold_version,
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote fold-augmented train/cal OOS negative surface scores to "
        f"{args.out} (status: {audit.get('status')}, "
        f"full score rows: {counts.get('candidate_rows_with_full_channel_scores')})"
    )
    return 0


def cmd_eval_fold_augmented_oos_calibrated_threshold_contract(args: argparse.Namespace) -> int:
    audit = write_fold_augmented_oos_calibrated_threshold_contract(
        threshold_contract_path=Path(args.threshold_contract),
...
def cmd_build_fold_only_train_cal_oos_negative_surface(args: argparse.Namespace) -> int:
    audit = write_fold_only_train_cal_oos_negative_surface(
        train_cal_oos_surface_path=Path(args.train_cal_oos_surface),
        out_path=Path(args.out),
        report_path=Path(args.report) if args.report else None,
    )
    counts = audit.get("counts", {})
    print(
        "Wrote fold-only train/cal OOS negative surface to "
        f"{args.out} (rows: {counts.get('fold_only_rows')})"
    )
    return 0


def cmd_build_fold_augmented_train_cal_oos_negative_surface_blocker_resolution(
    args: argparse.Namespace,
) -> int:
    audit = write_fold_augmented_train_cal_oos_negative_surface_blocker_resolution(
        train_cal_oos_surface_path=Path(args.train_cal_oos_surface),
        out_path=Path(args.out),
        report_path=Path(args.report) if args.report else None,
    )
    counts = audit.get("counts", {})
    print(
        "Wrote fold-augmented train/cal OOS blocker resolution to "
        f"{args.out} (missing rows: {counts.get('missing_full_score_rows')})"
    )
    return 0
...
def cmd_build_mechanism_feature_active_site_role_graph_sidecar(args: argparse.Namespace) -> int:
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote mechanism-feature active-site role graph sidecar to "
        f"{args.out} (ok rows: {counts.get('rows_with_ok_role_graph')})"
    )
    return 0


def cmd_build_mechanism_feature_reaction_center_template_sidecar(args: argparse.Namespace) -> int:
    audit = write_mechanism_feature_reaction_center_template_sidecar(
        label_manifest_path=Path(args.label_manifest),
...
def cmd_build_learned_mechanism_feature_embedding_plan(args: argparse.Namespace) -> int:
    audit = write_learned_mechanism_feature_embedding_plan(
        ...
        active_site_role_graph_sidecar_path=(
            Path(args.active_site_role_graph_sidecar)
            if args.active_site_role_graph_sidecar
            else None
        ),
...
def cmd_build_mechanism_feature_embedding_train_cal_input_manifest(
    args: argparse.Namespace,
) -> int:
    audit = write_mechanism_feature_embedding_train_cal_input_manifest(
        learned_embedding_plan_path=Path(args.learned_embedding_plan),
        label_manifest_path=Path(args.label_manifest),
        active_site_role_graph_sidecar_path=Path(args.active_site_role_graph_sidecar),
        reaction_center_template_sidecar_path=Path(args.reaction_center_template_sidecar),
        selected_organic_cofactor_sidecar_path=Path(args.selected_organic_cofactor_sidecar),
        inorganic_cofactor_completion_audit_path=Path(
            args.inorganic_cofactor_completion_audit
        ),
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote mechanism-feature embedding train/cal input manifest to "
        f"{args.out} (ready rows: {counts.get('minimal_feature_bundle_ready_rows')})"
    )
    return 0


def cmd_build_mechanism_feature_embedding_train_cal_split_manifest(
    args: argparse.Namespace,
) -> int:
    audit = write_mechanism_feature_embedding_train_cal_split_manifest(
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote mechanism-feature embedding train/cal split manifest to "
        f"{args.out} (train rows: {counts.get('train_rows')}, "
        f"calibration rows: {counts.get('calibration_rows')})"
    )
    return 0


def cmd_build_mechanism_feature_embedding_feature_contract(
    args: argparse.Namespace,
) -> int:
...
def cmd_build_current_docs_artifact_reference_check(args: argparse.Namespace) -> int:
    doc_paths = [Path(path) for path in args.docs] if args.docs else None
    ...
    counts = audit.get("counts", {})
    print(
        "Wrote current-docs artifact reference check to "
        f"{args.out} (missing: {counts.get('missing_references')})"
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    current_docs_reference_check = subparsers.add_parser(
        "build-current-docs-artifact-reference-check",
        help=(
            "check durable current docs for concrete repo-path references, "
            "excluding globs and template placeholders"
        ),
    )
    current_docs_reference_check.add_argument(
        "--docs",
        nargs="*",
        default=None,
        help="Docs to check; defaults to the durable current-doc set",
    )
    current_docs_reference_check.add_argument(
        "--out",
        default=(
            "artifacts/v3_current_docs_artifact_reference_check_"
            "current702_20260601.json"
        ),
    )
    current_docs_reference_check.add_argument(
        "--report",
        default="work/current_docs_artifact_reference_check_current702_20260601.md",
    )
    current_docs_reference_check.set_defaults(
        func=cmd_build_current_docs_artifact_reference_check
    )
    ...
    predicted_geometry_robustness = subparsers.add_parser(
        "build-predicted-geometry-robustness-audit",
        help="score frozen geometry router/heads on AlphaFoldDB predicted geometry",
    )
    ...
    predicted_geometry_robustness.add_argument(
        "--experimental-geometry-retrieval",
        default="artifacts/v3_geometry_retrieval_1025.json",
    )
    predicted_geometry_robustness.add_argument(
        "--labels",
    ...
    predicted_geometry_robustness.add_argument("--random-state", type=int, default=702)
    predicted_geometry_robustness.add_argument("--cal-fraction", type=float, default=0.2)
    predicted_geometry_robustness.add_argument("--hidden-layer-size", type=int, default=32)
    predicted_geometry_robustness.add_argument("--max-rows", type=int, default=0)
    predicted_geometry_robustness.add_argument(
        "--out",
        default=(
            "artifacts/"
    ...
    predicted_geometry_robustness.add_argument(
        "--report",
        default="work/predicted_geometry_robustness_audit_current702_20260529.md",
    )
    ...
    external_pilot_dna_pol_x_lyase_control = subparsers.add_parser(
        "build-external-source-pilot-dna-pol-x-lyase-repair-control",
        help=(
            "stage a review-only sequence-derived DNA Pol X/5'-dRP lyase "
            "contrast control for the selected external pilot repair lane"
        ),
    )
    external_pilot_dna_pol_x_lyase_control.add_argument(
        "--repair-lanes",
        default=(
            "artifacts/"
            "v3_external_source_pilot_mechanism_repair_lanes_1025.json"
    ...
    external_pilot_dna_pol_x_lyase_control.set_defaults(
        func=cmd_build_external_source_pilot_dna_pol_x_lyase_repair_control
    )

    external_pilot_dna_pol_x_lyase_import_safety = subparsers.add_parser(
        "build-external-source-pilot-dna-pol-x-lyase-import-safety-adjudication",
        help=(
            "adjudicate the DNA Pol X/5'-dRP lyase repair control inside the "
            "external pilot import-safety decision path"
        ),
    )
    external_pilot_dna_pol_x_lyase_import_safety.add_argument(
        "--dna-pol-x-lyase-repair-control",
        default=(
            "artifacts/"
            "v3_external_source_pilot_dna_pol_x_lyase_repair_control_1025.json"
        ),
    ...
    external_pilot_dna_pol_x_lyase_import_safety.set_defaults(
        func=(
            cmd_build_external_source_pilot_dna_pol_x_lyase_import_safety_adjudication
        )
    )
    ...
    external_pilot_sugar_isomerase_import_safety = subparsers.add_parser(
        "build-external-source-pilot-sugar-phosphate-isomerase-import-safety-adjudication",
        help=(
            "adjudicate the sugar-phosphate isomerase scope control inside the "
            "external pilot import-safety decision path"
        ),
    )
    external_pilot_sugar_isomerase_import_safety.add_argument(
        "--sugar-phosphate-isomerase-control",
        default=(
            "artifacts/"
            "v3_external_source_pilot_sugar_phosphate_isomerase_control_1025.json"
        ),
    ...
    external_pilot_sugar_isomerase_import_safety.set_defaults(
        func=cmd_build_external_source_pilot_sugar_phosphate_isomerase_import_safety_adjudication
    )

    external_pilot_schiff_base_lyase = subparsers.add_parser(
        "build-external-source-pilot-schiff-base-lyase-control",
        help=(
            "stage a review-only Schiff-base lyase/aldolase versus weak heme "
            "peroxidase scope control for the selected external pilot repair lane"
        ),
    )
    external_pilot_schiff_base_lyase.add_argument(
        "--repair-lanes",
        default=(
            "artifacts/"
            "v3_external_source_pilot_mechanism_repair_lanes_1025.json"
    ...
    external_pilot_schiff_base_lyase.set_defaults(
        func=cmd_build_external_source_pilot_schiff_base_lyase_control
    )

    external_pilot_schiff_base_lyase_import_safety = subparsers.add_parser(
        "build-external-source-pilot-schiff-base-lyase-import-safety-adjudication",
        help=(
            "adjudicate the Schiff-base lyase/aldolase scope control inside "
            "the external pilot import-safety decision path"
        ),
    )
    external_pilot_schiff_base_lyase_import_safety.add_argument(
        "--schiff-base-lyase-control",
        default=(
            "artifacts/"
            "v3_external_source_pilot_schiff_base_lyase_control_1025.json"
        ),
    ...
    external_pilot_schiff_base_lyase_import_safety.set_defaults(
        func=cmd_build_external_source_pilot_schiff_base_lyase_import_safety_adjudication
    )
    ...
    external_hard_negative_broader_sourcing = subparsers.add_parser(
        "build-external-hard-negative-broader-structural-sourcing",
        help=(
            "source a multi-prior, lane-balanced review-only external "
            "hard-negative structural surface"
        ),
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--query-manifest",
        default="artifacts/v3_external_source_query_manifest_1025.json",
    )
    external_hard_negative_broader_sourcing.add_argument(
    ...
    external_hard_negative_broader_sourcing.add_argument(
        ...
        default=None,
        help=(
            "prior sourcing artifact to exclude; may be provided more than once"
        ),
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--prior-new-candidate-terminal-decisions",
        ...
        default=None,
        help=(
            "prior terminal-decision artifact to exclude; may be provided more "
            "than once"
        ),
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--prior-deferred-candidate-decision",
        action="append",
        default=None,
        help=(
            "prior explicit deferral artifact to exclude; may be provided more "
            "than once"
        ),
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--max-records-per-lane", type=int, default=50
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--max-active-site-fetches", type=int, default=96
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--max-candidates", type=int, default=8
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--max-candidates-per-lane", type=int, default=2
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--min-sourced-lanes", type=int, default=3
    )
    external_hard_negative_broader_sourcing.add_argument(
        "--out",
        default=(
    ...
    external_hard_negative_broader_sourcing.set_defaults(
        func=cmd_build_external_hard_negative_broader_structural_sourcing
    )

    external_hard_negative_post_p06744_sourcing = subparsers.add_parser(
        "build-external-hard-negative-post-p06744-sourcing",
        help=(
            "source a review-only external hard-negative surface after the "
            "P06744 import"
        ),
    )
    external_hard_negative_post_p06744_sourcing.add_argument(
        "--query-manifest",
        default="artifacts/v3_external_source_query_manifest_1025.json",
    )
    external_hard_negative_post_p06744_sourcing.add_argument(
    ...
    external_hard_negative_post_p06744_sourcing.add_argument(
        "--max-records-per-lane", type=int, default=70
    )
    external_hard_negative_post_p06744_sourcing.add_argument(
        "--max-active-site-fetches", type=int, default=140
    )
    external_hard_negative_post_p06744_sourcing.add_argument(
        "--max-candidates", type=int, default=8
    )
    ...
    external_hard_negative_post_p06744_sourcing.set_defaults(
        func=cmd_build_external_hard_negative_post_p06744_sourcing
    )
    ...
    epk_sibling_negative_control_alternate_plan = subparsers.add_parser(
        "build-epk-sibling-negative-control-alternate-structure-plan",
        help="screen alternate structures for sibling ePK negative controls",
    )
    epk_sibling_negative_control_alternate_plan.add_argument(
    ...
    epk_sibling_negative_control_alternate_plan.add_argument(
        "--entry-ids",
        default="",
        help="comma-separated entry ids to screen; defaults to unmeasured controls",
    )
    epk_sibling_negative_control_alternate_plan.add_argument(
        "--max-structures-per-entry",
        type=int,
        default=8,
    )
    epk_sibling_negative_control_alternate_plan.add_argument("--cif-dir", default=None)
    epk_sibling_negative_control_alternate_plan.add_argument(
        "--out",
        default=(
            "artifacts/"
    ...
    epk_sibling_negative_control_alternate_plan.set_defaults(
    ...
    epk_sibling_control_repair_review = subparsers.add_parser(
        "build-epk-sibling-control-repair-review",
        help="review one missing ePK sibling control family for local repair status",
    )
    epk_sibling_control_repair_review.add_argument(
    ...
    epk_sibling_control_repair_review.add_argument(
        "--family-id",
        default="pfkb",
        help="missing sibling family id to review",
    )
    epk_sibling_control_repair_review.add_argument("--cif-dir", default=None)
    epk_sibling_control_repair_review.add_argument(
        "--out",
        default="artifacts/v3_epk_sibling_control_repair_review.json",
    )
    epk_sibling_control_repair_review.set_defaults(
    ...
    epk_protein_substrate_acceptor = subparsers.add_parser(
        "build-epk-protein-substrate-acceptor-candidate-audit",
        help="audit a stricter review-only protein-substrate ePK acceptor feature",
    )
    epk_protein_substrate_acceptor.add_argument(
        "--epk-chain-ligand-acceptor-disambiguation-audit",
        default=(
            "artifacts/"
            "v3_epk_chain_ligand_acceptor_disambiguation_audit.json"
    ...
    epk_protein_substrate_acceptor.set_defaults(
        func=cmd_build_epk_protein_substrate_acceptor_candidate_audit
    )

    epk_ligand_analog_policy = subparsers.add_parser(
        "build-epk-ligand-analog-policy-blocker-decision",
        help="record review-only ePK ligand-analog production-admissibility blockers",
    )
    epk_ligand_analog_policy.add_argument(
    ...
    epk_ligand_analog_policy.add_argument(
        ...
        default=(
            "artifacts/"
            "v3_epk_ligand_analog_policy_blocker_decision.json"
        ),
    )
    epk_ligand_analog_policy.set_defaults(
        func=cmd_build_epk_ligand_analog_policy_blocker_decision
    )

    epk_positive_source_triage = subparsers.add_parser(
        "build-epk-protein-substrate-positive-source-triage",
        help="triage review-only ePK source rows for protein-substrate coverage",
    )
    epk_positive_source_triage.add_argument(
        "--active-learning-queue",
        default="artifacts/v3_active_learning_review_queue_1000.json",
    )
    epk_positive_source_triage.add_argument(
        "--geometry",
        default="artifacts/v3_geometry_features_1000.json",
    ...
    epk_positive_source_triage.add_argument("--max-rows", type=int, default=10)
    epk_positive_source_triage.add_argument(
        "--out",
        default=(
            "artifacts/"
    ...
    epk_positive_source_triage.set_defaults(
        func=cmd_build_epk_protein_substrate_positive_source_triage
    )

    epk_m_csa760_atp_repair = subparsers.add_parser(
        "build-epk-m-csa760-atp-state-repair-scan",
        help="scan review-only m_csa:760 alternate structures for ATP/substrate context",
    )
    epk_m_csa760_atp_repair.add_argument(
    ...
    epk_m_csa760_atp_repair.add_argument("--entry-id", default="m_csa:760")
    epk_m_csa760_atp_repair.add_argument("--cif-dir", default=None)
    epk_m_csa760_atp_repair.add_argument(
        "--out",
        default="artifacts/v3_epk_m_csa760_atp_state_repair_scan.json",
    )
    epk_m_csa760_atp_repair.set_defaults(
    ...
    epk_protein_role_discriminator = subparsers.add_parser(
        "build-epk-source-free-protein-substrate-role-discriminator-audit",
        help=(
            "audit a review-only source-free protein-substrate role "
            "discriminator for ePK"
        ),
    )
    epk_protein_role_discriminator.add_argument(
        "--epk-protein-substrate-acceptor-candidate-audit",
        default=(
            "artifacts/"
            "v3_epk_protein_substrate_acceptor_candidate_audit_1025.json"
    ...
    epk_protein_role_discriminator.set_defaults(
        func=cmd_build_epk_source_free_protein_substrate_role_discriminator_audit
    )

    epk_protein_role_discriminator_stress = subparsers.add_parser(
        "build-epk-source-free-protein-substrate-role-discriminator-stress-audit",
        help=(
            "stress the review-only source-free protein-substrate role "
            "discriminator against source-expansion controls"
        ),
    )
    epk_protein_role_discriminator_stress.add_argument(
        "--epk-source-free-protein-substrate-role-discriminator-audit",
        default=(
            "artifacts/"
            "v3_epk_source_free_protein_substrate_role_discriminator_audit_1025.json"
    ...
    epk_protein_role_discriminator_stress.set_defaults(
    ...
    epk_nonready_exclusion_decision = subparsers.add_parser(
        "build-epk-nonready-ligand-exclusion-decision",
        help="decide review-only calibration exclusion for non-ready ePK ligand rows",
    )
    epk_nonready_exclusion_decision.add_argument(
        "--epk-nonready-ligand-repair-plan",
        default="artifacts/v3_epk_nonready_ligand_repair_plan.json",
    )
    epk_nonready_exclusion_decision.add_argument(
    ...
    epk_nonready_exclusion_decision.add_argument(
        "--out",
        default="artifacts/v3_epk_nonready_ligand_exclusion_decision.json",
    )
    epk_nonready_exclusion_decision.set_defaults(
        func=cmd_build_epk_nonready_ligand_exclusion_decision
    )
    ...
    epk_precount_gate_status.add_argument(
        "--epk-nonready-ligand-exclusion-decision",
        default=None,
    )
    epk_precount_gate_status.add_argument(
        "--epk-acceptor-identity-review",
    ...
    abstention_gate = subparsers.add_parser(
        "eval-mechanism-abstention-gate",
        help=(
            "D11 de novo precondition: two-channel abstention gate "
            "(cofactor-augmented PLM + geometry role-agreement)"
        ),
    )
    abstention_gate.add_argument(
        "--esm2-150m-embeddings",
        default="artifacts/representation_tracks/esm2_150m/esm2_150m_embeddings_current702_20260525.jsonl",
    )
    abstention_gate.add_argument(
    ...
    abstention_gate.add_argument(
        ...
        default="artifacts/v3_geometry_retrieval_1025.json",
        help=(
            "geometry retrieval artifact with per-row role_match_fraction; "
            "source-agnostic so a predicted-geometry retrieval drops in"
        ),
    )
    abstention_gate.add_argument(
        "--out",
        default="artifacts/v3_mechanism_abstention_gate_eval_current702_20260531.json",
    )
    abstention_gate.add_argument(
        "--report",
    ...
    predicted_atlas_geometry_operating_grid = subparsers.add_parser(
        "build-predicted-atlas-geometry-novelty-operating-grid",
        help=(
            "summarize post-hoc retention/OOS-abstention operating points for "
            "existing predicted-atlas geometry novelty variants"
        ),
    )
    predicted_atlas_geometry_operating_grid.add_argument(
    ...
    predicted_atlas_geometry_operating_grid.add_argument(
        "--report",
        default=(
            "work/predicted_atlas_geometry_novelty_operating_grid_"
            "current702_20260601.md"
        ),
    )
    predicted_atlas_geometry_operating_grid.set_defaults(
    ...
    family_panel_source_free_geometry_manifest = subparsers.add_parser(
        "build-family-panel-source-free-predicted-geometry-sidecar-manifest",
        help=(
            "stage the source-free predicted active-site geometry sidecar "
            "readiness manifest for fold-scored family-panel rows"
        ),
    )
    ...
    family_panel_source_free_geometry_manifest.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_predicted_geometry_sidecar_manifest_"
            "current702_20260601.md"
        ),
    )
    ...
    family_panel_source_free_locator_schema = subparsers.add_parser(
        ...
        help=(
            "write the strict source-free active-site locator sidecar schema "
            "needed before external family-panel predicted geometry scoring"
        ),
    )
    family_panel_source_free_locator_schema.add_argument(
        "--source-free-geometry-manifest",
        default=(
            "artifacts/v3_family_panel_source_free_predicted_geometry_sidecar_"
            "manifest_current702_20260601.json"
    ...
    family_panel_source_free_locator_schema.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_active_site_locator_schema_"
            "current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_schema.set_defaults(
        func=cmd_build_family_panel_source_free_active_site_locator_schema
    )

    family_panel_source_free_locator_schema_audit = subparsers.add_parser(
        "audit-family-panel-source-free-active-site-locator-schema",
        help=(
            "validate source-free active-site locator sidecars before "
            "family-panel predicted-geometry scoring"
        ),
    )
    family_panel_source_free_locator_schema_audit.add_argument(
        "--locator-schema",
        default=(
            "artifacts/v3_family_panel_source_free_active_site_locator_schema_"
            "current702_20260601.json"
    ...
    family_panel_source_free_locator_schema_audit.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_active_site_locator_schema_audit_"
            "current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_schema_audit.set_defaults(
        func=cmd_audit_family_panel_source_free_active_site_locator_schema
    )

    family_panel_source_free_locator_plan = subparsers.add_parser(
        "build-family-panel-source-free-active-site-locator-materialization-plan",
        help=(
            "write the per-row materialization plan for source-free active-site "
            "locator sidecars"
        ),
    )
    family_panel_source_free_locator_plan.add_argument(
        "--source-free-geometry-manifest",
        default=(
            "artifacts/v3_family_panel_source_free_predicted_geometry_sidecar_"
            "manifest_current702_20260601.json"
    ...
    family_panel_source_free_locator_plan.add_argument(
        ...
        default=(
            ...
            "materialization_plan_current702_20260601.json"
        ),
    )
    family_panel_source_free_locator_plan.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_active_site_locator_materialization_plan_"
            "current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_plan.set_defaults(
        func=cmd_build_family_panel_source_free_active_site_locator_materialization_plan
    )

    family_panel_source_free_locator_templates = subparsers.add_parser(
        "build-family-panel-source-free-active-site-locator-template-bundle",
        help=(
            "write template-only source-free active-site locator sidecars outside "
            "the audited scoring directory"
        ),
    )
    family_panel_source_free_locator_templates.add_argument(
        "--locator-materialization-plan",
        default=(
            "artifacts/v3_family_panel_source_free_active_site_locator_"
            "materialization_plan_current702_20260601.json"
        ),
    ...
    family_panel_source_free_locator_templates.add_argument(
        "--template-dir",
        default=(
            "artifacts/family_panel_source_free_active_site_locator_templates_"
            "current702_20260601"
        ),
    )
    family_panel_source_free_locator_templates.add_argument(
        ...
        default=(
            "artifacts/v3_family_panel_source_free_active_site_locator_"
            "template_bundle_current702_20260601.json"
        ),
    )
    family_panel_source_free_locator_templates.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_active_site_locator_template_bundle_"
            "current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_templates.set_defaults(
        func=cmd_build_family_panel_source_free_active_site_locator_template_bundle
    )

    family_panel_source_free_locator_candidates = subparsers.add_parser(
        "build-family-panel-source-free-active-site-locator-candidate-audit",
        help=(
            "write review-only coordinate-contact candidate sidecars outside "
            "the audited source-free locator directory"
        ),
    )
    family_panel_source_free_locator_candidates.add_argument(
        "--source-free-geometry-manifest",
        default=(
            "artifacts/v3_family_panel_source_free_predicted_geometry_sidecar_"
            "manifest_current702_20260601.json"
    ...
    family_panel_source_free_locator_candidates.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_active_site_locator_candidate_audit_"
            "current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_candidates.set_defaults(
        func=cmd_build_family_panel_source_free_active_site_locator_candidate_audit
    )

    family_panel_source_free_locator_review_queue = subparsers.add_parser(
        "build-family-panel-source-free-active-site-locator-review-queue",
        help=(
            "rank source-free locator candidate sidecars for manual review "
            "without creating audited locator sidecars"
        ),
    )
    family_panel_source_free_locator_review_queue.add_argument(
        "--candidate-audit",
        default=(
            "artifacts/v3_family_panel_source_free_active_site_locator_"
            "candidate_audit_current702_20260601.json"
    ...
    family_panel_source_free_locator_review_queue.add_argument(
        "--report",
        default=(
            "work/family_panel_source_free_active_site_locator_review_queue_"
            "current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_review_queue.set_defaults(
        func=cmd_build_family_panel_source_free_active_site_locator_review_queue
    )

    family_panel_source_free_locator_candidate_integrity = subparsers.add_parser(
        "audit-family-panel-source-free-active-site-locator-candidate-integrity",
        help=(
            "audit review-only locator candidate sidecar file integrity without "
            "creating audited locator sidecars"
        ),
    )
    family_panel_source_free_locator_candidate_integrity.add_argument(
        "--candidate-audit",
        default=(
            "artifacts/v3_family_panel_source_free_active_site_locator_"
            "candidate_audit_current702_20260601.json"
    ...
    family_panel_source_free_locator_candidate_integrity.add_argument(
        ...
        default=(
            ...
            "candidate_integrity_audit_current702_20260601.md"
        ),
    )
    family_panel_source_free_locator_candidate_integrity.set_defaults(
        func=cmd_audit_family_panel_source_free_active_site_locator_candidate_integrity
    )

    family_panel_source_free_locator_manual_review_packet = subparsers.add_parser(
        "build-family-panel-source-free-active-site-locator-manual-review-packet",
        help=(
            "combine locator candidate extraction, integrity, and review queue "
            "artifacts into a manual review packet"
        ),
    )
    family_panel_source_free_locator_manual_review_packet.add_argument(
        "--candidate-audit",
        default=(
            "artifacts/v3_family_panel_source_free_active_site_locator_"
            "candidate_audit_current702_20260601.json"
    ...
    family_panel_source_free_locator_manual_review_packet.add_argument(
        ...
        default=(
            "work/family_panel_source_free_active_site_locator_"
            "manual_review_packet_current702_20260601.md"
        ),
    )
    ...
    train_cal_oos_surface = subparsers.add_parser(
        "score-fold-augmented-train-cal-oos-negative-surface",
        help=(
            "score hash-selected in-distribution OOS calibration negatives with "
            "predicted geometry plus Foldseek/TM against the threshold-contract train atlas"
        ),
    )
    train_cal_oos_surface.add_argument(
        "--negative-surface-manifest",
        default=(
            "artifacts/v3_fold_augmented_train_cal_oos_negative_surface_manifest_"
            "current702_20260601.json"
        ),
    )
    train_cal_oos_surface.add_argument(
    ...
    train_cal_oos_surface.add_argument("--graph", default="artifacts/v1_graph_1025.json")
    train_cal_oos_surface.add_argument(
        "--experimental-geometry-features",
        default="artifacts/v3_geometry_features_1025.json",
    )
    ...
    train_cal_oos_surface.add_argument(
        ...
        default=(
            "artifacts/v3_predicted_structure_fold_channel_current702_20260601_"
            "coordinates_foldseek_results/train_cal_oos_negatives_vs_train_atlas.tsv"
        ),
    )
    train_cal_oos_surface.add_argument(
        "--foldseek-binary",
    ...
    train_cal_oos_surface.add_argument(
        "--report",
        default=(
            "work/fold_augmented_train_cal_oos_negative_surface_scores_"
            "current702_20260601.md"
        ),
    )
    train_cal_oos_surface.set_defaults(
        func=cmd_score_fold_augmented_train_cal_oos_negative_surface
    )

    oos_calibrated_threshold = subparsers.add_parser(
        "eval-fold-augmented-oos-calibrated-threshold-contract",
        help=(
            "select fold-augmented abstention thresholds using calibration "
            "in-scope rows plus train/cal OOS negative scores"
        ),
    )
    ...
    oos_calibrated_threshold.add_argument(
        "--report",
        default=(
            "work/fold_augmented_abstention_threshold_contract_oos_calibrated_"
            "current702_20260601.md"
        ),
    )
    ...
    fold_only_oos_surface = subparsers.add_parser(
        ...
        help="write a fold-only salvage surface for train/cal OOS negatives missing combined geometry+fold scores",
    )
    fold_only_oos_surface.add_argument(
        "--train-cal-oos-surface",
        default=(
            "artifacts/v3_fold_augmented_train_cal_oos_negative_surface_scores_"
            "current702_20260601.json"
    ...
    fold_only_oos_surface.add_argument(
        "--report",
        default="work/fold_only_train_cal_oos_negative_surface_current702_20260601.md",
    )
    fold_only_oos_surface.set_defaults(
        func=cmd_build_fold_only_train_cal_oos_negative_surface
    )

    train_cal_oos_blockers = subparsers.add_parser(
        "build-fold-augmented-train-cal-oos-negative-surface-blocker-resolution",
        help="write row-level blocker resolution for train/cal OOS negatives missing combined channel scores",
    )
    train_cal_oos_blockers.add_argument(
        "--train-cal-oos-surface",
        default=(
            "artifacts/v3_fold_augmented_train_cal_oos_negative_surface_scores_"
            "current702_20260601.json"
    ...
    train_cal_oos_blockers.add_argument(
        "--report",
        default=(
            "work/fold_augmented_train_cal_oos_negative_surface_blocker_resolution_"
            "current702_20260601.md"
        ),
    )
    train_cal_oos_blockers.set_defaults(
    ...
    active_site_role_sidecar.add_argument(
        "--label-manifest",
        default="artifacts/v3_sequence_nn_label_manifest_current702_20260525.json",
    )
    active_site_role_sidecar.add_argument("--graph", default="artifacts/v1_graph_1025.json")
    active_site_role_sidecar.add_argument(
        "--out",
        default=(
    ...
    active_site_role_sidecar.add_argument(
        "--report",
        default="work/mechanism_feature_active_site_role_graph_sidecar_current702_20260601.md",
    )
    active_site_role_sidecar.set_defaults(
        func=cmd_build_mechanism_feature_active_site_role_graph_sidecar
    )

    reaction_center_sidecar = subparsers.add_parser(
        "build-mechanism-feature-reaction-center-template-sidecar",
        help="write a row-level fingerprint-template reaction-center sidecar for mechanism-feature embedding readiness",
    )
    reaction_center_sidecar.add_argument(
        "--label-manifest",
        default="artifacts/v3_sequence_nn_label_manifest_current702_20260525.json",
    )
    reaction_center_sidecar.add_argument(
    ...
    reaction_center_sidecar.add_argument(
        "--report",
        default=(
            "work/mechanism_feature_reaction_center_template_sidecar_"
            "current702_20260601.md"
        ),
    )
    ...
    mechanism_embedding_train_cal_manifest = subparsers.add_parser(
        "build-mechanism-feature-embedding-train-cal-input-manifest",
        help=(
            "write a no-fit train/cal input manifest for the mechanism-feature "
            "embedding pilot"
        ),
    )
    mechanism_embedding_train_cal_manifest.add_argument(
        "--learned-embedding-plan",
        default="artifacts/v3_learned_mechanism_feature_embedding_plan_current702_20260601.json",
    )
    mechanism_embedding_train_cal_manifest.add_argument(
        "--label-manifest",
    ...
    mechanism_embedding_train_cal_manifest.add_argument(
        "--inorganic-cofactor-completion-audit",
        default=(
            "artifacts/v3_mechanism_feature_inorganic_cofactor_locus_"
            "completion_audit_current702_20260601.json"
        ),
    )
    mechanism_embedding_train_cal_manifest.add_argument(
    ...
    mechanism_embedding_train_cal_manifest.add_argument(
        "--report",
        default=(
            "work/mechanism_feature_embedding_train_cal_input_manifest_"
            "current702_20260601.md"
        ),
    )
    mechanism_embedding_train_cal_manifest.set_defaults(
        func=cmd_build_mechanism_feature_embedding_train_cal_input_manifest
    )

    mechanism_embedding_train_cal_split = subparsers.add_parser(
        "build-mechanism-feature-embedding-train-cal-split-manifest",
        help=(
            "write a deterministic no-fit train/cal split for ready "
            "mechanism-feature embedding rows"
        ),
    )
    mechanism_embedding_train_cal_split.add_argument(
    ...
    mechanism_embedding_train_cal_split.add_argument(
        "--report",
        default=(
            "work/mechanism_feature_embedding_train_cal_split_manifest_"
            "current702_20260601.md"
        ),
    )
    mechanism_embedding_train_cal_split.set_defaults(
        func=cmd_build_mechanism_feature_embedding_train_cal_split_manifest
    )

    mechanism_embedding_feature_contract = subparsers.add_parser(
        "build-mechanism-feature-embedding-feature-contract",
        help=(
            "write a no-fit label-stripped feature contract for ready "
            "mechanism-feature embedding train/cal rows"
        ),
    )
    mechanism_embedding_feature_contract.add_argument(
        "--train-cal-input-manifest",
        default=(
            "artifacts/v3_mechanism_feature_embedding_train_cal_input_"
            "manifest_current702_20260601.json"
    ...
    mechanism_embedding_feature_contract.add_argument(
        "--report",
        default=(
            "work/mechanism_feature_embedding_feature_contract_"
            "current702_20260601.md"
        ),
    )
