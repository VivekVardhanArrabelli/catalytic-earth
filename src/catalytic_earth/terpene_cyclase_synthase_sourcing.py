"""Broadened-handle sourcing for terpene cyclase/synthase enzymes."""
...
ARTIFACT_ID = "v3_terpene_cyclase_synthase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "terpene_cyclase_synthase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "terpene_ec_4_2_3_broad_corroborator",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:4.2.3.*) AND "
                "((keyword:Terpene) OR (protein_name:cyclase) OR "
                "(protein_name:synthase) OR (cc_cofactor:magnesium) OR "
                "(cc_cofactor:manganese))"
            ),
        },
        {
            "lane_id": "terpene_ec_4_2_3_metal_diphosphate",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:4.2.3.*) AND "
                "((keyword:Terpene) OR (protein_name:terpene) OR "
                "(protein_name:cyclase) OR (protein_name:synthase)) AND "
                "((cc_cofactor:magnesium) OR (cc_cofactor:manganese) OR "
                "(protein_name:diphosphate) OR (protein_name:pyrophosphate))"
            ),
        },
        {
            "lane_id": "terpene_ec_4_2_3_named",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:4.2.3.*) AND "
                '((protein_name:"terpene synthase") OR '
                '(protein_name:"terpene cyclase") OR '
                '(protein_name:"monoterpene") OR '
                '(protein_name:"sesquiterpene") OR '
                '(protein_name:"diterpene") OR '
                '(protein_name:"triterpene"))'
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a terpene cyclase/synthase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_terpene_cyclase_synthase_sourcing(
    *,
    families: tuple[str, ...] = FAMILIES,
    max_records_per_lane: int = 160,
    ...
    """Source terpene cyclase/synthase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "terpene_cyclase_synthase_broadened_handle_fresh_fetch",
                "path": "terpene_cyclase_synthase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_terpene_cyclase_annotation",
        "stage": "scaling_plan_to_10k:wire_terpene_cyclase_synthase_broadened_handle",
        ...
        "guardrails": {
            ...
            "terpene_handles_scope_admission_only": True,
            ...
            "prenyltransferase_hydratase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Terpene handles scope-admission only, never predictive: "
            ...
            "- Boundary guards: "
            f"{audit['guardrails']['prenyltransferase_hydratase_side_ec_boundary_guard']}.",
...
def write_terpene_cyclase_synthase_sourcing(
    *,
    out_path: Path,
