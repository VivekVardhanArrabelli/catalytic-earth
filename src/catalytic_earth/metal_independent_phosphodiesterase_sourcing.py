"""Broadened-handle sourcing for metal-independent phosphodiesterases."""
...
ARTIFACT_ID = "v3_metal_independent_phosphodiesterase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "metal_independent_phosphodiesterase"
FAMILIES: tuple[str, ...] = (FAMILY,)

NON_METAL_FILTER = (
    'NOT ((cc_cofactor:magnesium) OR (cc_cofactor:manganese) OR '
...
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "metal_independent_pde_ec_3_1_4_act_binding_non_metal",
        ...
        {
            "lane_id": "metal_independent_pde_hydrolase_non_metal",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (protein_name:phosphodiesterase) "
                "AND (keyword:Hydrolase) "
                f"{NON_METAL_FILTER}"
            ),
        },
        {
            ...
            "query": (
                "(reviewed:true) AND ("
                '(protein_name:"cyclic nucleotide phosphodiesterase") OR '
                '(protein_name:"cyclic-nucleotide phosphodiesterase") OR '
                "(ec:3.1.4.17)) "
                f"{NON_METAL_FILTER}"
            ),
        ...
        {
            "lane_id": "metal_independent_pde_cyclic_amp_gmp_reaction",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.4.*) AND ("
                '(cc_catalytic_activity:"cyclic AMP") OR '
        ...
        {
            "lane_id": "metal_independent_pde_phosphodiester_reaction_non_metal",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (protein_name:phosphodiesterase) AND ("
...
UNREVIEWED_TIER2_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "metal_independent_pde_unreviewed_tier2_site_reaction_non_metal",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:false) AND (ec:3.1.4.*) AND "
                "(protein_name:phosphodiesterase) AND "
                "((ft_act_site:*) OR (ft_binding:*)) AND "
                "(cc_catalytic_activity:*) "
                f"{NON_METAL_FILTER}"
            ),
        },
        {
            "lane_id": "metal_independent_pde_unreviewed_tier2_cyclic_reaction_non_metal",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:false) AND (ec:3.1.4.*) AND "
                "((cc_catalytic_activity:\"cyclic AMP\") OR "
    ...
    *,
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a metal-independent PDE sourcing family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(
                "unknown metal-independent phosphodiesterase lane ids: "
                + ", ".join(missing)
            )
    return tuple(lanes)


def build_metal_independent_phosphodiesterase_sourcing(
    *,
    ...
    """Source metal-independent phosphodiesterase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "metal_independent_phosphodiesterase_broadened_handle_fresh_fetch",
                "path": "metal_independent_phosphodiesterase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": (
            "unreviewed_uniprot_tier2_ec_rhea_metal_independent_phosphodiester_hydrolysis_annotation"
            if source_tier == "source_tier_2"
            else "reviewed_swissprot_ec_rhea_metal_independent_phosphodiester_hydrolysis_annotation"
        ),
        "stage": "scaling_plan_to_10k:wire_metal_independent_phosphodiesterase_broadened_handle",
        ...
        "guardrails": {
            ...
            "metal_independent_pde_handles_scope_admission_only": True,
            ...
            "metal_absence_is_boundary_filter_not_counted_evidence": True,
            ...
            "metal_phosphoesterase_phosphatase_cyclase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Sources fresh reviewed Swiss-Prot bronze for EC 3.1.4/4.6.1 "
        "metal-independent phosphodiesterases through phosphodiesterase family/name "
        "context, hydrolytic phosphodiester/cyclic-nucleotide reaction evidence, "
        "and active-/binding-site context where available.",
        "EC / keyword / reaction text are scope-admission only, never predictive; "
        "metal-dependent phosphoesterase/nuclease, phosphatase, cyclase/lyase, "
        "kinase, transferase, side-EC, EC-only, and off-target rows are guarded.",
        "",
        "## Result",
        "",
    ...
    lines.extend(
        [
            ...
            f"- Metal absence counted as evidence: {not audit['guardrails']['metal_absence_is_boundary_filter_not_counted_evidence']}.",
            f"- Predictive features from broadened handles: {audit['guardrails']['predictive_features_use_ec_name_keyword_reaction_or_prose']}.",
            f"- Boundary guards: {audit['guardrails']['metal_phosphoesterase_phosphatase_cyclase_boundary_guard']}.",
...
def write_metal_independent_phosphodiesterase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_metal_independent_phosphodiesterase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
