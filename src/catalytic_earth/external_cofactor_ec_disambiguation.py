...
# supply only (NOT shared with any existing fingerprint); the required hard mechanism anchor is a
# Rhea/reviewed reaction that activates a carboxylate with ATP to a CoA thioester (acid + ATP + CoA
# -> acyl-CoA + AMP + diphosphate) -- both a CoA token AND an AMP/adenylate token, so a CoA
# transferase reaction (no AMP release) does not qualify. The second corroborator is the CoA-ligase /
# acyl-CoA-synthetase family name OR an annotated active/binding site (the protein name is NOT
# necessary). CoA transferases (EC 2.3 / 2.8.3), biotin carboxylases (EC 6.3.4 / 6.4.1), thiolases,
# and acyl-CoA dehydrogenases are boundary-guarded. NOTE: the ATP-adenylation step is shared with the
# EC 6.3 atp_amide_ligase and the CoA thioester with coa_acyltransferase, so this family is
# representation-confusable with both in the source-free feature space (capped at 150); EC scope
# (6.2.1 vs 6.3 vs 2.3.1) cleanly separates them at admission.
...
DISAMBIGUATION_RULES: tuple[tuple[str, Callable[[dict[str, bool], dict[str, Any]], bool]], ...] = (
    ...
    (
        # EC 1.11.1 is SCOPE only (shared with heme_peroxidase_oxidase); the peroxide-reduction
        # reaction is the required hard mechanism anchor, and the thiol/Sec cofactor context OR the
        # Prx/GPx family name is the second corroborator. The discriminator vs the heme sibling is
        # the cofactor (NOT heme / NOT flavin), never EC.
    ...
    (
        # EC 2.8.2 is SCOPE only; the PAPS sulfuryl-transfer reaction is the required hard
        # mechanism anchor and the sulfotransferase name OR an active/binding-site residue is the
        # second corroborator -- the protein name is no longer a necessary condition.
        "paps_sulfotransferase",
        lambda c, row: _ec_has_prefix(row, _PAPS_SULFOTRANSFERASE_EC)
        and c["paps_sulfotransferase_reaction"]
        and (
            c["paps_sulfotransferase_family_text"]
    ...
    (
        # EC 2.5.1.18 is SCOPE only; the glutathione-conjugation reaction is the required hard
        # mechanism anchor and the GST name OR an active/binding-site residue is the second
        # corroborator -- the protein name is no longer a necessary condition.
        ...
        and (
            c["glutathione_s_transferase_family_text"]
            or c["active_or_binding_site_present"]
        )
    ...
    (
        # EC 6.1.1 is SCOPE only; the ATP-dependent tRNA-aminoacylation reaction (aa + ATP + tRNA
        # = aa-tRNA + AMP + PPi) is the required hard mechanism anchor and the --tRNA-ligase name OR
        # an active/binding-site residue is the second corroborator -- the name is not necessary.
        ...
        and (
            c["aminoacyl_trna_synthetase_family_text"]
            or c["active_or_binding_site_present"]
        )
    ...
    (
        # EC 6.2.1 is SCOPE only (not shared with any existing fingerprint); the ATP-dependent
        # acid--CoA thioester-forming reaction (a CoA token AND an AMP/adenylate release token) is
        # the required hard mechanism anchor and the CoA-ligase / acyl-CoA-synthetase family name OR
        # an active/binding-site residue is the second corroborator -- the protein name is not a
        # necessary condition. EC 6.2.1 (vs 6.3 atp_amide_ligase and 2.3.1 coa_acyltransferase)
        # separates the representation-confusable siblings at admission; it never decides a label.
        "acid_coa_ligase",
        lambda c, row: _ec_has_prefix(row, _ACID_COA_LIGASE_EC)
        and c["acid_coa_ligase_reaction"]
        and (
            c["acid_coa_ligase_family_text"]
            or c["active_or_binding_site_present"]
        )
