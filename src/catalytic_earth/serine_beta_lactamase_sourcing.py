"""Guarded sourcing for serine beta-lactamase bronze labels."""
...
ARTIFACT_ID = "v3_serine_beta_lactamase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "serine_beta_lactamase"
FAMILIES: tuple[str, ...] = (FAMILY,)

SERINE_BETA_LACTAMASE_EXACT_EC_QUERY = (
    "(reviewed:true) AND (ec:3.5.2.6) AND "
    "((protein_name:\"beta-lactamase\") OR (keyword:\"Beta-lactamase\")) "
    "NOT ((protein_name:metallo) OR (keyword:\"Metal-binding\") OR "
    "(cc_cofactor:zinc) OR (ec:3.5.2.7))"
)
SERINE_BETA_LACTAMASE_EXACT_EC_ACTIVE_SITE_QUERY = (
    f"{SERINE_BETA_LACTAMASE_EXACT_EC_QUERY} AND ((ft_act_site:*) OR (ft_binding:*))"
)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "serine_beta_lactamase_reviewed_exact_ec_active_site",
            "target_family_lane": FAMILY,
            "query": SERINE_BETA_LACTAMASE_EXACT_EC_ACTIVE_SITE_QUERY,
        },
    )
}
UNREVIEWED_TIER2_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "serine_beta_lactamase_unreviewed_tier2_active_site_reaction",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:false) AND (ec:3.5.2.6) AND "
                "((protein_name:\"beta-lactamase\") OR (keyword:\"Beta-lactamase\")) "
                "AND ((ft_act_site:*) OR (ft_binding:*)) AND "
                "(cc_catalytic_activity:*) NOT ((protein_name:metallo) OR "
                "(keyword:\"Metal-binding\") OR (cc_cofactor:zinc) OR (ec:3.5.2.7))"
            ),
        },
    )
}
    ...
    *,
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a serine beta-lactamase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown serine beta-lactamase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_serine_beta_lactamase_sourcing(
    *,
    ...
    """Source serine beta-lactamase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "serine_beta_lactamase_fresh_fetch",
                "path": "serine_beta_lactamase_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": (
            "unreviewed_uniprot_tier2_beta_lactamase_active_site_reaction_annotation"
            if source_tier == "source_tier_2"
            else "reviewed_swissprot_exact_ec_beta_lactamase_active_site_reaction_annotation"
        ),
        "stage": "scaling_plan_to_10k:wire_serine_beta_lactamase_guarded_lane",
        ...
        "guardrails": {
            ...
            "serine_beta_lactamase_handles_scope_admission_only": True,
            ...
            "metallo_pbp_dd_peptidase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['metallo_pbp_dd_peptidase_boundary_guard']}.",
...
def write_serine_beta_lactamase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_serine_beta_lactamase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
