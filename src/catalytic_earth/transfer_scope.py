...
def build_external_source_pilot_sdr_redox_import_safety_adjudication(
    *,
    sdr_redox_repair_control: dict[str, Any],
    resolved_pilot_decisions: dict[str, Any],
    pilot_active_site_evidence_decisions: dict[str, Any],
    external_import_readiness_audit: dict[str, Any],
    ...
    """Adjudicate the O14756 SDR/NAD(P) repair control in import-safety terms."""
    ...
    for control_row in [
        row
        for row in sdr_redox_repair_control.get("rows", []) or []
        if isinstance(row, dict)
    ][:max_rows]:
        ...
        reference_contrasts = [
            reference
            for reference in control_row.get("current_reference_contrasts", []) or []
            if isinstance(reference, dict)
        ]
        candidate_has_sdr_axis = (
            candidate_features.get("sdr_sequence_axis_status")
            == "sdr_axis_present_with_source_active_site_overlap"
        )
        current_reference_sdr_axis_match_count = sum(
            1
            for reference in reference_contrasts
            if reference.get("sdr_sequence_axis_status") == "sdr_axis_present"
        )
        ...
        repair_control_ready = (
            ...
            == "review_only_sdr_axis_contrast_ready"
            and candidate_has_sdr_axis
            and current_reference_sdr_axis_match_count == 0
        )
        ...
        if representation_conflict_repaired:
            blockers = {
                blocker
                for blocker in blockers
                if blocker not in representation_blockers
        ...
        adjudicated_status = (
            ...
            if representation_conflict_repaired
            else "sdr_axis_representation_conflict_not_repaired"
        )
        ...
        rows.append(
            {
                ...
                "decision_effect": (
                    "representation_conflict_repaired_but_import_blocked_by_"
                    "duplicate_review_and_factory_gates"
                ...
                "sdr_axis_evidence": {
                    "candidate_sequence_axis_status": candidate_features.get(
                        "sdr_sequence_axis_status"
                    ),
                    "glycine_rich_nad_p_binding_motif_count": len(
                        candidate_features.get(
                            "glycine_rich_nad_p_binding_motif_hits", []
                        )
                        or []
                    ),
                    "source_active_site_yxxxk_motif_count": sum(
                        1
                        for hit in candidate_features.get(
                            "catalytic_yxxxk_motif_hits", []
                        )
                        or []
                        if isinstance(hit, dict)
                        and hit.get("overlaps_source_active_site") is True
                    ),
                    "current_reference_sdr_axis_match_count": (
                        current_reference_sdr_axis_match_count
                    ),
                    "current_reference_axis_statuses": [
                        {
                            "reference_accession": reference.get(
                                "reference_accession"
                            ),
                            "sdr_sequence_axis_status": reference.get(
                                "sdr_sequence_axis_status"
                            ),
                        }
                        for reference in reference_contrasts
                ...
                "review_status": (
                    "external_pilot_sdr_redox_import_safety_adjudication_review_only"
                ),
            }
        )
    ...
    blockers: list[str] = []
    if not rows:
        blockers.append("target_sdr_repair_control_row_missing")
    return {
        "metadata": {
            "method": "external_source_pilot_sdr_redox_import_safety_adjudication",
            "slice_id": _external_artifact_lineage_slice_id(artifact_lineage),
            "blocker_removed": (
            ...
            "candidate_count": len(rows),
            "max_rows": max_rows,
            "representation_conflict_repaired_count": status_counts.get(
                "sdr_axis_representation_conflict_repaired", 0
            ),
            ...
            "predictive_feature_sources": [
                "amino_acid_sequence_motif_presence",
                "source_traced_active_site_position_overlap",
                "current_reference_sequence_axis_absence",
            ...
            "source_sdr_redox_repair_control_method": (
                sdr_redox_repair_control.get("metadata", {}).get("method")
            ),
        ...
        "warnings": [
            (
                "SDR/NAD(P) import-safety adjudication is a review-only decision "
                "path; it repairs one representation conflict but cannot bypass "
                "duplicate review, expert/review decisions, or full factory gates"
            )
        ],
    }


def build_external_source_pilot_akr_nadp_repair_control(
    *,
    repair_lanes: dict[str, Any],
    needs_review_resolution: dict[str, Any],
    pilot_representation_sample: dict[str, Any],
    ...
    """Stage the C9JRZ8 AKR/NADP redox lane as review-only control evidence."""

    if max_rows < 1:
        raise ValueError("max_rows must be positive")

    target_lane = "add_akr_nadp_redox_representation_axis"
    resolution_by_accession = {
        _normalize_accession(row.get("accession")): row
        for row in needs_review_resolution.get("rows", []) or []
        if isinstance(row, dict) and _normalize_accession(row.get("accession"))
    ...
    for lane_row in repair_lanes.get("rows", []) or []:
        ...
        candidate_record = external_sequences.get(accession, {})
        candidate_sequence = _clean_sequence(candidate_record.get("sequence"))
        candidate_features = _external_akr_nadp_sequence_features(
            candidate_sequence,
            active_site_positions=active_site_positions,
        )
        baseline_row = baseline_representation_by_accession.get(accession, {})
        ...
        reference_contrasts = [
            _external_akr_nadp_reference_contrast(
                reference_accession=reference_accession,
                reference_sequences=reference_sequences,
                baseline_row=baseline_row,
                larger_row=larger_row,
        ...
        full_reference_match_count = sum(
            1
            for reference in reference_contrasts
            if reference.get("akr_nadp_sequence_axis_status")
            == "akr_nadp_axis_present"
        )
        candidate_has_axis = (
            candidate_features.get("akr_nadp_sequence_axis_status")
            == "akr_nadp_axis_present_with_source_active_site_tyr"
        )
        control_status = (
            "review_only_akr_nadp_axis_contrast_ready"
            if candidate_has_axis and full_reference_match_count == 0
        ...
        rows.append(
            {
                ...
                "entry_id": lane_row.get("entry_id") or f"uniprot:{accession}",
                "repair_lane": target_lane,
                "implemented_control": "akr_nadp_redox_sequence_axis_contrast",
                "control_status": control_status,
                "source_resolved_status": resolution_row.get("revised_status"),
                "source_confidence": resolution_row.get("confidence"),
                "candidate_sequence_features": candidate_features,
                ...
                "control_interpretation": _external_akr_nadp_control_interpretation(
                    candidate_has_axis=candidate_has_axis,
                    full_reference_match_count=full_reference_match_count,
                ),
                "decision_effect": (
                    "keeps row rejected_representation_conflict; this control only "
                    "stages non-text AKR/NADP repair evidence for a future scoring "
                    "experiment"
                ),
                "countable_label_candidate": False,
                "ready_for_label_import": False,
                "review_status": "external_pilot_akr_nadp_repair_control_review_only",
            }
        )
        if len(rows) >= max_rows:
            break
    ...
    blockers: list[str] = []
    if not rows:
        blockers.append("target_akr_nadp_repair_lane_row_missing")
    if any(row["control_status"].endswith("_incomplete") for row in rows):
        blockers.append("akr_nadp_sequence_axis_contrast_incomplete")
    return {
        "metadata": {
            "method": "external_source_pilot_akr_nadp_repair_control",
            "slice_id": _external_artifact_lineage_slice_id(artifact_lineage),
            "blocker_removed": (
                "akr_nadp_redox_lane_has_sequence_derived_contrast_control"
            ),
            "blocker_not_removed": [
                "control_not_integrated_into_geometry_or_representation_scorer",
            ...
            "implemented_control": "akr_nadp_redox_sequence_axis_contrast",
            "control_status_counts": dict(sorted(control_status_counts.items())),
            "candidate_with_akr_nadp_axis_count": sum(
                1
                for row in rows
                if row["candidate_sequence_features"].get(
                    "akr_nadp_sequence_axis_status"
                )
                == "akr_nadp_axis_present_with_source_active_site_tyr"
            ),
            "current_reference_akr_nadp_axis_match_count": (
                reference_axis_match_count
            ),
            "predictive_feature_sources": [
                "amino_acid_sequence_motif_presence",
                "source_traced_active_site_residue_code",
                "source_traced_active_site_position_overlap",
            ],
            "review_context_fields": [
                "Rhea reaction identifiers and equations",
        ...
        "warnings": [
            (
                "AKR/NADP repair controls are review-only contrast evidence; "
                "they do not validate enzyme function or authorize import"
            )
        ],
    }
...
def build_external_source_pilot_akr_nadp_import_safety_adjudication(
    *,
    akr_nadp_repair_control: dict[str, Any],
    resolved_pilot_decisions: dict[str, Any],
    pilot_active_site_evidence_decisions: dict[str, Any],
    external_import_readiness_audit: dict[str, Any],
    pilot_success_criteria: dict[str, Any] | None = None,
    ...
    """Adjudicate the C9JRZ8 AKR/NADP repair control for import safety."""
    ...
    for control_row in [
        row
        for row in akr_nadp_repair_control.get("rows", []) or []
        if isinstance(row, dict)
    ][:max_rows]:
        accession = _normalize_accession(control_row.get("accession"))
        if not accession:
        ...
        current_reference_contrasts = [
            reference
            for reference in control_row.get("current_reference_contrasts", [])
            or []
            if isinstance(reference, dict)
        ]
        candidate_has_axis = (
        ...
        candidate_has_source_tyr = (
            _external_parse_int(
                candidate_features.get("source_active_site_tyr_count")
            )
            >= 1
        )
        candidate_has_cofactor_proxy = bool(
            candidate_features.get(
        ...
        reference_axis_match_count = sum(
            1
            for reference in current_reference_contrasts
            if reference.get("akr_nadp_sequence_axis_status")
            == "akr_nadp_axis_present"
        )
        active_site_resolved = (
        ...
        repair_control_ready = (
            control_row.get("control_status")
            == "review_only_akr_nadp_axis_contrast_ready"
            and candidate_has_axis
            and candidate_has_source_tyr
            and candidate_has_cofactor_proxy
            and reference_axis_match_count == 0
        )
        representation_conflict_repaired = (
            repair_control_ready and active_site_resolved and bounded_sequence_no_signal
        )
        ...
        adjudicated_status = (
            ...
            if representation_conflict_repaired
            else "akr_nadp_axis_representation_conflict_not_repaired"
        )
        normalized_status_after_repair = (
            "import_ready_candidate"
            if ready_for_import
        ...
        rows.append(
            {
                ...
                "decision_effect": (
                    "representation_conflict_repaired_but_import_blocked_by_"
                    "duplicate_review_unscored_heuristic_and_factory_gates"
                    if representation_conflict_repaired and not ready_for_import
                    else "import_ready_candidate"
                    if ready_for_import
                    else "keeps_previous_representation_conflict_rejection"
                ),
                "akr_nadp_axis_evidence": {
                    "source_active_site_tyr_positions": (
                        candidate_features.get("source_active_site_tyr_positions", [])
                    ),
                    "active_site_tyr_local_h_or_k_count": (
                        candidate_features.get("active_site_tyr_local_h_or_k_count")
                    ),
                    "n_terminal_glycine_pair_cofactor_proxy_motif_hits": (
                        candidate_features.get(
                            "n_terminal_glycine_pair_cofactor_proxy_motif_hits",
                ...
                "ready_for_label_import": ready_for_import,
                "review_status": (
                    "external_pilot_akr_nadp_import_safety_adjudication_review_only"
                ),
            }
        )
    ...
    blockers: list[str] = []
    if not rows:
        blockers.append("target_akr_nadp_control_row_missing")
    repaired_count = status_counts.get(
        "akr_nadp_axis_representation_conflict_repaired", 0
    )
    return {
        "metadata": {
            "method": (
            ...
            "source_akr_nadp_repair_control_method": (
                akr_nadp_repair_control.get("metadata", {}).get("method")
            ),
            "source_resolved_pilot_decisions_method": (
                resolved_pilot_decisions.get("metadata", {}).get("method")
            ),
        ...
        "warnings": [
            (
                "AKR/NADP import-safety adjudication is a review-only decision "
                "path; it repairs one representation conflict but cannot bypass "
                "duplicate review, heuristic scoring, review decisions, or full "
                "factory gates"
            )
        ],
    }
...
def build_external_source_pilot_glycoside_hydrolase_boundary_control(
    *,
    repair_lanes: dict[str, Any],
    needs_review_resolution: dict[str, Any],
    pilot_representation_sample: dict[str, Any],
    ...
    """Stage a Q6NSJ0 glycoside-hydrolase boundary control."""

    if max_rows < 1:
        raise ValueError("max_rows must be positive")

    target_lane = "split_glycoside_hydrolase_from_metal_hydrolase_control"
    resolution_by_accession = {
        _normalize_accession(row.get("accession")): row
        for row in needs_review_resolution.get("rows", []) or []
        if isinstance(row, dict) and _normalize_accession(row.get("accession"))
    ...
    for lane_row in repair_lanes.get("rows", []) or []:
        ...
        active_site_features = _external_glycoside_hydrolase_active_site_features(
            sequence,
            active_site_positions=active_site_positions,
        )
        heuristic_row = heuristic_by_accession.get(accession, {})
        ...
        metal_ligand_context_absent = not any(
        ...
        rows.append(
            {
                ...
                "repair_lane": target_lane,
                "implemented_control": (
                    "glycoside_hydrolase_active_site_vs_metal_hydrolase_boundary"
                ),
                "control_status": (
                    "review_only_glycoside_hydrolase_boundary_ready"
                    if glycan_boundary_ready
                    else "review_only_glycoside_hydrolase_boundary_incomplete"
                ),
                "source_resolved_status": resolution_row.get("revised_status"),
                "source_confidence": resolution_row.get("confidence"),
                ...
                "pocket_boundary_features": {
                    "aromatic_fraction": pocket_descriptors.get(
                        "aromatic_fraction"
                    ),
                    "negative_fraction": pocket_descriptors.get(
                        "negative_fraction"
                    ),
                    "nearby_aromatic_residue_count": sum(
                        1
                        for site in nearby_sites
                        if site.get("code") in {"PHE", "TRP", "TYR"}
                    ),
                    "nearby_acidic_residue_count": sum(
                        1
                        for site in nearby_sites
                        if site.get("code") in {"ASP", "GLU"}
                    ),
                ...
                "decision_effect": (
                    ...
                    "stages non-text boundary evidence for later import-safety "
                    "adjudication"
                ),
                "countable_label_candidate": False,
                "ready_for_label_import": False,
                "review_status": (
                    "external_pilot_glycoside_hydrolase_boundary_control_review_only"
                ),
            }
        )
        if len(rows) >= max_rows:
    ...
    blockers: list[str] = []
    if not rows:
        blockers.append("target_glycoside_hydrolase_repair_lane_row_missing")
    if any(row["control_status"].endswith("_incomplete") for row in rows):
        blockers.append("glycoside_hydrolase_boundary_control_incomplete")
    return {
        "metadata": {
            "method": "external_source_pilot_glycoside_hydrolase_boundary_control",
            "slice_id": _external_artifact_lineage_slice_id(artifact_lineage),
            "blocker_removed": (
            ...
            "control_status_counts": dict(sorted(control_status_counts.items())),
            "candidate_with_acidic_dyad_count": sum(
                1
                for row in rows
                if row["candidate_active_site_features"][
                    "acidic_source_active_site_residue_count"
                ]
                >= 2
            ),
            "metal_ligand_context_absent_count": sum(
                1
                for row in rows
                if row["metal_hydrolase_contrast_features"][
        ...
        "warnings": [
            (
                "glycoside-hydrolase boundary controls are review-only contrast "
...
def build_external_source_pilot_glycoside_hydrolase_import_safety_adjudication(
    *,
    glycoside_hydrolase_boundary_control: dict[str, Any],
    resolved_pilot_decisions: dict[str, Any],
    pilot_active_site_evidence_decisions: dict[str, Any],
    external_import_readiness_audit: dict[str, Any],
    pilot_success_criteria: dict[str, Any] | None = None,
    ...
    """Adjudicate the Q6NSJ0 glycoside-hydrolase boundary control for import safety."""
    ...
    for control_row in [
        row
        for row in glycoside_hydrolase_boundary_control.get("rows", []) or []
        if isinstance(row, dict)
    ][:max_rows]:
        accession = _normalize_accession(control_row.get("accession"))
        if not accession:
        ...
        metal_contrast = control_row.get("metal_hydrolase_contrast_features", {})
        if not isinstance(metal_contrast, dict):
            metal_contrast = {}
        ...
        acidic_pair_present = (
            active_site_features.get("has_acidic_active_site_pair") is True
            and _external_parse_int(
                active_site_features.get("acidic_source_active_site_residue_count")
            )
            >= 2
        )
        metal_ligand_context_absent = (
            metal_contrast.get("metal_ligand_context_absent") is True
        )
        metal_role_hint_match_count = _external_parse_int(
            metal_contrast.get("metal_role_hint_match_count")
        )
        metal_role_hints_absent = metal_role_hint_match_count == 0
        active_site_resolved = (
            active_row.get("active_site_evidence_source_category")
            == "explicit_active_site_source_present"
        )
        ...
        repair_control_ready = (
            control_row.get("control_status")
            == "review_only_glycoside_hydrolase_boundary_ready"
            and acidic_pair_present
            and metal_ligand_context_absent
            and metal_role_hints_absent
        )
        representation_conflict_repaired = (
            repair_control_ready and active_site_resolved and bounded_sequence_no_signal
        )
        ...
        adjudicated_status = (
            ...
            if representation_conflict_repaired
            else "glycoside_boundary_representation_conflict_not_repaired"
        )
        normalized_status_after_repair = (
            "import_ready_candidate"
            if ready_for_import
        ...
        rows.append(
            {
                ...
                "glycoside_boundary_evidence": {
                    "has_acidic_active_site_pair": active_site_features.get(
                        "has_acidic_active_site_pair"
                    ),
                    "acidic_source_active_site_positions": active_site_features.get(
                        "acidic_source_active_site_positions", []
                    ),
                    "acidic_active_site_spacing_values": active_site_features.get(
                        "acidic_active_site_spacing_values", []
                    ),
                    "metal_ligand_context_absent": metal_ligand_context_absent,
                    "metal_role_hint_match_count": metal_role_hint_match_count,
                    "top1_role_match_fraction": metal_contrast.get(
                        "top1_role_match_fraction"
                    ),
                    "nearby_acidic_residue_count": pocket_features.get(
                        "nearby_acidic_residue_count"
                    ),
                ...
                "ready_for_label_import": ready_for_import,
                "review_status": (
                    "external_pilot_glycoside_hydrolase_import_safety_"
    ...
    blockers: list[str] = []
    if not rows:
        blockers.append("target_glycoside_hydrolase_boundary_control_row_missing")
    return {
        "metadata": {
            "method": (
                "external_source_pilot_glycoside_hydrolase_import_safety_"
                "adjudication"
            ),
            "slice_id": _external_artifact_lineage_slice_id(artifact_lineage),
            "blocker_removed": (
            ...
            "max_rows": max_rows,
            "representation_conflict_repaired_count": status_counts.get(
                "glycoside_boundary_representation_conflict_repaired", 0
            ),
            "import_safety_adjudication_status_counts": dict(
                sorted(status_counts.items())
            ),
            ...
            "source_glycoside_hydrolase_boundary_control_method": (
                glycoside_hydrolase_boundary_control.get("metadata", {}).get("method")
            ),
            "source_resolved_pilot_decisions_method": (
                resolved_pilot_decisions.get("metadata", {}).get("method")
            ),
        ...
        "warnings": [
            (
                "Glycoside-hydrolase import-safety adjudication is review-only; "
                "it repairs one boundary-control conflict but cannot bypass "
                "duplicate review, expert/review decisions, or full factory gates"
            )
        ],
    }
...
def build_external_source_pilot_sugar_phosphate_isomerase_control(
    *,
    ...
    """Stage a P34949 sugar-phosphate isomerase contrast control."""

    if max_rows < 1:
        raise ValueError("max_rows must be positive")

    target_lane = "add_sugar_phosphate_isomerase_scope_control"
    resolution_by_accession = {
        _normalize_accession(row.get("accession")): row
        for row in needs_review_resolution.get("rows", []) or []
        if isinstance(row, dict) and _normalize_accession(row.get("accession"))
    ...
    for lane_row in repair_lanes.get("rows", []) or []:
        ...
        active_site_features = (
            _external_sugar_phosphate_isomerase_active_site_features(
                sequence,
                active_site_positions=active_site_positions,
            )
        )
        ...
        flavin_ligand_context_absent = not any(
            ligand_context.get(key)
            for key in (
                "cofactor_families",
                "ligand_codes",
        ...
        flavin_role_hint_match_count = sum(
            1
            for match in top1.get("matched_signature_roles", []) or []
            if isinstance(match, dict) and match.get("role_hint_match") is True
        )
        ...
        isomerase_scope_ready = (
            active_site_features["source_active_site_arg_count"] >= 1
            and top1.get("fingerprint_id") == "flavin_dehydrogenase_reductase"
            and top1_score <= 0.25
            and flavin_ligand_context_absent
            and flavin_role_hint_match_count == 0
        ...
        rows.append(
            {
                ...
                "repair_lane": target_lane,
                "implemented_control": (
                    "sugar_phosphate_isomerase_vs_weak_flavin_redox_scope_control"
                ),
                "control_status": (
                    "review_only_sugar_phosphate_isomerase_scope_ready"
                    if isomerase_scope_ready
                    else "review_only_sugar_phosphate_isomerase_scope_incomplete"
                ),
                "source_resolved_status": resolution_row.get("revised_status"),
                "source_confidence": resolution_row.get("confidence"),
                "candidate_active_site_features": active_site_features,
                ...
                "pocket_boundary_features": {
                    "polar_fraction": pocket_descriptors.get("polar_fraction"),
                    ...
                    "nearby_basic_or_polar_residue_count": sum(
                        1
                        for site in nearby_sites
                        if site.get("code")
                        in {
                ...
                "control_interpretation": (
                    "Candidate has a source-traced basic active-site residue and "
                    "a local pocket compatible with sugar-phosphate chemistry, "
                    "while the weak flavin top1 match has absent flavin context "
                    "and zero role-hint support. This is review-only scope "
                    "control evidence, not import."
                    if isomerase_scope_ready
                    else "The sugar-phosphate isomerase scope evidence is incomplete."
                ),
                "decision_effect": (
                    "keeps row rejected_representation_conflict; this control "
                    "stages non-text isomerase-vs-flavin evidence for later "
                    "import-safety adjudication"
                ),
                "countable_label_candidate": False,
                "ready_for_label_import": False,
                "review_status": (
                    "external_pilot_sugar_phosphate_isomerase_control_review_only"
                ),
            }
        )
        if len(rows) >= max_rows:
    ...
    blockers: list[str] = []
    if not rows:
        blockers.append("target_sugar_phosphate_isomerase_repair_lane_row_missing")
    if any(row["control_status"].endswith("_incomplete") for row in rows):
        blockers.append("sugar_phosphate_isomerase_scope_control_incomplete")
    return {
        "metadata": {
            "method": "external_source_pilot_sugar_phosphate_isomerase_control",
            "slice_id": _external_artifact_lineage_slice_id(artifact_lineage),
            "blocker_removed": (
                "sugar_phosphate_isomerase_has_non_text_scope_control"
            ),
            "blocker_not_removed": [
                "control_not_integrated_into_import_safety_adjudication",
                "full_label_factory_gate_not_run_for_external_import",
            ...
            "control_status_counts": dict(sorted(control_status_counts.items())),
            "candidate_with_basic_active_site_count": sum(
                1
                for row in rows
                if row["candidate_active_site_features"][
            ...
            "flavin_ligand_context_absent_count": sum(
                1
                for row in rows
                if row["flavin_redox_contrast_features"][
        ...
        "warnings": [
            (
                "Sugar-phosphate isomerase controls are review-only contrast "
                "evidence; they do not validate enzyme function or authorize import"
            )
        ],
    }
...
def _external_glycoside_hydrolase_active_site_features(
    ...
    *,
    ...
    for position in sorted(set(active_site_positions)):
        ...
        if residue in {"D", "E"}:
            acidic_positions.append(position)
    ...
    spacing_values = [
        later - earlier
        for index, earlier in enumerate(acidic_positions)
        for later in acidic_positions[index + 1 :]
    ]
    return {
        "sequence_length": len(sequence),
        "source_active_site_positions": sorted(set(active_site_positions)),
        "active_site_residue_windows": residue_windows,
        "acidic_source_active_site_positions": acidic_positions,
        "acidic_source_active_site_residue_count": len(acidic_positions),
        "acidic_active_site_spacing_values": spacing_values,
        "has_acidic_active_site_pair": len(acidic_positions) >= 2,
    }


def _external_sugar_phosphate_isomerase_active_site_features(
    sequence: str,
    *,
    active_site_positions: list[int],
) -> dict[str, Any]:
    ...
    basic_or_polar_positions: list[int] = []
    residue_windows: list[dict[str, Any]] = []
    basic_or_polar_residues = {"R", "K", "H", "N", "Q", "S", "T", "Y", "D", "E"}
    for position in sorted(set(active_site_positions)):
        if position < 1 or position > len(sequence):
            continue
        residue = sequence[position - 1]
        if residue == "R":
            arg_positions.append(position)
        if residue in basic_or_polar_residues:
            basic_or_polar_positions.append(position)
        window_start = max(1, position - 12)
        window_end = min(len(sequence), position + 12)
        residue_windows.append(
            {
    ...
    return {
        ...
        "source_active_site_positions": sorted(set(active_site_positions)),
        "active_site_residue_windows": residue_windows,
        "source_active_site_arg_positions": arg_positions,
...
def _external_akr_nadp_sequence_features(
    sequence: str,
    *,
    active_site_positions: list[int],
) -> dict[str, Any]:
    sequence = _clean_sequence(sequence)
    active_site_position_set = set(active_site_positions)
    active_site_tyr_positions: list[int] = []
    active_site_tyr_windows: list[dict[str, Any]] = []
    local_h_or_k_count = 0
    for position in sorted(active_site_position_set):
        if position < 1 or position > len(sequence):
            continue
        residue = sequence[position - 1]
        window_start = max(1, position - 8)
        window_end = min(len(sequence), position + 8)
        window = sequence[window_start - 1 : window_end]
        if residue == "Y":
            active_site_tyr_positions.append(position)
            local_h_or_k_count += sum(1 for residue_code in window if residue_code in {"H", "K"})
        active_site_tyr_windows.append(
            {
                "position": position,
                "residue": residue,
                "window_start": window_start,
                ...
                "local_h_or_k_count": sum(
                    1 for residue_code in window if residue_code in {"H", "K"}
                ),
            }
        )

    glycine_pair_hits: list[dict[str, Any]] = []
    n_terminal_scan_end = min(len(sequence), 60)
    for index in range(0, max(n_terminal_scan_end - 3, 0)):
        motif = sequence[index : index + 4]
        if (
            motif[0] in {"I", "L", "M", "V"}
            and motif[1] == "G"
            and motif[2] in {"I", "L", "M", "V"}
            and motif[3] == "G"
        ):
            glycine_pair_hits.append(
                {
                    "motif": motif,
                    "motif_type": "n_terminal_xgxg_cofactor_binding_proxy",
                    "start": index + 1,
                    "end": index + 4,
                }
            )

    has_source_active_site_tyr = bool(active_site_tyr_positions)
    has_local_h_or_k_near_tyr = local_h_or_k_count > 0
    has_glycine_pair_proxy = bool(glycine_pair_hits)
    if has_source_active_site_tyr and has_local_h_or_k_near_tyr and has_glycine_pair_proxy:
        status = "akr_nadp_axis_present_with_source_active_site_tyr"
    elif has_source_active_site_tyr and has_glycine_pair_proxy:
        status = "partial_akr_nadp_axis_signal"
    elif has_source_active_site_tyr or has_glycine_pair_proxy:
        status = "partial_akr_nadp_axis_signal"
    else:
        status = "akr_nadp_axis_absent"

    return {
        "sequence_length": len(sequence),
        "source_active_site_positions": sorted(active_site_position_set),
        "source_active_site_tyr_positions": active_site_tyr_positions,
        "source_active_site_tyr_count": len(active_site_tyr_positions),
        "active_site_tyr_local_windows": active_site_tyr_windows,
        "active_site_tyr_local_h_or_k_count": local_h_or_k_count,
        "n_terminal_glycine_pair_cofactor_proxy_motif_hits": glycine_pair_hits,
        "has_n_terminal_glycine_pair_cofactor_proxy": has_glycine_pair_proxy,
        "akr_nadp_sequence_axis_status": status,
    }


def _external_akr_nadp_reference_contrast(
    *,
    reference_accession: str,
    reference_sequences: dict[str, dict[str, Any]],
    baseline_row: dict[str, Any],
    ...
    features = _external_akr_nadp_sequence_features(
        sequence,
        active_site_positions=[],
    )
    label_summaries = [
    ...
    return {
        ...
        "embedding_cosine_by_backend": dict(sorted(cosine_by_backend.items())),
        "sequence_length": features["sequence_length"],
        "n_terminal_glycine_pair_cofactor_proxy_motif_hits": features[
            "n_terminal_glycine_pair_cofactor_proxy_motif_hits"
        ],
        "source_active_site_tyr_positions": features[
            "source_active_site_tyr_positions"
        ],
        "akr_nadp_sequence_axis_status": (
            "akr_nadp_axis_present"
            if features["akr_nadp_sequence_axis_status"]
            == "akr_nadp_axis_present_with_source_active_site_tyr"
            else features["akr_nadp_sequence_axis_status"]
        ),
    }


def _external_akr_nadp_control_interpretation(
    *,
    candidate_has_axis: bool,
    full_reference_match_count: int,
) -> str:
    if candidate_has_axis and full_reference_match_count == 0:
        return (
            "Candidate carries a sequence-derived AKR/NADP axis signal with a "
            "source-traced active-site Tyr, while conflicting current-reference "
            "neighbors lack that complete axis. This supports a bounded "
            "review-only repair-control experiment, not import."
        )
    if candidate_has_axis:
        return (
            "Candidate carries the sequence-derived AKR/NADP axis, but at least "
            "one current-reference neighbor also carries it; broader controls "
            "remain needed before any scorer change."
        )
    return (
        "The source row does not yet have a complete sequence-derived AKR/NADP "
        "axis signal, so the repair lane remains unimplemented."
    )
...
def _external_parse_int(value: Any) -> int:
    parsed = _external_parse_float(value)
