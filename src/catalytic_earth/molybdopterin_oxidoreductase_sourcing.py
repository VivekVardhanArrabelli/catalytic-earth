"""Broadened-handle sourcing for molybdopterin/Mo-cofactor oxidoreductases."""
...
ARTIFACT_ID = "v3_molybdopterin_oxidoreductase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "molybdopterin_oxidoreductase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# Lanes are supply selectors only. EC 1.* scopes oxidoreductase candidates;
# membership is confirmed later by Mo-cofactor/molybdopterin mechanism evidence.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "moco_nitrate_tmao_reductases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND "
                "((ec:1.7.5.*) OR (ec:1.9.6.*) OR (ec:1.7.2.*)) AND "
                "((cc_cofactor:molybdopterin) OR (cc_cofactor:molybdenum) OR "
                "(keyword:Molybdenum))"
            ),
        },
        {
            "lane_id": "moco_formate_xanthine_oxidoreductases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND "
                "((ec:1.17.*) OR (ec:1.2.3.*)) AND "
                "((cc_cofactor:molybdopterin) OR (cc_cofactor:molybdenum) OR "
                "(keyword:Molybdenum))"
            ),
        },
        {
            "lane_id": "moco_sulfur_dmso_reductases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND "
                "((ec:1.8.3.*) OR (ec:1.8.5.*) OR (ec:1.20.*)) AND "
                "((cc_cofactor:molybdopterin) OR (cc_cofactor:molybdenum) OR "
                "(keyword:Molybdenum))"
            ),
        },
        {
            "lane_id": "moco_broad_molybdenum_oxidoreductase",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.*) AND "
                "(keyword:Molybdenum) AND (keyword:Oxidoreductase)"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a molybdopterin sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_molybdopterin_oxidoreductase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "molybdopterin_oxidoreductase_broadened_handle_fresh_fetch",
                "path": "molybdopterin_oxidoreductase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_molybdopterin_annotation",
        "stage": "scaling_plan_to_10k:wire_molybdopterin_oxidoreductase_broadened_handle",
        ...
        "guardrails": {
            ...
            "predictive_features_use_ec_name_keyword_molybdopterin_or_prose": False,
            "ec_used_for_scope_assignment_only_never_predictive": True,
            "molybdopterin_handles_scope_admission_only": True,
            ...
            "heme_flavin_copper_peroxidase_hydrolase_boundary_guard": True,
            "non_oxidoreductase_side_ec_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Molybdopterin handles scope-admission only, never predictive: "
            ...
            "- Boundary guards: "
            f"{audit['guardrails']['heme_flavin_copper_peroxidase_hydrolase_boundary_guard']}.",
...
def write_molybdopterin_oxidoreductase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_molybdopterin_oxidoreductase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
