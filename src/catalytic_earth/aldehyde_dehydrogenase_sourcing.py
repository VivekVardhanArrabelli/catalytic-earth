"""Broadened-handle sourcing for aldehyde dehydrogenases."""
...
ARTIFACT_ID = "v3_aldehyde_dehydrogenase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "aldehyde_dehydrogenase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "aldehyde_dehydrogenase_ec_1_2_1_named_nad",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.2.1.*) AND "
                '((protein_name:"aldehyde dehydrogenase") OR (protein_name:ALDH)) AND '
                "((keyword:NAD) OR (keyword:NADP))"
            ),
        },
        {
            "lane_id": "aldehyde_dehydrogenase_ec_1_2_1_active_site",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.2.1.*) AND "
                '((protein_name:"aldehyde dehydrogenase") OR (protein_name:ALDH)) AND '
                "((ft_act_site:cysteine) OR (ft_act_site:glutamate) OR "
                "(ft_act_site:cys) OR (ft_act_site:glu))"
            ),
        },
        {
            "lane_id": "aldehyde_dehydrogenase_ec_1_2_1_cys_glu",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.2.1.*) AND "
                '((protein_name:"aldehyde dehydrogenase") OR (protein_name:ALDH)) AND '
                "((cc_catalytic_activity:aldehyde) OR (cc_catalytic_activity:NAD) OR "
                "(cc_catalytic_activity:NADP))"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an aldehyde dehydrogenase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_aldehyde_dehydrogenase_sourcing(
    *,
    ...
    """Source ALDH labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "aldehyde_dehydrogenase_broadened_handle_fresh_fetch",
                "path": "aldehyde_dehydrogenase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_aldh_nad_active_site_annotation",
        "stage": "scaling_plan_to_10k:wire_aldehyde_dehydrogenase_broadened_handle",
        ...
        "guardrails": {
            ...
            "aldehyde_dehydrogenase_handles_scope_admission_only": True,
            ...
            "molybdopterin_flavin_sdr_akr_generic_nad_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['molybdopterin_flavin_sdr_akr_generic_nad_boundary_guard']}.",
...
def write_aldehyde_dehydrogenase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_aldehyde_dehydrogenase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
