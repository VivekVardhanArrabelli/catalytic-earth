"""Broadened-handle sourcing for flavin-dependent disulfide reductases (EC 1.8.1)."""
...
ARTIFACT_ID = "v3_flavin_disulfide_reductase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "flavin_disulfide_reductase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 1.8.1 scopes the reviewed candidate supply only. EC 1.8.1 is a SUBSET of the existing
# flavin_dehydrogenase_reductase EC scope (1.3 / 1.6 / 1.8.1); the required hard mechanism anchor is
# an annotated FAD cofactor PLUS a Rhea/reviewed reaction reducing a disulfide/dithiol substrate
# (glutathione disulfide, thioredoxin-S2, lipoamide, trypanothione, CoA-disulfide) with NAD(P)H. The
# second corroborator is a disulfide-reductase / pyridine-nucleotide-disulfide family/name OR an
# annotated active/binding site. Sulfite reductase (EC 1.8.1.2, siroheme, no disulfide substrate),
# non-flavin thiol-redox enzymes, and non-1.8.1 side-EC bifunctional rows are boundary-guarded by the
# disambiguation engine; the `flavin_disulfide_reductase_signal` carves these rows out of the flavin
# rule so exactly one rule fires. NOTE: the FAD + NAD(P)H signature is shared with
# flavin_dehydrogenase_reductase, so this family is representation-confusable with it in the
# source-free feature space (cap 150).
FLAVIN_DISULFIDE_SCOPE = "(reviewed:true) AND (ec:1.8.1.*)"

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "flavin_disulfide_reductase_reaction_disulfide",
            "target_family_lane": FAMILY,
            "query": f'{FLAVIN_DISULFIDE_SCOPE} AND (cc_catalytic_activity:"disulfide")',
        },
        {
            "lane_id": "flavin_disulfide_reductase_fad_active_binding",
            "target_family_lane": FAMILY,
            "query": (
                f'{FLAVIN_DISULFIDE_SCOPE} AND (cc_cofactor:"FAD") AND '
                "((ft_act_site:*) OR (ft_binding:*))"
            ),
        },
        {
            "lane_id": "flavin_disulfide_reductase_named_all",
            "target_family_lane": FAMILY,
            "query": (
                f"{FLAVIN_DISULFIDE_SCOPE} AND "
                '((protein_name:"disulfide reductase") OR (protein_name:"glutathione reductase") '
                'OR (protein_name:"thioredoxin reductase") OR (protein_name:"dihydrolipoyl dehydrogenase") '
                'OR (protein_name:"trypanothione reductase"))'
            ),
        },
    )
}
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a flavin-disulfide-reductase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown flavin-disulfide-reductase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_flavin_disulfide_reductase_sourcing(
    *,
    families: tuple[str, ...] = FAMILIES,
    max_records_per_lane: int = 300,
    ...
    """Source flavin-disulfide-reductase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "flavin_disulfide_reductase_broadened_handle_fresh_fetch",
                "path": "flavin_disulfide_reductase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_fad_nadph_disulfide_reduction_annotation",
        "stage": "scaling_plan_to_10k:wire_flavin_disulfide_reductase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "flavin_disulfide_reductase_handles_scope_admission_only": True,
            ...
            "sulfite_reductase_glutaredoxin_peroxidase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['sulfite_reductase_glutaredoxin_peroxidase_side_ec_boundary_guard']}.",
...
def write_flavin_disulfide_reductase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_flavin_disulfide_reductase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
