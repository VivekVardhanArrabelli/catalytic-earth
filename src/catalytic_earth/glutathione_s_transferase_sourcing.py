"""Broadened-handle sourcing for glutathione S-transferases (EC 2.5.1.18)."""
...
ARTIFACT_ID = "v3_glutathione_s_transferase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "glutathione_s_transferase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 2.5.1.18 scopes the reviewed candidate supply only (NOT shared with any existing fingerprint);
# counted corroboration comes from a glutathione-transferase family/name plus a Rhea/reviewed
# reaction conjugating glutathione to an electrophile (-> an S-substituted glutathione). Glutathione
# peroxidase (EC 1.11.1), glutathione reductase (EC 1.8.1.7), glutathione synthetase (EC 6.3.2.3),
# glutaredoxin, and gamma-glutamyltransferase are guarded by the disambiguation engine's boundary
# signal.
GST_SCOPE = (
    "(reviewed:true) AND (ec:2.5.1.18) AND "
    '((protein_name:"glutathione transferase") OR (protein_name:"glutathione s-transferase"))'
)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "glutathione_s_transferase_reaction_glutathione",
            "target_family_lane": FAMILY,
            "query": f"{GST_SCOPE} AND (cc_catalytic_activity:glutathione)",
        },
        {
            "lane_id": "glutathione_s_transferase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{GST_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "glutathione_s_transferase_named_all",
            "target_family_lane": FAMILY,
            "query": GST_SCOPE,
        },
    )
...
def _lane_queries_for(
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown glutathione-S-transferase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_glutathione_s_transferase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "glutathione_s_transferase_broadened_handle_fresh_fetch",
                "path": "glutathione_s_transferase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_glutathione_conjugation_annotation",
        "stage": "scaling_plan_to_10k:wire_glutathione_s_transferase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "glutathione_s_transferase_handles_scope_admission_only": True,
            ...
            "glutathione_peroxidase_reductase_synthetase_glutaredoxin_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['glutathione_peroxidase_reductase_synthetase_glutaredoxin_boundary_guard']}.",
...
def write_glutathione_s_transferase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_glutathione_s_transferase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
