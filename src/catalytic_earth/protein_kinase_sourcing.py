"""Broadened-handle sourcing for Ser/Thr/Tyr protein kinases."""
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a protein kinase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_protein_kinase_sourcing(
    *,
    ...
    """Source Ser/Thr/Tyr protein kinase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "protein_kinase_broadened_handle_fresh_fetch",
                "path": "protein_kinase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_protein_kinase_annotation",
        "stage": "scaling_plan_to_10k:wire_protein_kinase_broadened_handle",
        ...
        "guardrails": {
            ...
            "protein_kinase_handles_scope_admission_only": True,
            ...
            "histidine_small_molecule_kinase_ligase_side_ec_boundary_guard": True,
...
def write_protein_kinase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_protein_kinase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
