"""Broadened-handle sourcing for alpha/beta hydrolase esterases/lipases."""
...
ARTIFACT_ID = "v3_alpha_beta_hydrolase_esterase_lipase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "alpha_beta_hydrolase_esterase_lipase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "alpha_beta_hydrolase_ec_3_1_1_esterase_lipase_named",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.1.*) AND "
                "((keyword:Esterase) OR (keyword:Lipase) OR "
                "(protein_name:esterase) OR (protein_name:lipase))"
            ),
        },
        {
            "lane_id": "alpha_beta_hydrolase_ec_3_1_1_ser_his_acid_site",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.1.*) AND "
                "((protein_name:esterase) OR (protein_name:lipase) OR "
                '(protein_name:"alpha/beta hydrolase")) AND '
                "((ft_act_site:serine) OR (ft_act_site:histidine) OR "
                "(ft_act_site:aspartate) OR (ft_act_site:glutamate))"
            ),
        },
        {
            "lane_id": "alpha_beta_hydrolase_ec_3_1_1_ester_hydrolysis",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.1.*) AND "
                "((cc_catalytic_activity:ester) OR (cc_catalytic_activity:lipase) OR "
                '(cc_catalytic_activity:"carboxylic ester"))'
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(
                f"{family!r} is not an alpha/beta hydrolase esterase/lipase sourcing family"
            )
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_alpha_beta_hydrolase_esterase_lipase_sourcing(
    *,
    ...
    """Source alpha/beta hydrolase esterase/lipase labels; return a preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "alpha_beta_hydrolase_esterase_lipase_broadened_handle_fresh_fetch",
                "path": "alpha_beta_hydrolase_esterase_lipase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_esterase_lipase_ser_his_acid_annotation",
        "stage": "scaling_plan_to_10k:wire_alpha_beta_hydrolase_esterase_lipase_broadened_handle",
        ...
        "guardrails": {
            ...
            "alpha_beta_hydrolase_handles_scope_admission_only": True,
            ...
            "protease_amidase_glycoside_metal_hydrolase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['protease_amidase_glycoside_metal_hydrolase_boundary_guard']}.",
...
def write_alpha_beta_hydrolase_esterase_lipase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_alpha_beta_hydrolase_esterase_lipase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
