"""Broadened-handle sourcing for aminoacyl-tRNA synthetases (EC 6.1.1)."""
...
ARTIFACT_ID = "v3_aminoacyl_trna_synthetase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "aminoacyl_trna_synthetase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 6.1.1 scopes the reviewed candidate supply only (NOT shared with any existing fingerprint);
# counted corroboration comes from an X--tRNA-ligase / aminoacyl-tRNA-synthetase family/name plus a
# Rhea/reviewed aminoacylation reaction (ATP + amino acid + tRNA -> aminoacyl-tRNA + AMP + diphosphate).
# tRNA-modifying enzymes (methyltransferases, pseudouridine synthases, CCA-adding nucleotidyltransferases,
# amidotransferases) are guarded by the disambiguation engine's boundary signal. NOTE: the ATP-adenylation
# step is shared with the EC 6.3 atp_amide_ligase, so this family is representation-confusable (cap 150).
AARS_SCOPE = (
...
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "aminoacyl_trna_synthetase_reaction_trna",
            "target_family_lane": FAMILY,
            "query": f"{AARS_SCOPE} AND (cc_catalytic_activity:tRNA)",
        },
        {
            "lane_id": "aminoacyl_trna_synthetase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{AARS_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "aminoacyl_trna_synthetase_named_all",
            "target_family_lane": FAMILY,
            "query": AARS_SCOPE,
        },
    )
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a PAPS-sulfotransferase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown aminoacyl-tRNA-synthetase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_aminoacyl_trna_synthetase_sourcing(
    *,
    ...
    """Source glutathione-S-transferase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "aminoacyl_trna_synthetase_broadened_handle_fresh_fetch",
                "path": "aminoacyl_trna_synthetase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_atp_trna_aminoacylation_annotation",
        "stage": "scaling_plan_to_10k:wire_aminoacyl_trna_synthetase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "aminoacyl_trna_synthetase_handles_scope_admission_only": True,
            ...
            "trna_modifying_methyltransferase_pseudouridine_amidotransferase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['trna_modifying_methyltransferase_pseudouridine_amidotransferase_boundary_guard']}.",
...
def write_aminoacyl_trna_synthetase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_aminoacyl_trna_synthetase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
