"""Broadened-handle sourcing for aldo-keto reductases (AKR superfamily)."""
...
ARTIFACT_ID = "v3_aldo_keto_reductase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "aldo_keto_reductase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 1.1.1 scopes the reviewed candidate supply only; counted corroboration comes
# from AKR family/name text, NADP(H) cosubstrate/binding context, and Rhea/reviewed
# NADP-coupled carbonyl-reduction reaction text. SDR/MDR/ALDH/flavin/metal/oxygenase
# rows are guarded by the disambiguation engine's AKR boundary signal.
AKR_SCOPE = (
    "(reviewed:true) AND (ec:1.1.1.*) AND "
    '((protein_name:"aldo-keto reductase") OR '
    '(protein_name:"aldose reductase") OR '
    '(protein_name:"aldehyde reductase") OR '
    "(protein_name:AKR))"
)
...
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "aldo_keto_reductase_named_nadp",
            "target_family_lane": FAMILY,
            "query": f"{AKR_SCOPE} AND ((keyword:NADP) OR (keyword:NAD))",
        },
        {
            "lane_id": "aldo_keto_reductase_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{AKR_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "aldo_keto_reductase_reaction_nadp",
            "target_family_lane": FAMILY,
            "query": (
                f"{AKR_SCOPE} AND ((cc_catalytic_activity:NADP) OR "
                "(cc_catalytic_activity:NADPH) OR (cc_catalytic_activity:NAD) OR "
                "(cc_catalytic_activity:NADH))"
            ),
        },
    )
}
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an aldo-keto reductase family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown AKR lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_aldo_keto_reductase_sourcing(
    *,
    ...
    """Source aldo-keto reductase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "aldo_keto_reductase_broadened_handle_fresh_fetch",
                "path": "aldo_keto_reductase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_akr_nadp_carbonyl_reduction_annotation",
        "stage": "scaling_plan_to_10k:wire_aldo_keto_reductase_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "aldo_keto_reductase_handles_scope_admission_only": True,
            ...
            "sdr_mdr_aldh_flavin_metal_redox_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['sdr_mdr_aldh_flavin_metal_redox_boundary_guard']}.",
...
def write_aldo_keto_reductase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_aldo_keto_reductase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
