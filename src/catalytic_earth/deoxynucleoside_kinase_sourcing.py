"""Broadened-handle sourcing for deoxynucleoside kinases."""
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a dNK sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_deoxynucleoside_kinase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "deoxynucleoside_kinase_broadened_handle_fresh_fetch",
                "path": "deoxynucleoside_kinase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_dnk_annotation",
        "stage": "scaling_plan_to_10k:wire_deoxynucleoside_kinase_broadened_handle",
        ...
        "guardrails": {
            ...
            "dnk_handles_scope_admission_only": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- dNK handles scope-admission only, never predictive: "
...
def write_deoxynucleoside_kinase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_deoxynucleoside_kinase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
