"""Broadened-handle sourcing for PfkA phosphofructokinases."""
...
ARTIFACT_ID = "v3_pfka_phosphofructokinase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "pfka_phosphofructokinase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "pfka_phosphofructokinase_reviewed_strict_ec_2_7_1",
            "target_family_lane": FAMILY,
            "query": (
                '(reviewed:true) AND (ec:2.7.1.*) AND '
                '((protein_name:"phosphofructokinase") OR '
                '(protein_name:"6-phosphofructokinase") OR '
                '(protein_name:"ATP-dependent 6-phosphofructokinase")) NOT '
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a PfkA sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_pfka_phosphofructokinase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "pfka_phosphofructokinase_broadened_handle_fresh_fetch",
                "path": "pfka_phosphofructokinase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_pfka_annotation",
        "stage": "scaling_plan_to_10k:wire_pfka_phosphofructokinase_broadened_handle",
        ...
        "guardrails": {
            ...
            "pfka_handles_scope_admission_only": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- PfkA handles scope-admission only, never predictive: "
...
def write_pfka_phosphofructokinase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_pfka_phosphofructokinase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
