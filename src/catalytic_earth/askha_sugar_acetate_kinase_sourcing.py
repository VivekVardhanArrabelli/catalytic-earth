"""Broadened-handle sourcing for ASKHA sugar acetate kinases."""
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not an ASKHA sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_askha_sugar_acetate_kinase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "askha_sugar_acetate_kinase_broadened_handle_fresh_fetch",
                "path": "askha_sugar_acetate_kinase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_askha_annotation",
        "stage": "scaling_plan_to_10k:wire_askha_sugar_acetate_kinase_broadened_handle",
        ...
        "guardrails": {
            ...
            "askha_handles_scope_admission_only": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- ASKHA handles scope-admission only, never predictive: "
            ...
            "- Protein-kinase/histidine-kinase/hydrolase/NDK/dNK/GHMP/Pfk boundary guards: "
...
def write_askha_sugar_acetate_kinase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_askha_sugar_acetate_kinase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
