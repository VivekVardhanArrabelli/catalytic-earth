"""Broadened-handle sourcing for aminoglycoside phosphotransferases."""
...
ARTIFACT_ID = "v3_aminoglycoside_phosphotransferase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "aminoglycoside_phosphotransferase"
...
APH_EXACT_EC_ACTIVE_BINDING_QUERY = (
    f"{APH_EXACT_EC_QUERY} AND ((ft_binding:*) OR (ft_act_site:*))"
)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "aminoglycoside_phosphotransferase_exact_ec_active_binding",
            "target_family_lane": FAMILY,
            "query": APH_EXACT_EC_ACTIVE_BINDING_QUERY,
        },
    )
}
...
def _lane_queries_for(
    ...
    *,
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown aminoglycoside phosphotransferase lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_aminoglycoside_phosphotransferase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "aminoglycoside_phosphotransferase_exact_ec_fresh_fetch",
                "path": "aminoglycoside_phosphotransferase_exact_ec_fresh_fetch",
    ...
    return {
        ...
        "guardrails": {
            ...
            "protein_kinase_acetyltransferase_nucleotidyltransferase_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['protein_kinase_acetyltransferase_nucleotidyltransferase_boundary_guard']}.",
...
def write_aminoglycoside_phosphotransferase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_aminoglycoside_phosphotransferase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
