"""Broadened-handle sourcing for zinc-dependent hydro-lyases/hydratases."""
...
ARTIFACT_ID = "v3_zinc_lyase_hydratase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "zinc_lyase_hydratase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 4.2.1 scopes reviewed hydro-lyase candidates only. Admission requires
# non-EC Zn cofactor/site context, Rhea hydration/elimination/carbonic reaction
# text, lyase/hydratase family text, or active-/binding-/metal-site evidence.
# PLP, ThDP, hydrolase/transferase/aldolase/isomerase side rows, non-4.2.1 side
# ECs, and multi-fingerprint rows are held.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "zinc_ec_4_2_1_reviewed",
            "target_family_lane": FAMILY,
            "query": (
                "(ec:4.2.1.*) AND (reviewed:true) AND "
                "((cc_cofactor:zinc) OR (keyword:Zinc))"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a zinc hydro-lyase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_zinc_lyase_hydratase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "zinc_lyase_hydratase_broadened_handle_fresh_fetch",
                "path": "zinc_lyase_hydratase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_zinc_lyase_hydratase_annotation",
        "stage": "scaling_plan_to_10k:wire_zinc_lyase_hydratase_broadened_handle",
        ...
        "guardrails": {
            ...
            "zinc_hydratase_handles_scope_admission_only": True,
            ...
            "plp_thdp_hydrolase_transferase_aldolase_isomerase_side_ec_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Zinc/hydratase handles scope-admission only, never predictive: "
            ...
            "- PLP/ThDP/hydrolase/transferase/aldolase/isomerase/side-EC boundary guards: "
            f"{audit['guardrails']['plp_thdp_hydrolase_transferase_aldolase_isomerase_side_ec_boundary_guard']}.",
...
def write_zinc_lyase_hydratase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_zinc_lyase_hydratase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
