"""Broadened-handle sourcing for HAD-like phosphatases."""
...
ARTIFACT_ID = "v3_had_like_phosphatase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "had_like_phosphatase"
FAMILIES: tuple[str, ...] = (FAMILY,)

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "had_like_phosphatase_ec_3_1_3_had_named",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.3.*) AND "
                '((protein_name:HAD) OR (protein_name:"haloacid dehalogenase") OR '
                '(protein_name:"HAD-like") OR (protein_name:"HAD family"))'
            ),
        },
        {
            "lane_id": "had_like_phosphatase_ec_3_1_3_asp_mg",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.3.*) AND "
                "((protein_name:phosphatase) OR (keyword:Phosphatase)) AND "
                "((cc_cofactor:magnesium) OR (protein_name:aspartate) OR "
                "(protein_name:phosphoserine))"
            ),
        },
        {
            "lane_id": "had_like_phosphatase_ec_3_1_3_phosphoserine",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:3.1.3.*) AND "
                '((protein_name:"phosphoserine phosphatase") OR '
                '(protein_name:"phosphoglycolate phosphatase") OR '
                '(protein_name:"haloacid dehalogenase-like"))'
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a HAD-like phosphatase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_had_like_phosphatase_sourcing(
    *,
    families: tuple[str, ...] = FAMILIES,
    max_records_per_lane: int = 220,
    ...
    """Source HAD-like phosphatase labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "had_like_phosphatase_broadened_handle_fresh_fetch",
                "path": "had_like_phosphatase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_had_phosphatase_annotation",
        "stage": "scaling_plan_to_10k:wire_had_like_phosphatase_broadened_handle",
        ...
        "guardrails": {
            ...
            "had_like_handles_scope_admission_only": True,
            ...
            "protein_phosphatase_metal_phosphatase_pde_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['protein_phosphatase_metal_phosphatase_pde_boundary_guard']}.",
...
def write_had_like_phosphatase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_had_like_phosphatase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
