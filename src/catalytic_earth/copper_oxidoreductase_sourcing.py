"""Broadened-handle sourcing for copper-center oxidoreductases."""
...
ARTIFACT_ID = "v3_copper_oxidoreductase_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "copper_oxidoreductase"
FAMILIES: tuple[str, ...] = (FAMILY,)

# Lanes are supply selectors only. EC scopes reviewed copper oxidase candidates;
# membership is confirmed later by non-EC copper/Rhea/active-site mechanism evidence.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "copper_multicopper_laccase_oxidases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.10.3.*) AND "
                "((cc_cofactor:copper) OR (keyword:Copper))"
            ),
        },
        {
            "lane_id": "copper_amine_oxidases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:1.4.3.*) AND "
                "((cc_cofactor:copper) OR (keyword:Copper))"
            ),
        },
    )
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a copper oxidoreductase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_copper_oxidoreductase_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "copper_oxidoreductase_broadened_handle_fresh_fetch",
                "path": "copper_oxidoreductase_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_copper_oxidoreductase_annotation",
        "stage": "scaling_plan_to_10k:wire_copper_oxidoreductase_broadened_handle",
        ...
        "guardrails": {
            ...
            "predictive_features_use_ec_name_keyword_copper_or_prose": False,
            "ec_used_for_scope_assignment_only_never_predictive": True,
            "copper_handles_scope_admission_only": True,
            ...
            "heme_flavin_molybdopterin_hydrolase_glyco_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Copper handles scope-admission only, never predictive: "
            ...
            "- Boundary guards: "
            f"{audit['guardrails']['heme_flavin_molybdopterin_hydrolase_glyco_boundary_guard']}.",
...
def write_copper_oxidoreductase_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_copper_oxidoreductase_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
