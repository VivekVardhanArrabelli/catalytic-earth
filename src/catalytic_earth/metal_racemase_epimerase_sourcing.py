"""Broadened-handle sourcing for non-PLP racemase/epimerase enzymes."""
...
ARTIFACT_ID = "v3_metal_racemase_epimerase_non_plp_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "metal_racemase_epimerase_non_plp"
FAMILIES: tuple[str, ...] = (FAMILY,)

# Lanes are supply selectors only. EC 5.1 scopes reviewed racemase/epimerase candidates;
# membership is confirmed later by non-EC racemase/epimerase text, Rhea isomerization,
# active-/binding-site, metal, or cofactorless mechanism evidence.
FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "non_plp_racemase_epimerase_reviewed_isomerases",
            "target_family_lane": FAMILY,
            "query": (
                "(reviewed:true) AND (ec:5.1.*) AND (keyword:Isomerase) "
                "NOT (cc_cofactor:pyridoxal)"
            ),
        },
    )
}
...
def _lane_queries_for(families: tuple[str, ...]) -> tuple[dict[str, str], ...]:
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a non-PLP racemase/epimerase sourcing family")
        lanes.extend(FAMILY_LANE_QUERIES[family])
    return tuple(lanes)


def build_metal_racemase_epimerase_non_plp_sourcing(
    *,
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "metal_racemase_epimerase_non_plp_broadened_handle_fresh_fetch",
                "path": "metal_racemase_epimerase_non_plp_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_rhea_metal_racemase_epimerase_non_plp_annotation",
        "stage": "scaling_plan_to_10k:wire_metal_racemase_epimerase_non_plp_broadened_handle",
        ...
        "guardrails": {
            ...
            "racemase_epimerase_handles_scope_admission_only": True,
            ...
            "plp_and_side_ec_boundary_guard": True,
            "non_5_1_side_ec_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Racemase/epimerase handles scope-admission only, never predictive: "
            ...
            "- PLP and side-EC boundary guards: "
            f"{audit['guardrails']['plp_and_side_ec_boundary_guard']}.",
...
def write_metal_racemase_epimerase_non_plp_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_metal_racemase_epimerase_non_plp_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
