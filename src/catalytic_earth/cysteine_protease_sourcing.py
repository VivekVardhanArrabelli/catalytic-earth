"""Broadened-handle sourcing for cysteine (thiol) proteases (EC 3.4.22)."""
...
ARTIFACT_ID = "v3_cysteine_protease_sourcing_preview_current702"
SCHEMA_VERSION = "external_annotation_anchored_import.v1"

FAMILY = "cysteine_protease"
FAMILIES: tuple[str, ...] = (FAMILY,)

# EC 3.4.22 scopes the reviewed candidate supply only (NOT shared with any existing fingerprint;
# metallopeptidase covers EC 3.4.24/17/11 and ser_his_acid_hydrolase the serine 3.4.21 class).
# Peptide-bond hydrolysis carries no specific Rhea, so the required hard mechanism anchor is the
# annotated catalytic active site (the catalytic Cys nucleophile / Cys-His dyad), and a cysteine/
# thiol-peptidase or protease family/name is the second corroborator. Serine proteases (EC 3.4.21),
# aspartic proteases (EC 3.4.23), metallopeptidases (catalytic metal), and protease inhibitors
# (cystatins) are guarded by the disambiguation engine's boundary signal. NOTE: cofactor-free
# peptide hydrolysis is representation-confusable with ser_his_acid_hydrolase (cap 150).
CYS_SCOPE = "(reviewed:true) AND (ec:3.4.22.*)"

FAMILY_LANE_QUERIES: dict[str, tuple[dict[str, str], ...]] = {
    FAMILY: (
        {
            "lane_id": "cysteine_protease_active_binding",
            "target_family_lane": FAMILY,
            "query": f"{CYS_SCOPE} AND ((ft_act_site:*) OR (ft_binding:*))",
        },
        {
            "lane_id": "cysteine_protease_thiol_keyword",
            "target_family_lane": FAMILY,
            "query": f'{CYS_SCOPE} AND (keyword:"Thiol protease")',
        },
        {
            "lane_id": "cysteine_protease_named_all",
            "target_family_lane": FAMILY,
            "query": (
                f"{CYS_SCOPE} AND "
                '((protein_name:"cathepsin") OR (protein_name:"caspase") '
                'OR (protein_name:"peptidase") OR (protein_name:"protease"))'
            ),
        },
    )
}
...
def _lane_queries_for(
    ...
    for family in families:
        if family not in FAMILY_LANE_QUERIES:
            raise ValueError(f"{family!r} is not a cysteine-protease family")
    ...
    if requested_lane_ids:
        ...
        if missing:
            raise ValueError(f"unknown cysteine-protease lane ids: {', '.join(missing)}")
    return tuple(lanes)


def build_cysteine_protease_sourcing(
    *,
    ...
    """Source cysteine-protease labels; return a non-destructive preview."""
    ...
    disambig = build_cofactor_ec_disambiguation(
        pools=[
            {
                "pool": "cysteine_protease_broadened_handle_fresh_fetch",
                "path": "cysteine_protease_broadened_handle_fresh_fetch",
    ...
    return {
        ...
        "evidence_basis": "reviewed_swissprot_ec_active_site_cys_his_thiol_protease_annotation",
        "stage": "scaling_plan_to_10k:wire_cysteine_protease_new_fingerprint_lane",
        ...
        "guardrails": {
            ...
            "cysteine_protease_handles_scope_admission_only": True,
            ...
            "serine_aspartic_metallo_protease_inhibitor_boundary_guard": True,
...
def _report(audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Boundary guards: {audit['guardrails']['serine_aspartic_metallo_protease_inhibitor_boundary_guard']}.",
...
def write_cysteine_protease_sourcing(
    *,
    out_path: Path,
    ...
    audit = build_cysteine_protease_sourcing(
        families=families,
        max_records_per_lane=max_records_per_lane,
        record_offset_per_lane=record_offset_per_lane,
        record_limit_per_lane=record_limit_per_lane,
