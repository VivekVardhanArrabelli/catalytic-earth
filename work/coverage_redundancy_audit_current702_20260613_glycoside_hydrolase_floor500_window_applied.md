...
Run: 2026-06-13T18:10:37Z

Non-destructive, metadata-only audit of all combined labels (7190 = 702 frozen current702 + 6488 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5494; out_of_scope: 1696; positive:OOS = 3.2394.
- Fingerprint Gini 0.1675 / normalized entropy 0.9851; max/min(nonzero) ratio 6.7 (308 vs 46).
...
- Measurable rows: 6334; clusters (size >= 3): 133; rows in clusters: 519 (0.0819 of measurable).
