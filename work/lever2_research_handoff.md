...
- STARTED_AT: `2026-06-06T03:31:51Z`
- STARTED_LOCAL: `2026-06-05T22:31:51-0500 CDT`
- ENDED_AT: `2026-06-06T03:49:41Z`
- ENDED_LOCAL: `2026-06-05T22:49:41-0500 CDT`
- ELAPSED_MINUTES: `17.83`

#### Intent

Continue Lever 2 electron-flow research only on `lever-2-research-track`.
Use the source-free row-gate audit from Run 34 as input and produce the next
train/cal-disciplined measured readout: direct source-free electron-flow
operating-point value beyond the current geometry/fold surface while preserving
primary retention, without applying protected imports or editing protected
surfaces.
...
- Initial disk check in the detached automation checkout and then in the
  branch worktree showed 11 GiB free, above the 10 GiB guardrail.
- Read the prior Lever 2 handoff first. The automation memory file was absent
  at start, so the handoff was the only available run memory.
- Added
  `build-lever2-source-free-electron-flow-current-split-operating-point-readout`.
  It consumes the Run 34 row-gate audit, computes the current geometry/fold
  baseline from the measured full-gate recall/delta, compares smoke and full
  direct source-free electron-flow OR overlays, estimates the calibration-OOS
  denominator/row counts, and decomposes the signal by PQQ, NAD-family, and
  Fe-S/iron components.
...
- No canonical approved sidecar, protected import, production threshold,
  registry, ontology, label, heldout split, or Lever 3 surface was edited.
...
- Status:
  ...
  Result class:
  `research_only_direct_source_free_operating_point_signal_pending_protected_import`.
- Source row-gate matrix SHA-256:
  `98a5eb56d8c5cf28bba414f085fa4f3723ef65ce46c73007825b29acb3a8d5b1`.
- Estimated calibration-OOS denominator: 75 rows.
- Current geometry/fold baseline OOS abstain recall: `0.466667`
  (estimated 35/75 OOS abstained).
- Smoke tranche (`m_csa:104` plus 34 primary rows): 35/35 complete source-free
  rows, 0 primary-positive rows, primary retain recall `1.0`, OOS abstain
  recall `0.48`, incremental OOS abstain recall `0.013333`, and estimated
  36/75 OOS abstained.
- Full current split: 74/74 complete source-free rows, 34 current primary rows,
  40 retained-OOS rows, 0 primary-positive rows, primary retain recall `1.0`,
  retained-OOS positives `m_csa:104`, `m_csa:119`, and `m_csa:464`, OOS
  abstain recall `0.506667`, incremental OOS abstain recall `0.04`, and
  estimated 38/75 OOS abstained.
- Component decomposition: PQQ (`m_csa:104`), NAD-family (`m_csa:464`), and
  Fe-S/iron (`m_csa:119`) each add `0.013333` OOS abstain recall with zero
  primary-positive rows.
- Decision flags: direct source-free electron-flow operating point measured
  `true`, adds operating-point value beyond current geometry/fold `true`,
  primary retention preserved `true`, done-bar evidence measured `true`,
  deployable now `false`.
- Interpretation: no source-free electron-flow evidence gap remains for the
  current train/cal split. The remaining gap is explicit protected import
  authorization and approved-sidecar rerun, not missing source-free rows.
...
- No mechanism text, EC/Rhea IDs, labels, source IDs, target names,
  ...
  feature values. Entry IDs are used only for tranche, delta, row-gate, and
  contract accounting.
- The operating-point readout uses only the eight direct source-free
  electron-flow fields already audited in the source row-gate matrix.
- Protected imports executed: 0. Approved sidecar rows written: 0.
- Final disk check showed 11079 MiB free, above the 10 GiB guardrail.

#### Validation

- New CLI artifact generation:
  `PYTHONPATH=src python -m catalytic_earth.cli build-lever2-source-free-electron-flow-current-split-operating-point-readout`:
  wrote baseline/smoke/full OOS recall
  `0.466667/0.48/0.506667` with result class
  `research_only_direct_source_free_operating_point_signal_pending_protected_import`.
- Focused new unit/parser/regression slice:
  `PYTHONPATH=src python -m pytest tests/test_lever2_mechanism_incremental_readout.py::Lever2MechanismIncrementalReadoutTests::test_electron_flow_current_split_operating_point_readout_scores_gate_overlay tests/test_cli.py::CliTests::test_lever2_electron_flow_current_split_operating_point_parser_defaults tests/test_geometry_artifact_regression.py::GeometryArtifactRegressionTests::test_lever2_electron_flow_current_split_operating_point_current_counts -q`:
  3 passed.
- Touched-file slice:
  ...
  521 passed, 193 subtests passed.
- `PYTHONPATH=src python -m compileall -q src tests`: passed.
- Full pytest:
  `PYTHONPATH=src python -m pytest -q`: 1598 passed, 212 subtests passed,
  with the existing sklearn/SciPy L-BFGS-B deprecation warning.
- Full unittest discovery:
  `PYTHONPATH=src python -m unittest discover -s tests -q`: 1553 tests OK,
  with the same existing warning.
- Hash-seed stability: focused component-aware new slice passed for
  `PYTHONHASHSEED=1` through `5`.
...
- Guardrail scans found no changed label/registry/ontology/threshold/heldout,
  protected import, or Lever 3 paths and no true guardrail flags in the new
  artifact/report.
...
- With explicit protected import authorization only: apply the 35-row smoke
  tranche, rerun this operating-point readout on the approved sidecar, and
  expand the remaining 39 rows only if primary positives remain zero. Without
  that authorization, do not edit the approved sidecar/import surfaces; the
  remaining gap is policy/authorization, not source-free electron-flow
  evidence.
...
Continue Lever 2 electron-flow research only on `lever-2-research-track`.
Start from the candidate-sidecar positive readout and pursue the next measured
train/cal-disciplined source-free electron-flow route without editing canonical
approved imports or production thresholds.
...
- Added
  ...
  approved train/cal feature sidecar, extracts the exact collision-safe
  namespaced direct electron-flow smoke/full delta rows, applies those deltas
  to an in-memory copy of the approved sidecar, and reruns the fixed smoke and
  74-row gates from the reconstructed sidecar states.
- Generated the measured artifact/report:
  `artifacts/v3_lever2_source_free_electron_flow_approval_import_delta_package_readout_current702_20260605.json`
  and
...
- Status:
  ...
  Result class:
  `research_only_candidate_sidecar_delta_package_ready_pending_protected_import`.
- Approved sidecar before delta: 43 rows, only 1 current-split entry present
  before delta, and 0 current-split rows complete for the eight direct
  source-free electron-flow component fields.
- Smoke package: 35 rows, 34 new approved-sidecar rows and 1 existing-row
  update, 280 direct field-cell writes, 0 conflicts. Reconstructed smoke
  sidecar has 77 rows.
- Smoke replay gate: 35/35 complete, 34 current primary rows, 0 primary
  positives, primary retain recall `1.0`, retained-OOS positive `m_csa:104`,
  incremental OOS abstain recall `0.013333`, union OOS recall `0.48`, and exact
  match to the candidate-sidecar smoke gate.
- Full current-split package: 74 rows total after smoke, 73 new rows and 1
  existing-row update, 592 direct field-cell writes, 0 conflicts. Reconstructed
  full sidecar has 116 rows.
- Full replay gate: 74/74 complete, 34 current primary rows, 0 primary
  positives, primary retain recall `1.0`, retained-OOS positives `m_csa:104`,
  `m_csa:119`, and `m_csa:464`, incremental OOS abstain recall `0.04`, union
  OOS recall `0.506667`, and exact match to the candidate-sidecar full gate.
- Direct feature whitelist/integrity audit found only the eight allowed
  namespaced source-free electron-flow keys across all 74 packaged rows.
- Interpretation: direct source-free electron-flow signal is now measured as
  an apply-ready research-only protected delta package. No source-free
  electron-flow evidence blocker remains for the current train/cal split; the
  remaining gap is protected import authorization.
...
- Focused new unit/parser/regression slice:
  `PYTHONPATH=src python -m pytest tests/test_lever2_mechanism_incremental_readout.py::Lever2MechanismIncrementalReadoutTests::test_electron_flow_approval_import_delta_package_readout tests/test_cli.py::CliTests::test_lever2_electron_flow_approval_import_delta_package_parser_defaults tests/test_geometry_artifact_regression.py::GeometryArtifactRegressionTests::test_lever2_electron_flow_approval_import_delta_package_current_counts -q`:
  3 passed.
- Touched-file slice:
  `PYTHONPATH=src python -m pytest tests/test_cli.py tests/test_lever2_mechanism_incremental_readout.py tests/test_geometry_artifact_regression.py -q`:
  510 passed, 193 subtests passed.
- Full pytest:
  `PYTHONPATH=src python -m pytest -q`: 1587 passed, 212 subtests passed, with
  the existing sklearn/SciPy L-BFGS-B deprecation warning.
- Full unittest discovery:
...
- `PYTHONPATH=src python -m pytest tests/test_doc_reference_check.py -q`:
  2 passed.
- `git diff --check`: passed.
- Normalized regeneration matched after deleting `created_utc`; direct-field
  whitelist/integrity check passed with 74 rows and 8 allowed fields.
...
- With explicit protected import authorization, apply the 35-row smoke delta
  package to the approved sidecar, rerun the written approved-sidecar-only
  smoke gate, then apply the remaining 39 current-split rows and rerun the
  74-row gate. Without that authorization, treat the remaining gap as policy,
  not missing source-free electron-flow evidence.

### 2026-06-05 Lever 2 Research Run 30

#### Wall-clock ledger

- STARTED_AT: `2026-06-05T20:32:40Z`
- STARTED_LOCAL: `2026-06-05T15:32:40-0500 CDT`
- ENDED_AT: `2026-06-05T20:51:45Z`
- ENDED_LOCAL: `2026-06-05T15:51:45-0500 CDT`
- ELAPSED_MINUTES: `19.08`

#### Intent

Continue Lever 2 electron-flow research only on `lever-2-research-track`.
Execute the next train/cal-disciplined measured route after the positive
source-free approval/import smoke materialization readout: protected
namespaced smoke import, written approved-sidecar-only smoke gate, then expand
toward the remaining current-split rows only if primary retention remains
preserved.
...
- Switched substantive work to the dedicated branch worktree
  `/Users/vivekvardhanarrabelli/.codex/worktrees/88bb/catalytic-earth` on
  `lever-2-research-track`, isolated from Lever 3.
- Initial disk was below the 10 GiB guardrail at 8.0 GiB free. The clean
  detached duplicate checkout
  `/Users/vivekvardhanarrabelli/.codex/worktrees/6814/catalytic-earth` was
  removed with `git worktree remove`, restoring disk to 12 GiB free before
  research writes continued.
- Added
  `build-lever2-source-free-electron-flow-approval-import-candidate-sidecar-readout`.
  This consumes the prior smoke materialization readout and the current
  approved train/cal feature sidecar, overlays only the collision-safe
  namespaced direct electron-flow fields into a research-only candidate sidecar
  for the 35-row smoke tranche, reruns the written-sidecar-only smoke gate,
  then expands the same candidate sidecar to the remaining 39 current-split rows
  and reruns the full 74-row fixed gate.
- Added candidate-sidecar component ablations in the written-sidecar shape for
  PQQ-only, NAD-family-only, Fe-S-only, PQQ+NAD, and PQQ+NAD+Fe-S direct
  source-free electron-flow components.
- Generated the measured artifact/report:
  `artifacts/v3_lever2_source_free_electron_flow_approval_import_candidate_sidecar_readout_current702_20260605.json`
  and
  `work/lever2_source_free_electron_flow_approval_import_candidate_sidecar_readout_current702_20260605.md`.
  SHA-256:
  `e075997a85fbc46442438bd64b4ca9f656669177b53acb6a8fb5d35c633ab90c`
  for the JSON and
  `3d7dea6086d062aef6498d21ba36ac1c5e0c74014fc8102af73301c4983bac4a`
  for the Markdown report.
...
- Status:
  ...
  Result class:
  `research_only_candidate_sidecar_smoke_written_gate_positive_full_expansion_positive`.
- The research-only candidate sidecar has 77 rows after smoke import
  simulation: 43 canonical approved sidecar rows plus 34 new smoke rows and
  one existing-row update. The canonical approved sidecar was not edited.
- Candidate written-sidecar-only smoke gate: 35/35 complete, 34 current primary
  rows, 0 current-primary positives, primary retain recall `1.0`, retained-OOS
  positives `m_csa:104`, incremental OOS abstain recall `0.013333`, union OOS
  recall `0.48`, and exact match to the prior smoke materialization gate.
- Candidate written-sidecar-only full current-split gate after expanding the
  remaining 39 rows: 74/74 complete, 34 current primary rows, 40 retained-OOS
  rows, 0 current-primary positives, primary retain recall `1.0`, retained-OOS
  positives `m_csa:104`, `m_csa:119`, and `m_csa:464`, incremental OOS abstain
  recall `0.04`, union OOS recall `0.506667`, and exact match to the prior
  full materialization gate.
- Component ablation in candidate written-sidecar shape confirms the direct
  source-free component split: PQQ-only catches `m_csa:104`, NAD-family-only
  catches `m_csa:464`, Fe-S-only catches `m_csa:119`, PQQ+NAD catches
  `m_csa:104` and `m_csa:464`, and Fe-S adds `m_csa:119` as the incremental
  row beyond PQQ+NAD.
- Interpretation: direct source-free electron-flow features now have a
  written-sidecar-shaped train/cal readout in a candidate artifact that
  preserves primary retention and adds operating-point value beyond the current
  geometry/fold surface. No source-free electron-flow evidence blocker remains
  for the current train/cal split; the remaining gap is policy/authorization to
  edit the canonical approved sidecar/import surface.
...
- No mechanism text, EC/Rhea IDs, labels, source IDs, target names,
  accessions, PDB IDs, coordinate paths, or provenance were used as predictive
  feature values. Entry IDs are used only for tranche membership, merge
...
- The readout remains research-only: it writes a candidate artifact, not a
  canonical approved sidecar; `approved_sidecar_written=false`,
  `imports_or_promotions_performed=false`, and
...
- Focused new tests:
  `PYTHONPATH=src python -m pytest tests/test_lever2_mechanism_incremental_readout.py::Lever2MechanismIncrementalReadoutTests::test_electron_flow_approval_import_candidate_sidecar_readout tests/test_cli.py::CliTests::test_lever2_electron_flow_approval_import_candidate_sidecar_parser_defaults -q`:
  2 passed.
- Current artifact regression:
  `PYTHONPATH=src python -m pytest tests/test_geometry_artifact_regression.py::GeometryArtifactRegressionTests::test_lever2_electron_flow_approval_import_candidate_sidecar_current_counts -q`:
  1 passed.
- Touched-file slice:
  `PYTHONPATH=src python -m pytest tests/test_cli.py tests/test_lever2_mechanism_incremental_readout.py tests/test_geometry_artifact_regression.py -q`:
  507 passed, 193 subtests passed.
- Full pytest:
  `PYTHONPATH=src python -m pytest -q`: 1584 passed, 212 subtests passed, with
  the existing sklearn/SciPy L-BFGS-B deprecation warning.
- Full unittest discovery:
  `PYTHONPATH=src python -m unittest discover -s tests -q`: 1539 tests OK,
  with the same existing warning.
- `PYTHONPATH=src python -m compileall -q src tests`: passed.
- `PYTHONPATH=src python -m catalytic_earth.cli validate`: 12 source records,
  8 mechanism fingerprints, 15 mechanism ontology families, and 702 curated
...
- `git diff --check`: passed.
- Normalized regeneration matched after deleting `created_utc`; direct-field
  integrity check found only the eight allowed source-free namespaced
  electron-flow keys in the candidate smoke/full gate rows; critical violation
  total is 0.
- Hash-seed focused stability:
  `PYTHONHASHSEED=1` through `PYTHONHASHSEED=4` new unit/regression/parser slice
  passed.
...
- If import edits are explicitly allowed in a future Lever 2 run, apply exactly
  the candidate 35-row namespaced smoke sidecar delta to the protected approved
  sidecar, rerun the written-sidecar-only smoke gate, then apply the remaining
  39 current-split rows. If import edits remain disallowed, treat the mechanism
  evidence as measured-positive and route the remaining work as an
  approval/policy decision, not a missing source-free electron-flow evidence
  blocker.
