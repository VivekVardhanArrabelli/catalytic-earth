...
Run: 2026-06-13T11:49:12Z

Non-destructive, metadata-only audit of all combined labels (6781 = 702 frozen current702 + 6079 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5085; out_of_scope: 1696; positive:OOS = 2.9982.
- Fingerprint Gini 0.162 / normalized entropy 0.9854; max/min(nonzero) ratio 6.7 (308 vs 46).
...
- Measurable rows: 5925; clusters (size >= 3): 130; rows in clusters: 507 (0.0856 of measurable).
...
| 10 | copper_oxidoreductase | BALANCED | 119 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 11 | cytochrome_p450_monooxygenase | BALANCED | 110 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
