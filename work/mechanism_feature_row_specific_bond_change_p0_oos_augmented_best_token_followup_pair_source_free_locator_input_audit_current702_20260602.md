...
- Blockers: priority1_rows_lack_source_free_contact_or_coordinate_anchor, source_free_coordinate_anchor_candidates_need_review, source_free_event_axis_missing_for_pair_token
...
- The priority-1 queue rows have predicted-geometry coordinates, 102 now expose review-only coordinate-local anchor candidates, but none are approved locator sidecars for scoring.
- Review coordinate-anchor candidates, rewrite only approved sidecars into the audited locator directory, and keep the event-axis blocker separate before any heldout read.
