...
- Scoring tranche rows: 66
- Unique query accessions: 66
- Query coordinate files missing: 2
- Train-atlas target coordinate files missing: 0
- Foldseek result TSV exists: 1
- Foldseek current-query hits: 64
- Foldseek runtime available: True
- Blockers: ['tranche_query_coordinate_files_missing']
...
- Next gate: Materialize the 2 missing tranche query AFDB-v6 CIFs if available; if the Foldseek TSV already has 64 current-query hits, parse those partial scores back into train/cal OOS rows before rerunning the fixed-threshold proxy audit.

## Missing Query Coordinates

| accession | rows | expected path |
| --- | --- | --- |
| P00806 | m_csa:586 | artifacts/v3_predicted_structure_fold_channel_current702_20260601_coordinates/confounded_proxy_train_cal_tranche_queries/afdb_P00806_v6.cif |
| P04531 | m_csa:637 | artifacts/v3_predicted_structure_fold_channel_current702_20260601_coordinates/confounded_proxy_train_cal_tranche_queries/afdb_P04531_v6.cif |
...
- 66 selected train/cal OOS rows are mapped to 66 unique query accessions.
- 2 tranche query coordinate files are missing; 0 threshold-contract train-atlas target files are missing.
