...
Run: 2026-06-12T22:26:10Z

Non-destructive, metadata-only audit of all combined labels (4042 = 702 frozen current702 + 3340 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 2346; out_of_scope: 1696; positive:OOS = 1.3833.
- Fingerprint Gini 0.1578 / normalized entropy 0.9814; max/min(nonzero) ratio 2.66 (308 vs 116).
- Below floor: [].
...
- Measurable rows: 3186; clusters (size >= 3): 82; rows in clusters: 349 (0.1095 of measurable).
...
| 14 | ser_his_acid_hydrolase | BALANCED | 129 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
