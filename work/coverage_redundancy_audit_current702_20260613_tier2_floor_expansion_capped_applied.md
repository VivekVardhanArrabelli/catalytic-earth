...
Run: 2026-06-13T23:32:30Z

Non-destructive, metadata-only audit of all combined labels (7583 = 702 frozen current702 + 6881 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5887; out_of_scope: 1696; positive:OOS = 3.4711.
- Fingerprint Gini 0.1312 / normalized entropy 0.9899; max/min(nonzero) ratio 2.73 (308 vs 113).
- Below floor: [].
...
- Measurable rows: 6727; clusters (size >= 3): 136; rows in clusters: 528 (0.0785 of measurable).
