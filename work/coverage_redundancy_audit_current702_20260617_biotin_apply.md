...
Run: 2026-06-17T19:17:48Z

Non-destructive, metadata-only audit of all combined labels (8769 = 702 frozen current702 + 8067 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 6973; out_of_scope: 1696; positive:OOS = 4.1114.
- Fingerprint Gini 0.1745 / normalized entropy 0.986; max/min(nonzero) ratio 3.08 (308 vs 100).
- Below floor: [].
...
- Measurable rows: 7914; clusters (size >= 3): 153; rows in clusters: 586 (0.074 of measurable).
...
- Reaction-saturated (over reaction-aware cap, rate 8): ['aminoglycoside_phosphotransferase', 'biotin_dependent_carboxylase', 'had_like_phosphatase', 'n_ribosyl_hydrolase', 'ser_thr_protein_phosphatase', 'serine_beta_lactamase'] (surplus 181).
