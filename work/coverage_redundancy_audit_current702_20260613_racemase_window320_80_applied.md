...
Run: 2026-06-13T19:25:50Z

Non-destructive, metadata-only audit of all combined labels (7211 = 702 frozen current702 + 6509 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5515; out_of_scope: 1696; positive:OOS = 3.2518.
- Fingerprint Gini 0.1643 / normalized entropy 0.9855; max/min(nonzero) ratio 6.7 (308 vs 46).
...
| metal_racemase_epimerase_non_plp | 0 | 129 | 129 | 83 | 1.55 |
...
- Measurable rows: 6355; clusters (size >= 3): 133; rows in clusters: 519 (0.0817 of measurable).
...
| 20 | metal_racemase_epimerase_non_plp | BALANCED | 129 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
