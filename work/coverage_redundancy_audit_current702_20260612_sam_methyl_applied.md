...
Run: 2026-06-12T23:27:25Z

Non-destructive, metadata-only audit of all combined labels (4292 = 702 frozen current702 + 3590 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).
...
- Measurable rows: 3436; clusters (size >= 3): 87; rows in clusters: 374 (0.1088 of measurable).
