...
Run: 2026-06-14T00:37:24Z

Non-destructive, metadata-only audit of all combined labels (7742 = 702 frozen current702 + 7040 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 6046; out_of_scope: 1696; positive:OOS = 3.5649.
- Fingerprint Gini 0.1385 / normalized entropy 0.9894; max/min(nonzero) ratio 2.73 (308 vs 113).
- Below floor: [].
...
- Measurable rows: 6886; clusters (size >= 3): 137; rows in clusters: 531 (0.0771 of measurable).
...
| 31 | radical_sam_enzyme | BALANCED | 214 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
