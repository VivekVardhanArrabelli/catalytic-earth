...
The targeted current-fingerprint rescue screen
`artifacts/v3_serine_hydrolase_deep_packet_targeted_current_ser_his_rescue_screen_20260521.json`
then screens the six materialized rows against the 40 current-countable
`ser_his_acid_hydrolase` structures. All six rows have high-TM current-countable
duplicate/leakage signals, so
`artifacts/v3_serine_hydrolase_deep_terminal_decision_packet_after_targeted_ser_his_rescue_screen_20260521.json`
converts those six rows to `terminal_rejection_duplicate_or_leakage` while
leaving only coordinate-missing `P31614` at `needs_new_extractor_or_structure`.
The rescue screen is targeted duplicate/leakage evidence only, does not claim
duplicate-clear for any row, and authorizes no import, scorer, threshold,
registry, or fingerprint change. The follow-up benchmark
`artifacts/v3_serine_hydrolase_deep_packet_post_targeted_ser_his_rescue_modern_baseline_benchmark_20260521.json`
keeps EC/keyword, sequence, geometry, Foldseek, and ESM caveats diagnostic only.
`artifacts/v3_serine_hydrolase_p31614_pdb_replacement_coordinate_screen_20260521.json`
narrows the remaining `P31614` blocker by materializing both frozen PDB
cross-references (`4C7L`, `4C7W`) and screening them against the same 40
current-countable serine-hydrolase targets. Both PDB replacement coordinates
...
therefore keeps six `terminal_rejection_duplicate_or_leakage` rows and one
...
That exact `I2DBY1` blocker is now closed by
`artifacts/v3_heme_peroxidase_deep_packet_i2dby1_full_current_subchunk_screen_20260521.json`.
The follow-up screen runs `I2DBY1` against all 672 staged current-countable
structures in 14 target chunks, maps 672/672 pairs, finds 0 `TM >= 0.7`
current-countable duplicate/leakage hits, and records a maximum TM score of
`0.5890`. The follow-up terminal packet
`artifacts/v3_heme_peroxidase_deep_terminal_decision_packet_after_i2dby1_subchunk_screen_20260521.json`
therefore sets `I2DBY1` to `mechanism_match_review_ready` while preserving six
`terminal_rejection_duplicate_or_leakage` rows. This is review-ready only, not
label import or production scoring; the companion benchmark
`artifacts/v3_heme_peroxidase_deep_packet_post_i2dby1_subchunk_modern_baseline_benchmark_20260521.json`
continues to make no superiority claim and records ESM as unavailable.
...
The first deep packet for this lane is now opened without new broad sourcing.
`artifacts/v3_flavin_monooxygenase_deep_packet_selection_20260521.json` freezes
seven non-exact-reference rows from that existing mini-campaign before deep
geometry/Foldseek scoring, and
`artifacts/v3_flavin_monooxygenase_deep_packet_coordinate_materialization_20260521.json`
materializes all seven AlphaFold sidecars. The targeted current-fingerprint
screen
`artifacts/v3_flavin_monooxygenase_deep_packet_targeted_current_fmo_rescue_screen_20260521.json`
checks the two current-countable `flavin_monooxygenase` structures and finds
three high-TM duplicate/leakage hits. The terminal packet
`artifacts/v3_flavin_monooxygenase_deep_terminal_decision_packet_after_targeted_fmo_rescue_screen_20260521.json`
therefore records three `terminal_rejection_duplicate_or_leakage` rows and four
`needs_new_extractor_or_structure` rows. The exact blocker for the four non-hit
rows is source-free flavin/cofactor geometry mapping plus full-current
...
makes no import, mechanism-match, or superiority claim.
The structure-mapping follow-up
`artifacts/v3_flavin_monooxygenase_deep_packet_structure_mapping_20260521.json`
maps active/cofactor binding positions onto all seven selected AlphaFold
sidecars with status `ok`, using feature positions only and no EC/name/prose as
predictive evidence. The final follow-up terminal packet
`artifacts/v3_flavin_monooxygenase_deep_terminal_decision_packet_after_structure_mapping_20260521.json`
keeps three duplicate/leakage rejections and four
`needs_new_extractor_or_structure` rows, but narrows the remaining exact blocker
to source-free FMO geometry scoring from the mapped flavin/cofactor coordinates
plus full current-countable duplicate/leakage screening. The post-mapping
benchmark
`artifacts/v3_flavin_monooxygenase_deep_packet_post_structure_mapping_modern_baseline_benchmark_20260521.json`
records 7 mapped candidates, 0 geometry-scored candidates, ESM unavailable, and
no import, mechanism-match, production-scoring, or superiority claim.
...
seeds, 0 source-free-axis-ready rows, no calibrated threshold, and no external
hard-negative re-audit under an expanded fingerprint universe.
`artifacts/v3_ghkl_vs_neighbor_family_control_tranche_preregistration_20260520.json`
then freezes the GHKL-vs-neighbor-ATP-family control tranche before scoring:
two GHKL boundary rows, two current hydrolase controls, two ATP-grasp
controls, and one ASKHA, GHMP, NDK, and PfkB countercontrol each.
`artifacts/v3_ghkl_vs_neighbor_family_control_tranche_axis_decisions_20260520.json`
closes the tranche as review-only terminal evidence: two hydrolase controls
remain `mechanism_match`, six neighboring ATP-family rows are `out_of_scope`,
and both GHKL rows are `terminal_rejection` because prior expert review
rejected those current label candidates and the source-free GHKL fold/acceptor
axis is absent. The matching
...
`artifacts/v3_atp_family_readiness_index_post_ghkl_20260520.json` supersedes
that queue after the GHKL tranche: ASKHA, ATP-grasp, and GHKL are now closed
review-only no-go tranches; ePK remains research-lane-only; NDK, PfkB, GHMP,
PfkA, and dNK still need review-only packets before any scoring. The next
bounded ATP-family small win is a review-only dNK packet, not production
activation.
Current dNK readiness scope:
`artifacts/v3_dnk_family_readiness_packet_20260520.json` packages that next
small win as review-only evidence only. It uses two expert-supported
deoxynucleoside kinase boundary rows (`m_csa:588` thymidine kinase and
`m_csa:615` deoxyguanosine kinase), both non-countable expert-rejected
mismatch lanes with current `metal_dependent_hydrolase` top1 collapse below
the calibrated abstention floor. The packet records deoxynucleoside
5-prime-hydroxyl transfer context and a review-only DTP gamma-to-hydroxyl
measurement for `m_csa:615`, but the source-free dNK fold/substrate axis,
complete shared ATP/Mg state, calibrated threshold, external hard-negative
audit, and countable seeds are all absent.
`artifacts/v3_atp_family_readiness_index_post_dnk_packet_20260520.json`
updates the ATP-family queue: dNK is now `readiness_packet_no_go`, not a
production candidate, and the only bounded next dNK step is to freeze a
...
then freezes that tranche before scoring: two dNK boundary rows, two current
hydrolase controls, one NDK control, one PfkA, one PfkB, one GHMP, one ASKHA,
and one ATP-grasp countercontrol. The decision packet
`artifacts/v3_dnk_vs_neighbor_family_control_tranche_axis_decisions_20260520.json`
closes the tranche as review-only terminal evidence: two hydrolase controls
remain `mechanism_match`, six neighboring ATP-family rows are `out_of_scope`,
and both dNK boundary rows are `terminal_rejection` because prior expert review
rejected those current label candidates and the source-free dNK
fold/substrate axis is absent. The no-claim baseline companion is
`artifacts/v3_dnk_vs_neighbor_family_control_tranche_baseline_comparison_20260520.json`;
`artifacts/v3_atp_family_readiness_index_post_dnk_tranche_20260520.json` now
keeps ASKHA, ATP-grasp, GHKL, and dNK closed as review-only no-go tranches.
Current PfkB readiness scope:
`artifacts/v3_pfkb_family_readiness_packet_20260520.json` is review-only and
not a production fingerprint. It packages two expert-supported
PfkB/ribokinase-family boundary rows (`m_csa:663` ribokinase and `m_csa:670`
hydroxymethylpyrimidine kinase), both non-countable mismatch lanes with
current `metal_dependent_hydrolase` top1 collapse and close cross-family
guardrail blockers. The packet records ATP/phosphoryl-transfer text, local
ADP/ribose and structure-level metal context for `m_csa:663`, and the ligand
state gap for `m_csa:670`, but there is still no source-free PfkB fold or
small-molecule hydroxyl-acceptor policy, no calibrated threshold, no external
hard-negative re-audit, and no countable positive seed set.
`artifacts/v3_atp_family_readiness_index_post_pfkb_packet_20260520.json`
updates the queue: PfkB is `readiness_packet_no_go`; the only bounded next
PfkB step is a frozen PfkB-vs-neighbor-ATP-family control tranche before any
scoring.
...
is the current post-ATP-family rollup for this automation run. It keeps the
late ePK synthesis research-lane-only, preserves the glycosyltransferase
terminal uncovered-lane rejection after the sequence baseline diagnostic,
records GHKL and dNK as closed review-only no-go tranches, and records PfkB as
a review-only packet no-go with only a future frozen tranche allowed. It makes
no production, import, superiority, registry, fingerprint, artifact migration,
upload, or removal claim.
...
- curated seed mechanism labels for 682 countable entries: 679 accepted M-CSA
...
- geometry-aware features are now available for 20-, 30-, 40-, 50-, 60-, 75-,
  ...
  pairwise geometry
  records
- curated labels now cover 682 entries, with 212 local active-site
  seed-fingerprint positives in the 1000-entry geometry evaluation and 470 total
...
- next bottleneck is external-source transfer, not another M-CSA-only tranche.
  ...
  structures after multi-model target names were mapped back to their selected
  structures. The cache is now complete at 4704/4704 unique query-target pairs.
  All seven sequence-clean rows have high-TM current-countable duplicate
  ...
  5 duplicate-signal rows and initially defer those 3
  no-current-structural-signal rows behind UniRef-wide duplicate screening,
  terminal review, and full factory gates. The bounded external all-vs-all
  sequence screen for the same 8 rows
  ...
  `P06744` as a no-current-structural-signal deferred row. Follow-on `P06744`
  checks clear bounded duplicate evidence, targeted UniRef nearest-reference
