...
Run: 2026-06-15T22:45:13Z

Non-destructive, metadata-only audit of all combined labels (8522 = 702 frozen current702 + 7820 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 6726; out_of_scope: 1696; positive:OOS = 3.9658.
- Fingerprint Gini 0.1944 / normalized entropy 0.9857; max/min(nonzero) ratio 3.08 (308 vs 100).
- Below floor: ['metal_independent_phosphodiesterase'].
- Above cap: ['metal_dependent_hydrolase'].
- Absent from expansion (holes): ['metal_independent_phosphodiesterase'].
...
- Measurable rows: 7667; clusters (size >= 3): 148; rows in clusters: 571 (0.0745 of measurable).
...
- Holes: ['metal_independent_phosphodiesterase'].
- Under floor: [].
- Over cap: ['metal_dependent_hydrolase'].
- Reaction-saturated (over reaction-aware cap, rate 8): ['aminoglycoside_phosphotransferase', 'had_like_phosphatase', 'n_ribosyl_hydrolase', 'ser_thr_protein_phosphatase'] (surplus 134).
- Total deficit to floor (next-batch positive target): 100.

| # | fingerprint | status | combined | deficit | surplus | action |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | metal_independent_phosphodiesterase | HOLE | 0 | 100 | 0 | SOURCE ~100 more seed labels to reach the 100 floor |
...
| 4 | alpha_beta_hydrolase_esterase_lipase | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 5 | aminoglycoside_phosphotransferase | BALANCED | 150 | 0 | 0 | TRIM -- 50 over the reaction-aware cap 100 (1 distinct reactions x rate 8); demote redundant orthologs toward reaction diversity, do not source more |
