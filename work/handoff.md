...
- STARTED_AT: `2026-06-01T12:37:46Z`
- STARTED_LOCAL: `Mon Jun  1 07:37:46 CDT 2026`
- ENDED_AT: `2026-06-01T13:28:35Z`
- ENDED_LOCAL: `Mon Jun  1 08:28:35 CDT 2026`
- ELAPSED_MINUTES: `50.8`
- Lock acquire result:
  `{"acquired": true, "lock_dir": ".git/catalytic-earth-automation.lock", "pid": "63191", "started_at": "2026-06-01T12:37:46Z", "status": "acquired"}`
...
- Git HEAD at start: `f3f8c61fcfd19318cdf096fc55eb60482ca574f8`
- Dirty files at start: none
- Git branch at end before commit: `main`
- Git HEAD at end before commit: `f3f8c61fcfd19318cdf096fc55eb60482ca574f8`
- Dirty files at end before staging:
  tracked docs/code/test/artifact/report updates plus new locator candidate,
  integrity, review-queue, manual-review-packet, feature-contract, and
  docs-reference-check artifacts; no unrelated dirty files observed.
- Input artifacts:
  ...
  mechanism-feature sidecars and split manifests, current docs, and runbook
  handoff inputs.
- Output artifacts:
  ...
  10 candidate sidecars under
  `artifacts/family_panel_source_free_active_site_locator_candidates_current702_20260601/`,
  ...
  refreshed fold/novelty/sidecar-manifest audit artifacts and updated
  `docs/project_state.md`, `docs/decision_log.md`, `docs/artifact_index.md`,
  `src/catalytic_earth/cli.py`, `src/catalytic_earth/northstar_next_levers.py`,
  `src/catalytic_earth/doc_reference_check.py`, and tests.
- Commands run:
  lock acquire; `git fetch origin`; `git pull --ff-only origin main`; required
  handoff/runbook/project-state/decision-log/artifact-index reads;
  ...
  `build-current-docs-artifact-reference-check`; repo-wide JSON parsing;
  disk checks; `compileall`; `git diff --check`; focused pytest; full pytest;
  full unittest; exact wall-clock ledger checks.
- Validation results:
  predicted-structure fold-channel contract audit passed with 0 critical
  violations; source-free locator schema audit still has 0 scoring-ready rows;
  sidecar manifest still blocks 10/10 rows on approved locator sidecars;
  predicted-atlas novelty variants refreshed with best AUC `0.776461`;
  operating grid regenerated with 40 rows; current-doc reference check found
  0 missing references; `PYTHONPATH=src python -m compileall -q src/catalytic_earth`
  ...
  repo-wide artifact JSON parse passed for 3088 JSON files; focused
  `tests/test_northstar_next_levers.py tests/test_geometry_artifact_regression.py`
  passed 101 tests plus 7 subtests; `tests/test_doc_reference_check.py`,
  `tests/test_northstar_next_levers.py`, and
  `tests/test_geometry_artifact_regression.py` passed 99 tests plus 7
  subtests before the final packet additions; full
  `PYTHONPATH=src python -m pytest tests -q` passed 1067 tests and 26 subtests
  in the final run with one existing sklearn deprecation warning; full
  `PYTHONPATH=src python -m unittest discover -s tests` passed 1044 tests.
...
- Exact next action for the next run:
  start from
  `artifacts/v3_family_panel_source_free_active_site_locator_manual_review_packet_current702_20260601.json`
  and manually review the priority-1 rows (`mh_066`, `mh_073`, and
  `secondary_probe::radical_sam_enzyme`) for forbidden-feature cleanliness and
  scientific locator validity. Only after approval should rewritten locator
  sidecars be copied into
  `artifacts/family_panel_source_free_active_site_locators_current702_20260601/`
  and schema-audited before any predicted-geometry scoring.

Summary: the requested real predicted-structure fold channel was already
present and contract-clean, so this run advanced the exact downstream blocker:
staged coordinate-only locator candidates for the 10 fold-scored family-panel
rows, audited their file/guardrail integrity, ranked the manual review queue,
and produced the manual review packet. It also added a no-fit
mechanism-feature feature contract and a current-doc artifact-reference check.
All outputs remain review-only: no labels, registries, ontologies, heldout
splits, thresholds, imports, or production scoring paths changed.
...
- STARTED_AT: `2026-06-01T09:35:11Z`
- STARTED_LOCAL: `Mon Jun  1 04:35:11 CDT 2026`
- ENDED_AT: `2026-06-01T10:25:08Z`
- ENDED_LOCAL: `Mon Jun  1 05:25:08 CDT 2026`
- ELAPSED_MINUTES: `50.0`
- Lock acquire result:
  `{"acquired": true, "lock_dir": ".git/catalytic-earth-automation.lock", "pid": "52579", "started_at": "2026-06-01T09:35:11Z", "status": "acquired"}`
- Lock release result: pending at handoff write; release command to run after
  clean synced push:
  `PYTHONPATH=src python -m catalytic_earth.cli automation-lock --lock-dir .git/catalytic-earth-automation.lock --repo-root "$PWD" release --require-clean --require-no-merge --require-synced`
- Git branch at start: `main`
- Git HEAD at start: `3200aea9ebf7a4c775c300e96d418f2c638a9413`
- Dirty files at start: none
- Git branch at end before commit: `main`
- Git HEAD at end before commit: `3200aea9ebf7a4c775c300e96d418f2c638a9413`
- Dirty files at end before staging:
  23 tracked files plus new artifacts/reports under `artifacts/` and `work/`;
  see `git status --short` in the run log. No unrelated dirty files were present.
- Input artifacts:
  `artifacts/v3_fold_level_novelty_signal_current702_20260601.json`,
  `artifacts/v3_predicted_geometry_in_distribution_atlas_retrieval_current702_20260601.json`,
  `artifacts/v3_predicted_structure_fold_channel_current702_20260601.json`,
  ...
  `artifacts/v3_fold_augmented_family_panel_missing_primary_channel_queue_current702_20260601.json`,
  `artifacts/v3_fold_augmented_family_panel_missing_primary_channel_diagnosis_current702_20260601.json`,
  family-panel packets/readouts, current702 label manifest, graph, geometry
  features, organic cofactor sidecars, mechanism-feature sidecars/schema audits,
  external identifier/materialization/glycoside/cobalamin/radical-SAM source
  artifacts, and prior FMO/source-check artifacts referenced in docs.
- Output artifacts:
  ...
  `artifacts/v3_predicted_structure_fold_channel_current702_20260601_coordinates_foldseek_results/family_panel_m_csa_repair_vs_atlas.tsv`,
  refreshed FMO and near-orphan family packets, refreshed family-panel coverage
  audit/readout/source-check queue/missing queue/diagnosis,
  ...
  refreshed `artifacts/v3_fmo_subtype_hard_negative_packet_current702_20260601.json`,
  `artifacts/v3_family_panel_source_backed_sidecar_materialization_plan_current702_20260601.json`,
  ...
  updated `docs/project_state.md`, `docs/decision_log.md`,
  `docs/artifact_index.md`, `src/catalytic_earth/cli.py`,
  `src/catalytic_earth/northstar_next_levers.py`, and regression tests.
- Commands run:
  lock acquire, `git fetch origin`, `git pull --ff-only origin main`, required
  doc reads, JSON probes with `jq`, real Foldseek exact-TM M-CSA repair search
  against `/private/tmp/catalytic-earth-predicted-structure-fold-channel-current702/atlas_in_distribution`,
  `build-fold-augmented-family-panel-m-csa-primary-channel-repair`, refreshed
  family-panel packet/readout/queue commands, generated static source-check and
  sidecar/materialization/novelty/cofactor-locus artifacts, JSON validation,
  doc artifact-reference check, `compileall`, `git diff --check`, focused pytest
  runs, `tests/test_cli.py`, `tests/test_transfer_scope.py`,
  `tests/test_automation_small_win_artifacts.py`, full `PYTHONPATH=src python -m pytest tests`,
  and the exact wall-clock ledger check.
- Validation results:
  new/changed JSON artifacts parsed; repo-wide `artifacts/*.json` parse check
  passed for 2991 JSON artifacts; `git diff --check` passed;
  `PYTHONPATH=src python -m compileall -q src/catalytic_earth` passed;
  focused `tests/test_geometry_artifact_regression.py` and
  `tests/test_northstar_next_levers.py` passed; `tests/test_cli.py` passed
  117 tests; `tests/test_transfer_scope.py` passed 122 tests;
  `tests/test_automation_small_win_artifacts.py` passed 231 tests; full
  `PYTHONPATH=src python -m pytest tests` passed 1040 tests with 1 upstream
  sklearn deprecation warning.
- Commit/push result: pending at handoff write; commit, push, sync verification,
  and lock release run immediately after this handoff is staged.
- Exact next action for the next run:
  materialize the P0 sidecars/coordinates from
  `artifacts/v3_family_panel_source_backed_sidecar_materialization_plan_current702_20260601.json`
  (`Q59490`, `A0A1M6T2I7`, and `Q6NSJ0` first), record coordinate hashes, run
  Foldseek/TM against the frozen predicted atlas, and refresh only review-only
  family-panel readouts. For mechanism features, repeat the audited sidecar
  pattern for `radical_sam_locus` and `iron_sulfur_locus`.
...
but this section records the state so the next automation run can start from
the right context instead of rediscovering it.

Wall-clock context: 2026-05-31 local evening session. Work type was manual
bundle verification, fast-forward import, focused validation, push, and
automation retarget.
...
- Imported first bundle: `/Users/vivekvardhanarrabelli/Downloads/d11work.bundle`
...
The second bundle contained exactly these expected commits:
...
Validation result after `5af7848`: `23 passed`.

Validation result after `98d803c`: `23 passed`.
...
Its stale t6/t12 blocker prompt was replaced. It now preserves the 55-minute
wall-clock cap, mandatory startup continuity, mandatory wall-clock handoff,
no silent label/ontology/threshold/split mutation, and no heldout leakage.
...
If predicted geometry structures or AF outputs are missing, do not fabricate
scores. Search existing caches/artifacts/worktrees first, then produce an exact
acquisition/job manifest and runnable command. Stop broadening once Priority 1
produces a valid nonzero atlas artifact; the next run should then rerun
deployment abstention and atlas novelty against that artifact.
...
The local branch started clean but three prior sequence-NN commits remained
ahead of `origin/main`:

```text
4e8a03a Record sequence NN gated rerun
...
The rerun was deterministic and produced no diff in the sequence-NN artifacts.
The compliance artifact remains `blocked_before_sequence_nn_metrics` with one
...
Contract SHA-256 retained by both sequence-NN artifacts:
...
Full unit discovery passed 936 tests. Underpowered metric cells were not
