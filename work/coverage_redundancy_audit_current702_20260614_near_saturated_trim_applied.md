...
Run: 2026-06-14T07:14:52Z

Non-destructive, metadata-only audit of all combined labels (7564 = 702 frozen current702 + 6862 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5868; out_of_scope: 1696; positive:OOS = 3.4599.
- Fingerprint Gini 0.1891 / normalized entropy 0.9833; max/min(nonzero) ratio 3.08 (308 vs 100).
- Below floor: [].
...
- Measurable rows: 6709; clusters (size >= 3): 123; rows in clusters: 489 (0.0729 of measurable).
...
- Reaction-saturated (over reaction-aware cap, rate 8): [] (surplus 0).
...
| 7 | cobalamin_radical_rearrangement | BALANCED | 123 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
...
| 29 | pfkb_ribokinase_family | BALANCED | 128 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 30 | plp_dependent_enzyme | BALANCED | 147 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 31 | protein_kinase_ser_thr_tyr | BALANCED | 100 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 32 | radical_sam_enzyme | BALANCED | 185 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 33 | sam_methyltransferase | BALANCED | 250 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 34 | ser_his_acid_hydrolase | BALANCED | 129 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 35 | terpene_cyclase_synthase | BALANCED | 173 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 36 | thiamine_diphosphate_enzyme | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 37 | zinc_lyase_hydratase | BALANCED | 100 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
