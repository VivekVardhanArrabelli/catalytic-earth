...
Run: 2026-06-13T04:32:42Z

Non-destructive, metadata-only audit of all combined labels (5638 = 702 frozen current702 + 4936 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 3942; out_of_scope: 1696; positive:OOS = 2.3243.
- Fingerprint Gini 0.1581 / normalized entropy 0.9862; max/min(nonzero) ratio 2.85 (308 vs 108).
- Below floor: [].
...
- Measurable rows: 4782; clusters (size >= 3): 112; rows in clusters: 453 (0.0947 of measurable).
