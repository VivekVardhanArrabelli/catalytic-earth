...
Run: 2026-06-13T07:42:43Z

Non-destructive, metadata-only audit of all combined labels (5985 = 702 frozen current702 + 5283 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 4289; out_of_scope: 1696; positive:OOS = 2.5289.
- Fingerprint Gini 0.1647 / normalized entropy 0.9856; max/min(nonzero) ratio 3.67 (308 vs 84).
...
- Measurable rows: 5129; clusters (size >= 3): 120; rows in clusters: 477 (0.093 of measurable).
