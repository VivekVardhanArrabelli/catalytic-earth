...
- Lanes queried: 1 (<= 190 rows each).
- Per-lane record window: offset 0, limit None.
- Fetched candidate rows: 190.
- Target mechanism-corroborated bronze labels: 81 (off-target held 0; disambiguation holds 5; skipped 104).
- **Novelty-admitted labels: 72** (throttled/rejected 9; held@cap 0).
- Combined registry 7915 -> **7987** if merged.

## Floor projection

- `protein_kinase_ser_thr_tyr`: 0 -> 72 (added 72; cap 150; floor reached: False; held@cap 0).
...
- Decisions: {'admit': 72, 'throttle': 9}.
- Reasons: {'closes_hole_fingerprint': 26, 'closes_under_floor_fingerprint': 46, 'needed_fingerprint_but_redundant_ortholog': 9}.

## Hold reasons

- {'no_mechanism_corroboration': 5}.
