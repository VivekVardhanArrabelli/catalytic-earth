...
Run: 2026-06-16T12:16:48Z

Non-destructive, metadata-only audit of all combined labels (8628 = 702 frozen current702 + 7926 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 6832; out_of_scope: 1696; positive:OOS = 4.0283.
- Fingerprint Gini 0.1948 / normalized entropy 0.9857; max/min(nonzero) ratio 3.08 (308 vs 100).
...
- Measurable rows: 7773; clusters (size >= 3): 152; rows in clusters: 583 (0.075 of measurable).
