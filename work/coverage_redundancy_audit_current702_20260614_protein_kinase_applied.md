...
Non-destructive, metadata-only audit of all combined labels (8065 = 702 frozen current702 + 7363 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).
...
- Measurable rows: 7209; clusters (size >= 3): 152; rows in clusters: 576 (0.0799 of measurable).
