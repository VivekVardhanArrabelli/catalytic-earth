...
- Verification passed: `jq -c .` on the standardized JSONL, `jq -e .` on the standardized metrics JSON, focused Python assertions for required Wave 1 row fields and counts, `PYTHONPATH=src python -m unittest tests.test_representation_baseline`, and `PYTHONPATH=src python -m catalytic_earth.cli validate`.
