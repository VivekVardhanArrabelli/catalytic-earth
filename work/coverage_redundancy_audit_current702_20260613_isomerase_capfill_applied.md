...
Run: 2026-06-13T21:20:16Z

Non-destructive, metadata-only audit of all combined labels (7240 = 702 frozen current702 + 6538 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5544; out_of_scope: 1696; positive:OOS = 3.2689.
- Fingerprint Gini 0.1611 / normalized entropy 0.9857; max/min(nonzero) ratio 6.7 (308 vs 46).
...
- Measurable rows: 6384; clusters (size >= 3): 133; rows in clusters: 519 (0.0813 of measurable).
