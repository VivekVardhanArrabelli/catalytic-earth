...
Non-destructive, metadata-only audit of all combined labels (7710 = 702 frozen current702 + 7008 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).
...
- Measurable rows: 6855; clusters (size >= 3): 124; rows in clusters: 492 (0.0718 of measurable).
