...
Run: 2026-06-13T05:16:34Z

Non-destructive, metadata-only audit of all combined labels (5788 = 702 frozen current702 + 5086 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 4092; out_of_scope: 1696; positive:OOS = 2.4127.
- Fingerprint Gini 0.1541 / normalized entropy 0.9868; max/min(nonzero) ratio 2.85 (308 vs 108).
- Below floor: [].
...
- Measurable rows: 4932; clusters (size >= 3): 116; rows in clusters: 465 (0.0943 of measurable).
