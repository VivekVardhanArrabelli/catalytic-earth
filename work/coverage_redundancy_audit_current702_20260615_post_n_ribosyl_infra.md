...
Run: 2026-06-15T16:08:06Z

Non-destructive, metadata-only audit of all combined labels (8122 = 702 frozen current702 + 7420 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 6426; out_of_scope: 1696; positive:OOS = 3.7889.
- Fingerprint Gini 0.2005 / normalized entropy 0.9849; max/min(nonzero) ratio 3.08 (308 vs 100).
- Below floor: ['n_ribosyl_hydrolase'].
- Above cap: ['metal_dependent_hydrolase'].
- Absent from expansion (holes): ['n_ribosyl_hydrolase'].
...
| n_ribosyl_hydrolase | 0 | 0 | 0 | 0 | None |

## Near-duplicate / saturation read (metadata proxy)

- Cluster key: (fingerprint_or_scope, full_ec, organism, sequence_length_bin).
- Measurable rows: 7267; clusters (size >= 3): 144; rows in clusters: 557 (0.0766 of measurable).
...
- Holes: ['n_ribosyl_hydrolase'].
- Under floor: [].
- Over cap: ['metal_dependent_hydrolase'].
- Reaction-saturated (over reaction-aware cap, rate 8): ['had_like_phosphatase', 'ser_thr_protein_phosphatase'] (surplus 46).
- Total deficit to floor (next-batch positive target): 100.

| # | fingerprint | status | combined | deficit | surplus | action |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | n_ribosyl_hydrolase | HOLE | 0 | 100 | 0 | SOURCE ~100 more seed labels to reach the 100 floor |
...
| 29 | nad_p_dehydrogenase | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 30 | non_heme_iron_2og_dioxygenase | BALANCED | 250 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 31 | nucleoside_diphosphate_kinase | BALANCED | 100 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 32 | pfka_phosphofructokinase | BALANCED | 100 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 33 | pfkb_ribokinase_family | BALANCED | 128 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 34 | plp_dependent_enzyme | BALANCED | 147 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 35 | protein_kinase_ser_thr_tyr | BALANCED | 100 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 36 | radical_sam_enzyme | BALANCED | 185 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 37 | sam_methyltransferase | BALANCED | 250 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 38 | ser_his_acid_hydrolase | BALANCED | 129 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 39 | ser_thr_protein_phosphatase | BALANCED | 112 | 0 | 0 | TRIM -- 12 over the reaction-aware cap 100 (3 distinct reactions x rate 8); demote redundant orthologs toward reaction diversity, do not source more |
| 40 | terpene_cyclase_synthase | BALANCED | 173 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 41 | thiamine_diphosphate_enzyme | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 42 | zinc_lyase_hydratase | BALANCED | 100 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
