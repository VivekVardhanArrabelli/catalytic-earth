...
Run: 2026-06-13T01:42:15Z

Non-destructive, metadata-only audit of all combined labels (4904 = 702 frozen current702 + 4202 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 3208; out_of_scope: 1696; positive:OOS = 1.8915.
- Fingerprint Gini 0.1613 / normalized entropy 0.9844; max/min(nonzero) ratio 2.8 (308 vs 110).
- Below floor: [].
...
- Measurable rows: 4048; clusters (size >= 3): 103; rows in clusters: 426 (0.1052 of measurable).
...
| 14 | nad_p_dehydrogenase | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 15 | non_heme_iron_2og_dioxygenase | BALANCED | 172 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 16 | plp_dependent_enzyme | BALANCED | 147 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 17 | radical_sam_enzyme | BALANCED | 133 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 18 | sam_methyltransferase | BALANCED | 250 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 19 | ser_his_acid_hydrolase | BALANCED | 129 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
