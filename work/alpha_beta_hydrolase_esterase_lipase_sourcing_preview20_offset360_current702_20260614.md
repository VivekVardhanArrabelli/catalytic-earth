...
- Lanes queried: 3 (<= 400 rows each).
- Per-lane record window: offset 360, limit 20.
- Fetched candidate rows: 40.
- Target mechanism-corroborated bronze labels: 5 (off-target held 0; disambiguation holds 35; skipped 0).
- **Novelty-admitted labels: 5** (throttled/rejected 0; held@cap 0).
- Combined registry 7860 -> **7865** if merged.

## Floor projection

- `alpha_beta_hydrolase_esterase_lipase`: 0 -> 5 (added 5; cap 150; floor reached: False; held@cap 0).
...
- Decisions: {'admit': 5}.
- Reasons: {'closes_hole_fingerprint': 5}.
