...
- Current heuristic shortlist: 8 rows; resolved prior label-quality blockers: m_csa:497, m_csa:750.
- Interpretation: cheap red-flag scan only, not a 210-row manual review.

| Row | Current family | Rule | Signals | Disposition | Action |
| --- | --- | --- | --- | --- | --- |
| m_csa:486 | metal_dependent_hydrolase | metal_dependent_hydrolase_boundary_signal | nucleotide | open_shortlist | review note only |
| m_csa:506 | flavin_dehydrogenase_reductase | flavin_metal_radical_conflict | semiquinone | keep_current_label | review note only |
| m_csa:535 | metal_dependent_hydrolase | metal_dependent_hydrolase_boundary_signal | GTP | open_shortlist | review note only |
| m_csa:714 | heme_peroxidase_oxidase | heme_nonheme_or_mixed_metal_oxidase_signal | copper, CuB, binuclear centre | move_to_secondary_or_future_family_blocker | quarantine from clean canaries until subfamily policy |
| m_csa:735 | heme_peroxidase_oxidase | heme_nonheme_or_mixed_metal_oxidase_signal | copper, CuA, CuB, binuclear centre | move_to_secondary_or_future_family_blocker | quarantine from clean canaries until subfamily policy |
| m_csa:855 | plp_dependent_enzyme | plp_cobalamin_radical_coupling_conflict | Fe-S | keep_current_label | review note only |
...
- Foldseek single-query fallback attempted all 134 heldout unique structures against 538 train unique structures.
- Retained unique heldout/train pairs: 32608/72092; exhaustive pair ceiling closed: `false`.
- Completed/reused query chunks: 134/134; failed chunks: 0.
- Exact/full fast attempts: `failed_sigterm_before_repo_artifact`, `failed_sigterm_before_repo_artifact`; fallback method: `heldout_vs_train_single_query_foldseek_tm_hit_signal`.
...
Do not treat the TM signal as a benchmark win claim. It is structural-neighborhood review evidence for fold-conflict and near-orphan classification only, with exhaustive pair-row closure still blocked by Foldseek SIGTERM on large `-e inf` runs.
