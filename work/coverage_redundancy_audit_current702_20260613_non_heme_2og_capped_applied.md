...
Run: 2026-06-14T00:05:18Z

Non-destructive, metadata-only audit of all combined labels (7661 = 702 frozen current702 + 6959 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 5965; out_of_scope: 1696; positive:OOS = 3.5171.
- Fingerprint Gini 0.137 / normalized entropy 0.9893; max/min(nonzero) ratio 2.73 (308 vs 113).
- Below floor: [].
...
- Measurable rows: 6805; clusters (size >= 3): 137; rows in clusters: 531 (0.078 of measurable).
