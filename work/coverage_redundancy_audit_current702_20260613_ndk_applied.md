...
Run: 2026-06-13T08:03:15Z

Non-destructive, metadata-only audit of all combined labels (6135 = 702 frozen current702 + 5433 expansion bronze). No registry is written and no label is emitted. Redundancy is measured from annotation metadata only (no mmseqs / no embeddings / no network).

## Class balance (seed fingerprints, combined)

- Seed positives: 4439; out_of_scope: 1696; positive:OOS = 2.6173.
- Fingerprint Gini 0.1608 / normalized entropy 0.9862; max/min(nonzero) ratio 3.67 (308 vs 84).
...
- Measurable rows: 5279; clusters (size >= 3): 123; rows in clusters: 486 (0.0921 of measurable).
...
| 22 | nucleoside_diphosphate_kinase | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 23 | plp_dependent_enzyme | BALANCED | 147 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 24 | radical_sam_enzyme | BALANCED | 133 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 25 | sam_methyltransferase | BALANCED | 250 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 26 | ser_his_acid_hydrolase | BALANCED | 129 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 27 | thiamine_diphosphate_enzyme | BALANCED | 150 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
| 28 | zinc_lyase_hydratase | BALANCED | 113 | 0 | 0 | HOLD -- at balance; only add if a new reaction/organism is gained |
