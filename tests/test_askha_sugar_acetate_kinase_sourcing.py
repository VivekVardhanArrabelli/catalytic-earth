"""Offline validation of ASKHA sugar acetate kinase broadened-handle sourcing."""

from __future__ import annotations

import unittest

from catalytic_earth.askha_sugar_acetate_kinase_sourcing import (
    FAMILIES,
    build_askha_sugar_acetate_kinase_sourcing,
)
...
_ROWS = {
    ...
    "AS0002": (
        ["2.7.1.30"],
        "Glycerol kinase",
        "ATP + glycerol = ADP + sn-glycerol 3-phosphate + H(+)",
        "Binding site",
        "ATP",
    ),
    "EC0001": (
        ["2.7.1.1"],
        "Hexokinase-like protein",
        "",
        "Binding site",
        "ATP",
    ),
    "PK0001": (
        ["2.7.1.1", "2.7.11.1"],
        "Hexokinase protein kinase boundary",
        "ATP + D-glucose = ADP + D-glucose 6-phosphate + H(+)",
        "Binding site",
        "ATP",
    ),
    "HK0001": (
        ["2.7.1.1", "2.7.13.3"],
        "Hexokinase histidine kinase boundary",
        "ATP + D-glucose = ADP + D-glucose 6-phosphate + H(+)",
        "Binding site",
        "ATP",
    ),
...
class AskhaSugarAcetateKinaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_askha_sugar_acetate_kinase_sourcing(
    ...
    def test_family_is_askha_sugar_acetate_kinase(self):
        self.assertEqual(FAMILIES, ("askha_sugar_acetate_kinase",))
    ...
    def test_admitted_labels_are_bronze_and_leakage_safe(self):
        ...
        self.assertEqual(
            ...
            {"askha_sugar_acetate_kinase": 2},
        )
        for label in audit["applied_labels"]:
            self.assertEqual(label["fingerprint_id"], "askha_sugar_acetate_kinase")
    ...
    def test_axes_are_mechanism_not_ec(self):
        ...
        self.assertEqual(set(labels), {"uniprot:AS0001", "uniprot:AS0002"})
        axes = labels["uniprot:AS0001"]["evidence"]["source_trust_tier"][
    ...
    def test_boundary_controls_do_not_enter_family(self):
        ...
        for accession in ("EC0001", "PK0001", "HK0001", "GH0001"):
