...
class GeometryArtifactRegressionTests(unittest.TestCase):
    ...
    def test_fold_augmented_p07658_computed_model_repository_broad_probe_counts(
        self,
    ) -> None:
        probe = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_p07658_computed_model_repository_"
        ...
        self.assertEqual(
            probe["status"],
            (
                "fold_augmented_p07658_computed_model_repository_broad_probe_"
                "blocked_no_public_computed_model"
            ),
        )
        self.assertEqual(probe["affected_row"]["entry_id"], "m_csa:562")
        self.assertEqual(probe["affected_row"]["accession"], "P07658")
        self.assertEqual(probe["affected_row"]["sequence_length"], 715)
        self.assertEqual(probe["counts"]["rcsb_computed_model_rows"], 0)
        self.assertEqual(probe["counts"]["rcsb_experimental_control_rows"], 5)
        self.assertEqual(
            probe["counts"]["direct_computed_model_endpoint_404_rows"], 4
        )
        self.assertEqual(
            probe["counts"]["direct_computed_model_coordinate_hits"], 0
        )
        self.assertEqual(
            probe["counts"][
                "deployment_valid_predicted_coordinate_rows_ready_now"
        ...
        self.assertEqual(probe["counts"]["coordinates_downloaded_or_staged"], 0)
        self.assertFalse(
            probe["decision"][
                "broad_public_repository_probe_clears_p07658_now"
            ]
        )
        self.assertFalse(
            probe["decision"]["fixed_threshold_audit_ready_to_rerun_now"]
    ...
    def test_fold_augmented_p07658_full_length_prediction_request_manifest_counts(
        self,
    ) -> None:
        manifest = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_p07658_full_length_prediction_request_"
                "manifest_current702_20260604.json"
            )
        )
        ...
        self.assertEqual(manifest["affected_row"]["entry_id"], "m_csa:562")
        self.assertEqual(manifest["affected_row"]["accession"], "P07658")
        self.assertEqual(manifest["counts"]["sequence_length"], 715)
        self.assertEqual(manifest["counts"]["selenocysteine_count"], 1)
        self.assertEqual(manifest["affected_row"]["selenocysteine_positions"], [140])
        self.assertIn(
            "position(s) 140",
            manifest["prediction_request"]["provider_requirements"][1],
        )
        self.assertEqual(manifest["counts"]["prediction_request_rows"], 1)
        self.assertEqual(manifest["counts"]["coordinates_staged_now"], 0)
        self.assertEqual(manifest["counts"]["rows_scored_now"], 0)
        self.assertEqual(
            manifest["counts"]["fixed_threshold_audit_ready_to_rerun_now"], 0
        )
        self.assertTrue(
        ...
        self.assertFalse(manifest["guardrails"]["coordinates_staged_now"])
        self.assertFalse(manifest["guardrails"]["candidate_rows_scored_now"])
        self.assertFalse(manifest["guardrails"]["threshold_selected_or_tuned"])

    def test_fold_augmented_p07658_prediction_acceptance_preflight_counts(
        self,
    ) -> None:
        preflight = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_p07658_prediction_acceptance_"
                "preflight_current702_20260604.json"
            )
        )
        ...
        self.assertEqual(preflight["affected_row"]["entry_id"], "m_csa:562")
        self.assertEqual(preflight["affected_row"]["accession"], "P07658")
        self.assertEqual(preflight["affected_row"]["selenocysteine_positions"], [140])
        self.assertEqual(preflight["counts"]["candidate_coordinate_exists"], 0)
        self.assertEqual(preflight["counts"]["candidate_provenance_exists"], 0)
        self.assertEqual(preflight["counts"]["acceptance_checks_total"], 8)
        self.assertEqual(preflight["counts"]["acceptance_checks_passed"], 1)
        self.assertEqual(preflight["counts"]["acceptance_checks_failed"], 7)
        self.assertEqual(preflight["counts"]["coordinates_staged_now"], 0)
        self.assertEqual(preflight["counts"]["rows_scored_now"], 0)
        self.assertIn(
            "p07658_candidate_coordinate_file_missing", preflight["blockers"]
        )
        self.assertFalse(
            preflight["decision"]["p07658_acceptance_preflight_passes_now"]
        )
        self.assertFalse(
            preflight["decision"]["p07658_coordinate_blocker_cleared_now"]
        )
        self.assertFalse(
            preflight["decision"]["fixed_threshold_audit_ready_to_rerun_now"]
        )
        self.assertFalse(preflight["guardrails"]["coordinates_staged_now"])
    ...
    def test_fold_augmented_q43088_locator_approval_packet_counts(
        self,
    ) -> None:
        packet = _load_json(
            ROOT
            / "artifacts"
            / "v3_fold_augmented_q43088_locator_approval_packet_current702_20260604.json"
        )
        ...
        self.assertEqual(packet["affected_row"]["entry_id"], "m_csa:604")
        self.assertEqual(packet["affected_row"]["accession"], "Q43088")
        self.assertTrue(packet["affected_row"]["blocker_cleared_now"])
        self.assertEqual(packet["counts"]["reviewed_priority_positions"], 4)
        self.assertEqual(packet["counts"]["approved_locator_positions"], 2)
        self.assertEqual(
            packet["counts"]["active_site_residue_count_after_approval"], 3
        )
        self.assertEqual(
            packet["counts"]["q43088_locator_contract_cleared_now"], 1
        )
        self.assertEqual(packet["counts"]["q43088_ready_for_row_rescore_now"], 1)
        self.assertEqual(
            packet["counts"]["fixed_threshold_audit_ready_to_rerun_now"], 0
        )
    ...
    def test_fold_augmented_q43088_source_free_locator_sidecar_counts(
        self,
    ) -> None:
        sidecar = _load_json(
            ROOT
            / "artifacts"
            / "v3_fold_augmented_q43088_source_free_locator_sidecar_current702_20260604.json"
        )

        self.assertEqual(
            sidecar["status"],
            "q43088_source_free_locator_sidecar_ready_review_only",
        )
        self.assertEqual(sidecar["entry_id"], "m_csa:604")
        self.assertEqual(sidecar["accession"], "Q43088")
        self.assertEqual(sidecar["approved_new_locator_positions"], [288, 286])
        self.assertEqual(len(sidecar["source_free_locators"]), 3)
        self.assertTrue(sidecar["ready_for_predicted_geometry_scoring"])
        self.assertFalse(sidecar["guardrails"]["row_rescored_now"])
        self.assertFalse(sidecar["guardrails"]["threshold_selected_or_tuned"])
        self.assertFalse(
            sidecar["guardrails"]["mechanism_text_or_source_ids_used_as_features"]
        )

    def test_fold_augmented_confounded_proxy_current_evidence_after_q43088_locator_approval_counts(
        self,
    ) -> None:
        packet = _load_json(
            ROOT
            ...
            / (
                "v3_fold_augmented_confounded_proxy_current_evidence_after_"
                "q43088_locator_approval_current702_20260604.json"
            )
        )
        ...
        self.assertEqual(packet["counts"]["staged_predicted_coordinate_rows"], 3)
        self.assertEqual(packet["counts"]["q43088_approved_locator_positions"], 2)
        self.assertEqual(
            packet["counts"]["q43088_locator_contract_cleared_now"], 1
        )
        self.assertEqual(
            packet["counts"]["partial_surface_rescore_input_ready_rows"], 4
        )
        ...
        self.assertEqual(
            packet["counts"]["fixed_threshold_score_rows_ready_to_rerun_now"], 0
        )
        self.assertEqual(
            packet["counts"]["high_cofactor_min_new_abstained_rows_for_80pct"],
            16,
        ...
        self.assertEqual(packet["counts"]["blockers"], 4)
        coordinate_rows = {
            row["entry_id"]: row
            for row in packet["surface_completeness_blocker_rows"][
                "coordinate_source_rows"
            ]
        }
        self.assertEqual(sorted(coordinate_rows), ["m_csa:562"])
        self.assertEqual(coordinate_rows["m_csa:562"]["accession"], "P07658")
        self.assertEqual(
            packet["surface_completeness_blocker_rows"][
                "locator_or_geometry_sidecar_rows"
            ],
            [],
        )
    ...
    def test_fold_augmented_confounded_proxy_surface_and_calibration_state_after_q43088_p07658_counts(
        self,
    ) -> None:
        packet = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_confounded_proxy_surface_and_calibration_"
                "state_after_q43088_p07658_current702_20260604.json"
            )
        )
        ...
        self.assertEqual(
            packet["status"],
            (
                "fold_augmented_confounded_proxy_surface_and_calibration_"
                "state_blocked_p07658_and_train_cal_acquisition"
            ),
        )
        self.assertEqual(packet["counts"]["surface_completeness_blocker_rows"], 1)
        self.assertEqual(packet["counts"]["coordinate_source_blocker_rows"], 1)
        ...
        self.assertEqual(packet["counts"]["p07658_public_computed_model_rows"], 0)
        self.assertEqual(
            packet["counts"]["q43088_locator_contract_cleared_now"], 1
        )
        self.assertEqual(
            packet["counts"]["high_cofactor_candidate_pool_rows_available_now"],
            0,
        )
        self.assertEqual(
            packet["counts"]["high_cofactor_min_new_abstained_rows_for_80pct"],
        ...
        self.assertFalse(packet["decision"]["q43088_requires_more_locator_work_now"])
        self.assertTrue(
            packet["decision"]["p07658_requires_credentialed_or_local_predictor_now"]
        )
        self.assertFalse(
            packet["decision"][
                "high_cofactor_current_candidate_pool_can_close_16_row_shortfall"
            ]
        )
        self.assertFalse(
            packet["decision"]["fixed_threshold_audit_ready_to_rerun_now"]
    ...
    def test_fold_augmented_confounded_proxy_high_cofactor_candidate_near_miss_triage_counts(
        self,
    ) -> None:
        packet = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_confounded_proxy_high_cofactor_candidate_"
                "near_miss_triage_current702_20260604.json"
            )
        )
        ...
        self.assertEqual(packet["counts"]["ready_train_cal_oos_rows"], 353)
        self.assertEqual(packet["counts"]["priority_candidate_rows_available"], 80)
        self.assertEqual(packet["counts"]["near_miss_rows_reported"], 16)
        self.assertEqual(packet["counts"]["high_cofactor_axis_candidate_rows"], 0)
        self.assertEqual(packet["counts"]["high_organic_cofactor_candidate_rows"], 0)
        self.assertEqual(
            packet["counts"]["high_inorganic_cofactor_locus_candidate_rows"], 0
        )
        self.assertEqual(
            packet["counts"]["minimum_new_abstained_rows_for_80pct"], 16
        )
    ...
    def test_fold_augmented_confounded_proxy_high_cofactor_acquisition_blocker_packet_counts(
        self,
    ) -> None:
        packet = _load_json(
            ROOT
        ...
        self.assertEqual(packet["counts"]["priority_candidate_rows_available"], 80)
        self.assertEqual(packet["counts"]["high_cofactor_axis_candidate_rows"], 0)
        self.assertEqual(packet["counts"]["affected_near_miss_rows"], 16)
        self.assertEqual(
            packet["counts"]["minimum_new_abstained_rows_for_80pct"], 16
        )
        self.assertEqual(
            packet["counts"]["eligible_rows_missing_for_minimum"], 16
        )
        self.assertEqual(packet["counts"]["candidate_rows_registered_now"], 0)
        self.assertEqual(packet["counts"]["candidate_rows_scored_now"], 0)
        self.assertEqual(packet["counts"]["blockers"], 4)
        self.assertEqual(packet["affected_rows"][0]["entry_id"], "m_csa:288")
        self.assertFalse(
    ...
    def test_fold_augmented_confounded_proxy_same_family_structural_acquisition_blocker_packet_counts(
        ...
        self.assertEqual(packet["counts"]["ready_train_cal_oos_rows"], 353)
        self.assertEqual(
            packet["counts"]["unscored_ready_train_cal_oos_candidate_rows"],
            170,
        )
        self.assertEqual(packet["counts"]["priority_candidate_rows_available"], 80)
        self.assertEqual(
        ...
        self.assertEqual(
            packet["counts"]["affected_background_candidate_rows"], 80
        )
        self.assertEqual(
            packet["counts"]["minimum_new_abstained_rows_for_80pct"], 170
        )
        self.assertEqual(
        ...
        self.assertEqual(packet["counts"]["candidate_rows_registered_now"], 0)
        self.assertEqual(packet["counts"]["candidate_rows_scored_now"], 0)
        self.assertEqual(packet["counts"]["blockers"], 5)
        self.assertEqual(packet["affected_rows"][0]["entry_id"], "m_csa:288")
        self.assertFalse(
    ...
    def test_fold_augmented_lever3_blocker_packet_guardrail_audit_counts(
        ...
        audit = _load_json(
            ...
            / "artifacts"
            / (
                "v3_fold_augmented_lever3_blocker_packet_guardrail_"
        ...
        self.assertEqual(audit["counts"]["coordinate_stage_artifacts"], 0)
        self.assertEqual(audit["counts"]["fixed_threshold_rerun_artifacts"], 0)
        self.assertEqual(
            audit["counts"]["heldout_training_or_tuning_artifacts"], 0
        )
        self.assertEqual(
            audit["counts"]["label_registry_import_change_artifacts"], 0
        )
        self.assertEqual(
            audit["counts"]["experimental_pdb_metadata_shortcut_artifacts"], 0
        )
        self.assertTrue(
    ...
    def test_fold_augmented_lever3_minimum_next_experiment_queue_counts(
        self,
    ) -> None:
        queue = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_lever3_minimum_next_experiment_"
        ...
        self.assertEqual(queue["counts"]["experiment_steps"], 3)
        self.assertEqual(queue["counts"]["blocked_experiment_steps"], 3)
        self.assertEqual(
            queue["counts"]["p07658_acceptance_checks_failed"], 7
        )
        self.assertEqual(
            queue["counts"]["high_cofactor_eligible_rows_available"], 0
        )
        self.assertEqual(
            queue["counts"]["high_cofactor_rows_missing_for_minimum"], 16
        )
        self.assertEqual(
            queue["counts"]["same_family_structural_eligible_rows_available"],
            0,
        )
        self.assertEqual(
            queue["counts"][
                "same_family_structural_rows_missing_for_minimum"
            ],
            170,
        )
        self.assertEqual(
            queue["counts"]["minimum_new_train_cal_oos_rows_needed"], 186
        )
        self.assertEqual(queue["counts"]["affected_rows_total_reported"], 97)
        self.assertEqual(queue["counts"]["guardrail_violation_artifacts"], 0)
        self.assertEqual(
    ...
    def test_fold_augmented_lever3_current_measured_readout_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_lever3_current_measured_readout_"
                "current702_20260604.json"
            )
        )
        ...
        self.assertEqual(
            readout["status"],
            (
                "fold_augmented_lever3_current_measured_readout_ready_"
        ...
        self.assertEqual(
            readout["counts"]["all_train_cal_oos_retained_at_fixed_threshold"],
            132,
        )
        self.assertEqual(readout["counts"]["calibration_in_scope_retained"], 31)
        self.assertEqual(readout["counts"]["calibration_in_scope_total"], 34)
        self.assertEqual(readout["counts"]["high_cofactor_proxy_rows"], 4)
        self.assertEqual(
            readout["counts"]["high_cofactor_proxy_abstained_at_fixed_threshold"],
            0,
        )
        self.assertEqual(
            readout["counts"]["same_family_structural_proxy_rows"], 59
        )
        self.assertEqual(
            readout["counts"][
                "same_family_structural_proxy_abstained_at_fixed_threshold"
        ...
        self.assertEqual(
            readout["counts"]["p07658_surface_completeness_blocker_rows"], 1
        )
        self.assertEqual(readout["counts"]["high_cofactor_intake_slots_required"], 16)
        self.assertEqual(
            readout["counts"]["same_family_structural_intake_slots_required"],
            170,
        )
        ...
        self.assertFalse(
            readout["decision"]["train_cal_same_family_structural_proxy_target_met"]
        )
        self.assertTrue(readout["guardrails"]["measured_readout_first"])
    ...
    def test_fold_augmented_lever3_evidence_sufficiency_readout_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_fold_augmented_lever3_evidence_sufficiency_readout_"
                "current702_20260604.json"
            )
        )
        ...
        self.assertEqual(
            readout["counts"]["canonical_scored_train_cal_oos_rows"], 204
        )
        self.assertEqual(readout["counts"]["canonical_train_cal_oos_abstained"], 72)
        self.assertEqual(readout["counts"]["calibration_in_scope_retained"], 31)
        self.assertEqual(readout["counts"]["calibration_in_scope_total"], 34)
        self.assertEqual(readout["counts"]["strict_high_cofactor_rows"], 4)
        self.assertEqual(readout["counts"]["strict_high_cofactor_abstained"], 0)
        self.assertEqual(readout["counts"]["near_cofactor_diagnostic_rows"], 16)
        self.assertEqual(readout["counts"]["near_cofactor_diagnostic_abstained"], 8)
        self.assertEqual(
            readout["counts"]["strict_high_cofactor_contract_rows_added"], 0
        )
        self.assertEqual(readout["counts"]["strict_same_family_rows"], 59)
        self.assertEqual(readout["counts"]["strict_same_family_abstained"], 11)
        self.assertEqual(
            readout["counts"]["strict_plus_loose_same_family_diagnostic_rows"],
            80,
        )
        self.assertEqual(
            readout["counts"]["strict_plus_loose_same_family_diagnostic_abstained"],
            26,
        )
        self.assertEqual(readout["counts"]["protein_only_topology_rows"], 8)
        self.assertEqual(readout["counts"]["protein_only_topology_abstained"], 7)
        self.assertEqual(readout["counts"]["protein_only_topology_fold_only_rows"], 8)
        self.assertEqual(
            readout["counts"]["protein_only_topology_fold_only_abstained"], 2
        )
        self.assertEqual(
        ...
        self.assertEqual(readout["counts"]["p07658_api_models_returned"], 0)
        ...
        self.assertTrue(
            readout["decision"][
                "protein_only_topology_residual_promising_but_not_closure"
            ]
        )
    ...
    def test_fold_augmented_lever3_queue_and_template_guardrail_audit_counts(
        ...
        audit = _load_json(
            ...
            / "artifacts"
            / (
                "v3_fold_augmented_lever3_queue_and_template_guardrail_"
                "audit_current702_20260604.json"
            )
        )
    ...
    def test_fold_augmented_lever3_current_run_guardrail_audit_counts(
        self,
    ) -> None:
        audit = _load_json(
            ROOT
            / "artifacts"
            / "v3_fold_augmented_lever3_current_run_guardrail_audit_current702_20260604.json"
        )
        ...
        self.assertEqual(audit["counts"]["scoring_artifacts"], 0)
        self.assertEqual(audit["counts"]["coordinate_stage_artifacts"], 0)
        self.assertEqual(audit["counts"]["heldout_training_or_tuning_artifacts"], 0)
        self.assertEqual(
            audit["counts"]["label_registry_import_change_artifacts"], 0
        )
        self.assertEqual(
            audit["counts"]["mechanism_text_or_source_feature_artifacts"], 0
        )
        self.assertEqual(
            audit["counts"]["experimental_pdb_metadata_shortcut_artifacts"], 0
        )
        self.assertTrue(
            audit["decision"]["current_run_artifacts_guardrail_clean"]
        )
        self.assertFalse(
            audit["decision"]["fixed_threshold_audit_ready_to_rerun_now"]
        )
    ...
    def test_lever2_electron_flow_pqq_donor_acceptor_current_split_feature_sidecar_readout_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
        ...
        self.assertEqual(
            ...
            (
                ...
                "feature_sidecar_readout_research_only_materialized_feature_sidecar_operating_point_signal"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertEqual(
            readout["counts"]["source_free_electron_flow_feature_incomplete_rows"],
            0,
        )
        ...
        self.assertEqual(readout["counts"]["current_primary_retain_recall"], 1.0)
        self.assertEqual(
            readout["counts"]["current_retained_oos_abstain_recall"], 0.025
        )
        self.assertEqual(readout["counts"]["current_geometry_fold_oos_rows"], 75)
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"]["source_donor_acceptor_full_complete_rows"], 74
        )
        self.assertEqual(
            readout["counts"]["projection_row_scout_pqq_positive_rows"], 0
        )
        ...
        self.assertEqual(
            readout["counts"]["primary_safe_non_pqq_family_exclusion_candidates"],
            1,
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["feature_sidecar_contract"]["feature_fields"],
            [
                "has_electron_transfer_event",
                "electron_transfer_count",
        ...
        self.assertEqual(
            full_gate["retained_oos_positive_entry_ids"], ["m_csa:104"]
        )
        positive_rows = [
            row
        ...
        self.assertTrue(
            readout["decision"]["current_split_feature_sidecar_complete"]
        )
        self.assertTrue(
            readout["decision"][
    ...
    def test_lever2_electron_flow_relaxed_non_pqq_donor_acceptor_feature_sidecar_readout_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_relaxed_non_pqq_"
        ...
        self.assertEqual(
            ...
            (
                ...
                "feature_sidecar_readout_research_only_fixed_relaxed_non_pqq_distance_operating_point_signal"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertEqual(readout["counts"]["current_primary_retain_recall"], 1.0)
        self.assertEqual(
            readout["counts"]["current_retained_oos_abstain_recall"], 0.05
        )
        self.assertEqual(readout["counts"]["current_geometry_fold_oos_rows"], 75)
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"]["union_or_gate_oos_abstain_recall"], 0.493333
        )
        self.assertEqual(readout["counts"]["forbidden_row_feature_key_hits"], 0)
        self.assertEqual(readout["counts"]["fixed_distance_cutoff_angstrom"], 8.0)
        self.assertEqual(readout["counts"]["projection_row_scout_rows"], 43)
        self.assertEqual(
            readout["counts"]["projection_row_scout_complete_rows"], 43
        )
        self.assertEqual(
            readout["counts"]["projection_row_scout_positive_rows"], 2
        )
        self.assertEqual(
            readout["counts"]["projection_row_scout_train_positive_rows"], 0
        )
        self.assertEqual(
            readout["counts"]["projection_row_scout_calibration_positive_rows"],
            2,
        )
        self.assertEqual(
        ...
        self.assertEqual(readout["counts"]["family_split_candidates_checked"], 3)
        self.assertEqual(
            readout["counts"][
                "family_split_primary_safe_current_split_candidates_with_retained_oos_signal"
            ],
            2,
        )
        self.assertEqual(
            readout["counts"]["family_split_candidates_with_projection_support"],
            1,
        )
        self.assertEqual(
            readout["counts"][
                "family_split_current_signal_without_projection_support"
            ],
            1,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            readout["feature_sidecar_contract"]["excluded_redox_families"],
            ["flavin", "heme", "pqq"],
        )
        full_gate = readout["measured_readout"][
            "full_retained_oos_current_split_tranche"
        ...
        self.assertEqual(
            [row["entry_id"] for row in positive_rows],
            ["m_csa:119", "m_csa:464"],
        )
        feature_text = json.dumps(
            [row["row_specific_event_features"] for row in readout["feature_rows"]]
        ...
        self.assertEqual(
            projection_scout["positive_entry_ids"], ["m_csa:59", "m_csa:256"]
        )
        self.assertEqual(
            projection_scout["family_positive_row_counts"], {"nad": 2}
        )
        family_split = readout["measured_readout"][
        ...
        self.assertEqual(
            by_candidate["nad_family_only"]["fixed_gate_readout"][
                "retained_oos_positive_entry_ids"
            ],
            ["m_csa:464"],
        )
        self.assertEqual(
            by_candidate["nad_family_only"]["projection_scout"][
                "positive_entry_ids"
            ],
            ["m_csa:59", "m_csa:256"],
        )
        self.assertEqual(
            by_candidate["iron_sulfur_or_iron_only"]["fixed_gate_readout"][
                "retained_oos_positive_entry_ids"
            ],
            ["m_csa:119"],
        )
        self.assertEqual(
            by_candidate["iron_sulfur_or_iron_only"]["projection_scout"][
                "positive_entry_ids"
            ],
            [],
        )
    ...
    def test_lever2_electron_flow_combined_direct_feature_sidecar_readout_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_combined_direct_"
        ...
        self.assertEqual(
            ...
            (
                ...
                "readout_research_only_combined_direct_electron_flow_operating_point_signal"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertEqual(readout["counts"]["current_primary_positive_rows"], 0)
        self.assertEqual(
            readout["counts"]["current_retained_oos_positive_rows"], 3
        )
        self.assertEqual(readout["counts"]["smoke_feature_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_complete_feature_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_primary_positive_rows"], 0)
        self.assertEqual(
            readout["counts"]["smoke_retained_oos_positive_rows"], 1
        )
        ...
        self.assertEqual(readout["counts"]["current_primary_retain_recall"], 1.0)
        self.assertEqual(
            readout["counts"]["current_retained_oos_abstain_recall"], 0.075
        )
        self.assertEqual(readout["counts"]["current_geometry_fold_oos_rows"], 75)
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"]["union_or_gate_oos_abstain_recall"], 0.506667
        )
        self.assertEqual(
            readout["counts"]["pqq_current_retained_oos_positive_rows"], 1
        )
        self.assertEqual(
            readout["counts"]["relaxed_non_pqq_current_retained_oos_positive_rows"],
            2,
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"][
                "projection_backed_pqq_plus_nad_unsupported_relaxed_positive_rows"
            ],
            1,
        )
        self.assertEqual(readout["counts"]["combined_projection_positive_rows"], 2)
        self.assertEqual(
        ...
        self.assertEqual(readout["counts"]["forbidden_row_feature_key_hits"], 0)
        self.assertEqual(
            readout["counts"]["projection_backed_forbidden_row_feature_key_hits"],
            0,
        )
        full_gate = readout["measured_readout"][
            "full_retained_oos_current_split_tranche"
        ...
        self.assertEqual(
            projection_backed["fixed_gate_readout"][
                "retained_oos_positive_entry_ids"
            ],
            ["m_csa:104", "m_csa:464"],
        )
        self.assertEqual(
            projection_backed["unsupported_relaxed_non_pqq_positive_rows"],
            [
                {
                    "entry_id": "m_csa:119",
                    "current_split_role": "current_retained_oos",
                    "unsupported_families": ["iron_sulfur_or_iron"],
                }
            ],
        )
        feature_text = json.dumps(
    ...
    def test_lever2_electron_flow_projection_backed_pqq_nad_feature_sidecar_readout_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_projection_backed_pqq_"
                "nad_feature_sidecar_readout_current702_20260605.json"
            )
        )
        ...
        self.assertEqual(
            ...
            (
                ...
                "feature_sidecar_readout_research_only_projection_backed_pqq_nad_operating_point_signal"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertTrue(
            readout["decision"][
                "projection_backed_pqq_nad_preserves_primary_retention"
            ]
        )
        self.assertTrue(
            readout["decision"][
    ...
    def test_lever2_electron_flow_iron_sulfur_projection_support_readout_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_iron_sulfur_projection_"
                "support_readout_current702_20260605.json"
            )
        )
        ...
        self.assertEqual(
            ...
            (
                ...
                "readout_research_only_iron_sulfur_current_split_signal_tiny_materialization_support_gap"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertEqual(readout["counts"]["projection_rows"], 43)
        self.assertEqual(readout["counts"]["projection_complete_rows"], 43)
        self.assertEqual(readout["counts"]["projection_positive_rows"], 0)
        self.assertEqual(
            readout["counts"]["review_only_locus_rows"], 702
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"]["review_only_nonheldout_rows_scanned"], 562
        )
        self.assertEqual(
            readout["counts"]["review_only_nonheldout_proximal_rows"], 13
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"]["smallest_noncurrent_projection_tranche_rows"], 3
        )
        self.assertTrue(
            readout["counts"]["tiny_projection_materialization_available"]
        )
        ...
        self.assertEqual(readout["counts"]["expanded_projection_candidate_rows"], 12)
        self.assertEqual(readout["counts"]["expanded_projection_complete_rows"], 12)
        self.assertEqual(readout["counts"]["expanded_projection_positive_rows"], 12)
        self.assertEqual(
            readout["counts"]["expanded_projection_positive_entry_ids"],
            [
                "m_csa:108",
                "m_csa:123",
                "m_csa:127",
        ...
        self.assertEqual(gate["retained_oos_positive_entry_ids"], ["m_csa:119"])
        projection = readout["measured_readout"][
            "existing_train_cal_projection_attempt"
        ]
        self.assertEqual(projection["positive_entry_ids"], [])
        support_scan = readout["measured_readout"][
            "review_only_iron_sulfur_locus_support_scan"
        ]
        self.assertEqual(
            support_scan["smallest_noncurrent_projection_tranche_entry_ids"],
            ["m_csa:443", "m_csa:127", "m_csa:281"],
        )
        tiny = readout["measured_readout"][
            "tiny_iron_sulfur_projection_materialization_attempt"
        ]
        self.assertEqual(tiny["positive_rows"], 3)
        self.assertEqual(
            tiny["positive_entry_ids"], ["m_csa:127", "m_csa:281", "m_csa:443"]
        )
        expanded = readout["measured_readout"][
            "expanded_iron_sulfur_projection_materialization_attempt"
        ]
        self.assertEqual(expanded["positive_rows"], 12)
        self.assertFalse(
            readout["decision"][
        ...
        self.assertTrue(
            readout["decision"]["tiny_projection_materialization_attempt_positive"]
        )
        self.assertFalse(
            readout["decision"][
        ...
        self.assertFalse(readout["guardrails"]["heldout_rows_scored_by_this_artifact"])
        self.assertTrue(
            readout["guardrails"]["heldout_rows_excluded_from_support_scan"]
        )
    ...
    def test_lever2_electron_flow_approval_import_smoke_review_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
        ...
        self.assertEqual(
            ...
            (
                ...
                "review_readout_research_only_smoke_review_positive_full_"
                "expansion_positive_pending_protected_import"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertEqual(readout["counts"]["smoke_tranche_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_current_primary_rows"], 34)
        self.assertEqual(readout["counts"]["smoke_retained_oos_rows"], 1)
        self.assertEqual(
            readout["counts"]["smoke_complete_direct_component_rows"], 35
        )
        self.assertEqual(
            readout["counts"]["smoke_current_primary_positive_rows"], 0
        )
        self.assertEqual(
            readout["counts"]["smoke_current_primary_retain_recall"], 1.0
        )
        self.assertEqual(
            readout["counts"]["smoke_retained_oos_positive_entry_ids"],
            ["m_csa:104"],
        ...
        self.assertEqual(readout["counts"]["full_current_split_rows"], 74)
        self.assertEqual(readout["counts"]["full_current_primary_rows"], 34)
        self.assertEqual(readout["counts"]["full_current_retained_oos_rows"], 40)
        self.assertEqual(
            readout["counts"]["full_complete_direct_component_rows"], 74
        )
        self.assertEqual(
            readout["counts"]["full_current_primary_positive_rows"], 0
        )
        self.assertEqual(
            readout["counts"]["full_current_primary_retain_recall"], 1.0
        )
        self.assertEqual(
        ...
        self.assertEqual(readout["counts"]["smoke_rows_new_to_approved_sidecar"], 34)
        self.assertEqual(
            readout["counts"]["smoke_rows_updating_existing_approved_sidecar_rows"],
            1,
        )
        self.assertEqual(readout["counts"]["full_rows_new_to_approved_sidecar"], 73)
        self.assertEqual(
            readout["counts"]["full_rows_updating_existing_approved_sidecar_rows"],
            1,
        )
        self.assertEqual(readout["counts"]["critical_violation_total"], 0)
        self.assertTrue(
    ...
    def test_lever2_electron_flow_approval_import_delta_package_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
        ...
        self.assertEqual(
            ...
            (
                ...
                "readout_research_only_candidate_sidecar_delta_package_ready_"
                "pending_protected_import"
            ),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            readout["counts"]["approved_sidecar_rows_before_delta"], 43
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(readout["counts"]["smoke_delta_field_cell_writes"], 280)
        self.assertEqual(
            readout["counts"]["remaining_current_split_delta_rows"], 39
        )
        self.assertEqual(readout["counts"]["full_delta_rows_after_smoke"], 74)
        self.assertEqual(readout["counts"]["full_delta_new_rows"], 73)
        self.assertEqual(readout["counts"]["full_delta_updated_existing_rows"], 1)
        self.assertEqual(readout["counts"]["full_delta_field_cell_writes"], 592)
        self.assertEqual(readout["counts"]["full_delta_field_conflicts"], 0)
        self.assertEqual(
            readout["counts"]["reconstructed_sidecar_rows_after_smoke_delta"],
            77,
        )
        self.assertEqual(
            readout["counts"]["reconstructed_sidecar_rows_after_full_delta"],
            116,
        )
        self.assertEqual(readout["counts"]["smoke_gate_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_gate_complete_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_gate_primary_rows"], 34)
        ...
        self.assertEqual(
            readout["counts"][
                "smoke_gate_incremental_oos_abstain_recall_vs_current_geometry_fold"
            ],
            0.013333,
        )
        self.assertEqual(
            readout["counts"]["smoke_gate_union_or_gate_oos_abstain_recall"],
            0.48,
        )
        self.assertTrue(readout["counts"]["smoke_gate_matches_candidate_sidecar_gate"])
        self.assertEqual(readout["counts"]["full_gate_rows"], 74)
        self.assertEqual(readout["counts"]["full_gate_complete_rows"], 74)
        self.assertEqual(readout["counts"]["full_gate_primary_rows"], 34)
        ...
        self.assertEqual(readout["counts"]["retained_oos_delta_rows"], 40)
        self.assertEqual(
            readout["counts"]["retained_oos_delta_positive_entry_ids"],
            ["m_csa:104", "m_csa:119", "m_csa:464"],
        )
        self.assertEqual(readout["counts"]["critical_violation_total"], 0)
    ...
    def test_lever2_electron_flow_protected_import_sequence_preflight_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_protected_import_"
        ...
        self.assertEqual(
            readout["status"],
            (
                "lever2_source_free_electron_flow_protected_import_sequence_"
                "preflight_readout_research_only_protected_import_sequence_"
                "preflight_ready"
            ),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            readout["preflight_contract"]["contract_status"],
            "ready_pending_explicit_protected_import_authorization",
        )
        self.assertEqual(
            readout["counts"]["critical_preflight_failure_total"], 0
        )
        self.assertEqual(readout["counts"]["approved_sidecar_rows_current"], 43)
        self.assertEqual(readout["counts"]["smoke_stage_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_stage_complete_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_stage_add_new_rows"], 34)
        self.assertEqual(readout["counts"]["smoke_stage_update_existing_rows"], 1)
        self.assertEqual(readout["counts"]["smoke_stage_primary_positive_rows"], 0)
        self.assertEqual(
            readout["counts"]["smoke_stage_retained_oos_positive_entry_ids"],
            ["m_csa:104"],
        )
        self.assertEqual(readout["counts"]["smoke_stage_primary_retain_recall"], 1.0)
        self.assertEqual(readout["counts"]["remaining_stage_rows_after_smoke"], 39)
        self.assertEqual(
            readout["counts"]["remaining_stage_complete_rows_after_smoke"], 39
        )
        self.assertEqual(readout["counts"]["full_current_split_rows_after_smoke"], 74)
        self.assertEqual(
            readout["counts"]["full_current_split_complete_rows_after_smoke"], 74
        )
        self.assertEqual(readout["counts"]["full_current_split_add_new_rows"], 73)
        self.assertEqual(
            readout["counts"]["full_current_split_update_existing_rows"], 1
        )
        self.assertEqual(
            readout["counts"]["full_current_split_primary_positive_rows"], 0
        )
        ...
        self.assertTrue(
            readout["counts"]["current_sidecar_sha256_matches_contract"]
        )
        self.assertTrue(
            readout["counts"][
                "source_delta_package_file_sha256_matches_contract"
            ]
        )
        self.assertTrue(
            readout["counts"][
                "source_delta_package_normalized_sha256_matches_contract"
            ]
        )
        self.assertTrue(
            readout["counts"]["source_candidate_sha256_matches_contract"]
        )
        self.assertTrue(
            readout["counts"]["smoke_delta_rows_sha256_matches_contract"]
        )
        self.assertTrue(
            readout["counts"][
                "remaining_current_split_delta_rows_sha256_matches_contract"
            ]
        )
        self.assertTrue(
            readout["counts"]["full_current_split_delta_rows_sha256_matches_contract"]
        )
        self.assertEqual(readout["counts"]["protected_imports_executed"], 0)
        self.assertEqual(readout["counts"]["approved_sidecar_rows_written"], 0)
        self.assertTrue(readout["decision"]["protected_import_sequence_ready"])
        self.assertTrue(
            readout["decision"][
                "ready_for_explicit_protected_import_authorization"
            ]
        )
        self.assertTrue(
            readout["decision"][
    ...
    def test_lever2_electron_flow_current_split_row_gate_audit_current_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_current_split_row_gate_"
                "audit_readout_current702_20260606.json"
            )
        )
        ...
        self.assertEqual(
            ...
            (
                ...
                "readout_research_only_current_split_row_gate_audit_"
        ...
        self.assertEqual(readout["counts"]["smoke_complete_rows"], 35)
        self.assertEqual(readout["counts"]["smoke_primary_rows"], 34)
        self.assertEqual(readout["counts"]["smoke_primary_positive_rows"], 0)
        self.assertEqual(readout["counts"]["smoke_primary_retain_recall"], 1.0)
        self.assertEqual(
        ...
        self.assertEqual(readout["counts"]["remaining_rows_after_smoke"], 39)
        self.assertEqual(
            readout["counts"]["remaining_complete_rows_after_smoke"], 39
        )
        self.assertEqual(
            readout["counts"]["remaining_retained_oos_positive_entry_ids"],
            ["m_csa:119", "m_csa:464"],
        )
        self.assertEqual(readout["counts"]["full_current_split_rows"], 74)
        self.assertEqual(readout["counts"]["full_current_split_complete_rows"], 74)
        self.assertEqual(readout["counts"]["full_current_split_primary_rows"], 34)
        self.assertEqual(
            readout["counts"]["full_current_split_primary_positive_rows"], 0
        )
        ...
        self.assertEqual(
            readout["counts"]["pqq_positive_retained_oos_entry_ids"],
            ["m_csa:104"],
        )
        self.assertEqual(
            readout["counts"]["nad_family_positive_retained_oos_entry_ids"],
            ["m_csa:464"],
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(readout["counts"]["field_conflict_rows"], 0)
        self.assertTrue(
            readout["counts"]["current_sidecar_sha256_matches_preflight"]
        )
        self.assertEqual(readout["counts"]["protected_imports_executed"], 0)
        self.assertEqual(readout["counts"]["approved_sidecar_rows_written"], 0)
        self.assertTrue(
        ...
        self.assertTrue(readout["decision"]["smoke_gate_preserves_primary_retention"])
        self.assertTrue(readout["decision"]["full_gate_preserves_primary_retention"])
        self.assertTrue(
            readout["decision"][
                "full_gate_adds_operating_point_value_beyond_current_geometry_fold"
            ]
        ...
        self.assertTrue(
            readout["decision"]["direct_field_counts_component_consistent"]
        )
        self.assertFalse(readout["decision"]["approved_sidecar_written"])
        self.assertFalse(readout["decision"]["protected_surfaces_modified"])
        self.assertFalse(readout["decision"]["deployable_now"])
    ...
    def test_lever2_electron_flow_current_split_operating_point_current_counts(
        ...
        readout = _load_json(
            ...
            / "artifacts"
            / (
                "v3_lever2_source_free_electron_flow_current_split_operating_"
                "point_readout_current702_20260606.json"
            )
        )
        ...
        self.assertEqual(
            ...
            (
                ...
                "point_readout_research_only_direct_source_free_operating_"
                "point_signal_pending_protected_import"
            ),
        )
        self.assertEqual(
            readout["result_class"],
        ...
        self.assertTrue(
            readout["counts"]["component_operating_point_variants_primary_safe"]
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertTrue(
            readout["decision"][
                "direct_source_free_electron_flow_operating_point_measured"
            ]
        )
    ...
    def test_lever2_event_axis_primary_safe_frontier_readout_counts(
        self,
    ) -> None:
        readout = _load_json(
            ROOT
            / "artifacts"
            / (
                "v3_lever2_event_axis_primary_safe_frontier_readout_"
                "current702_20260604.json"
            )
        )
        ...
        self.assertEqual(
            readout["status"],
            "lever2_event_axis_primary_safe_frontier_readout_"
        ...
        self.assertEqual(
            readout["counts"][
                "primary_control_passing_projection_plus_axis_surfaces"
            ],
            0,
        )
        self.assertEqual(readout["counts"]["calibration_rows"], 32)
        ...
        self.assertEqual(
            readout["counts"]["best_marginal_axis_current_retained_oos_catches"],
            7,
        )
        self.assertEqual(
            readout["counts"][
        ...
        self.assertEqual(
            readout["counts"]["best_marginal_axis_primary_loo_control_rows"],
            4,
        )
        self.assertEqual(
        ...
        self.assertIsNone(readout["decision"]["best_primary_safe_axis_id"])
        self.assertEqual(
            readout["measured_readout"]["best_marginal_axis_before_primary_control"][
        ...
        self.assertEqual([row["entry_id"] for row in control_rows], ["m_csa:133"])
        self.assertEqual(control_rows[0]["baseline_axis_score"], 3.0)
        self.assertEqual(control_rows[0]["added_axis_score"], 3.0)
        self.assertEqual(
            control_rows[0]["selected_pair_rule"]["baseline_rule"]["direction"],
            "high",
        )
        self.assertEqual(
            control_rows[0]["selected_pair_rule"]["baseline_rule"]["threshold"],
            3.0,
        )
        self.assertEqual(
            control_rows[0]["selected_pair_rule"]["added_rule"]["direction"],
            "low",
        )
        self.assertEqual(
            control_rows[0]["selected_pair_rule"]["added_rule"]["threshold"],
            3.0,
        )
        sensitivity = readout["measured_readout"][
            "primary_retain_floor_sensitivity"
        ]
        self.assertEqual(
            [row["min_primary_retain"] for row in sensitivity],
            [1.0, 0.9, 0.75],
        )
        self.assertEqual(
            [
                row["best_primary_safe_axis_marginal_current_retained_oos_catches"]
                for row in sensitivity
            ],
            [0, 0, 1],
        )
        self.assertEqual(
            sensitivity[2]["best_primary_safe_axis_id"],
            "source_free_projected_proton_role_subset+electron_flow",
        )
        self.assertEqual(
            sensitivity[2]["best_primary_safe_axis_marginal_caught_entry_ids"],
            ["m_csa:256"],
        )
        self.assertTrue(
            readout["decision"][
        ...
        self.assertFalse(
            readout["decision"]["best_marginal_axis_primary_loo_control_passes"]
        )
        self.assertTrue(
            readout["decision"][
