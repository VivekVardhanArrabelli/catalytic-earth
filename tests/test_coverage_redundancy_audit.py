...
class WriteAuditRealRegistryTests(unittest.TestCase):
    def test_writes_artifact_and_report_without_touching_registries(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(written["totals"]["combined"], 5982)
            self.assertEqual(written["totals"]["frozen_current702"], 702)
            self.assertEqual(written["totals"]["expansion_bronze"], 5280)
            ...
            # aldolase, ThDP enzyme, zinc lyase/hydratase, and a guarded
            # biotin-dependent carboxylase tranche. Biotin carboxylase is below
            # floor but no longer absent; metal_dependent_hydrolase remains the
            # known intentional over-cap.
