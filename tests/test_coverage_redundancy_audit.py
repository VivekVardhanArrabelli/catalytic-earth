...
class WriteAuditRealRegistryTests(unittest.TestCase):
    def test_writes_artifact_and_report_without_touching_registries(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            # + 150 cysteine-protease + 150 flavin-disulfide-reductase + 74 dihydrofolate-reductase
            # (eleven new fingerprint families) = 1273 broadened-handle bronze rows.
            self.assertEqual(written["totals"]["combined"], 10001)
            self.assertEqual(written["totals"]["frozen_current702"], 702)
            self.assertEqual(written["totals"]["expansion_bronze"], 9299)
