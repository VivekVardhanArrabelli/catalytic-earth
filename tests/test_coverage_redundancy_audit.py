...
class WriteAuditRealRegistryTests(unittest.TestCase):
    def test_writes_artifact_and_report_without_touching_registries(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(written["totals"]["combined"], 4574)
            self.assertEqual(written["totals"]["frozen_current702"], 702)
            self.assertEqual(written["totals"]["expansion_bronze"], 3872)
            ...
            # The 2026-06-12 broadened-handle expansion applied NAD(P)-dehydrogenase,
            # glycosyltransferase, SAM methyltransferase, cytochrome P450, and
            # non-heme iron 2OG dioxygenase to their floors/caps, so no expansion
            # holes remain. metal_dependent_hydrolase remains the known intentional
            # over-cap.
