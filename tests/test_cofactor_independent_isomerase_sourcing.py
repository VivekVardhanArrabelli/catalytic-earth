"""Offline validation of cofactor-independent isomerase broadened-handle sourcing."""
...
from catalytic_earth.cofactor_independent_isomerase_sourcing import (
    FAMILIES,
    build_cofactor_independent_isomerase_sourcing,
)
...
_ROWS = {
    "CI0001": (
        ...
        "catalytic Glu active-site base",
        "Active site",
    ),
    "CI0002": (
        ["5.3.3.8"],
        ["Isomerase"],
        "cholestenol = cholestenone",
        "",
        "",
    ),
    # EC 5.3 + Isomerase but no Rhea isomerization or active-site handle: EC/keyword alone holds.
    "NX0001": (
        ["5.3.1.9"],
        ["Isomerase"],
        "a substrate becomes a product",
        "",
        "",
    ),
    # Multi-EC boundary row from the scout: explicit non-5.3 side-EC guard must hold.
    "OX0001": (
        ["5.3.3.8", "1.1.1.1"],
        ["Isomerase"],
        "cholestenol = cholestenone",
    ...
    # Off-target row should be held by the isomerase wrapper, not imported through this lane.
    "GT0001": (
        ["2.4.1.1"],
        ["Glycosyltransferase"],
...
class CofactorIndependentIsomeraseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_cofactor_independent_isomerase_sourcing(
    ...
    def test_family_is_cofactor_independent_isomerase(self):
        self.assertEqual(FAMILIES, ("cofactor_independent_isomerase",))
    ...
    def test_admitted_labels_are_bronze_and_leakage_safe(self):
        ...
        self.assertEqual(
            ...
            {"cofactor_independent_isomerase": 2},
        )
        for label in audit["applied_labels"]:
            self.assertEqual(label["fingerprint_id"], "cofactor_independent_isomerase")
    ...
    def test_isomerase_axes_are_mechanism_not_ec(self):
        audit = self._run()
        labels = {label["entry_id"]: label for label in audit["applied_labels"]}
        self.assertEqual(set(labels), {"uniprot:CI0001", "uniprot:CI0002"})
        axes = labels["uniprot:CI0001"]["evidence"]["source_trust_tier"][
    ...
    def test_ec_only_and_side_ec_controls_do_not_enter_family(self):
        ...
        self.assertNotIn("uniprot:OX0001", admitted_ids)

    def test_sequence_provenance_and_floor_projection(self):
        audit = self._run()
        proj = audit["floor_projection"]["cofactor_independent_isomerase"]
        self.assertEqual(
            proj["deploy_missing_active_site_context"],
            "active_site_base_isomerization_context",
        )
    ...
    def test_guardrails_non_destructive(self):
        ...
        self.assertTrue(g["non_5_3_side_ec_guard"])
