"""Offline validation of acid--CoA-ligase / acyl-CoA-synthetase sourcing."""
...
from catalytic_earth.acid_coa_ligase_sourcing import (
    FAMILIES,
    build_acid_coa_ligase_sourcing,
)
...
_ROWS = {
    "ACS001": (
        ...
        "a long-chain fatty acid + ATP + CoA = a long-chain acyl-CoA + AMP + diphosphate",
        "Binding site",
        "AMP-binding / adenylation site",
    ),
    "ACS002": (
        ["6.2.1.1"],
        "Acetyl-coenzyme A synthetase",
        ["Ligase"],
        "acetate + ATP + CoA = acetyl-CoA + AMP + diphosphate + H(+)",
        "Active site",
        "ANL-superfamily adenylation active site",
    ),
    # acyl-CoA-synthetase name only, no acid--CoA reaction: held (reaction is the hard anchor).
    "EC0001": (
        ["6.2.1.3"],
    ...
    # ADP-forming succinate--CoA ligase (EC 6.2.1.5): a CoA ligase by name, but the reaction
    # releases ADP + phosphate, NOT AMP (substrate-level phosphorylation, not an acyl-adenylate),
    # so it is NOT an ANL acid--CoA ligase and must be held.
    "SUCC001": (
        ["6.2.1.5"],
        ...
        "Binding site",
        "ATP-grasp nucleotide-binding site",
    ),
    # CoA transferase / acyltransferase (EC 2.3.1, no AMP release): off-target to coa_acyltransferase.
    "COAT001": (
        ["2.3.1.7"],
...
_QUERY_ORDER = ("EC0001", "ACS001", "ACS002", "SUCC001", "COAT001", "GLN001")
...
class AcidCoaLigaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_acid_coa_ligase_sourcing(
    ...
    def test_family_is_acid_coa_ligase(self):
        self.assertEqual(FAMILIES, ("acid_coa_ligase",))

    def test_fetches_and_routes_target_family(self):
        audit = self._run()
        self.assertEqual(audit["counts"]["fetched_candidate_rows"], 6)
        # Only the two AMP-forming ANL acyl-CoA synthetases corroborate.
        ...
        # EC0001 (no reaction) and SUCC001 (ADP-forming, no AMP) are held.
        self.assertGreaterEqual(audit["counts"]["disambiguation_hold_count"], 2)
        # CoA transferase and glutamine synthetase route off-target (coa_acyltransferase / atp_amide).
    ...
    def test_adp_forming_succinyl_coa_ligase_is_held(self):
        # SUCC001 has a CoA-ligase name and EC 6.2.1 scope, but its reaction releases ADP, not
        # AMP, so the acid--CoA acyl-adenylate reaction anchor is absent and the row must not be
        # admitted to acid_coa_ligase (mechanism-not-name discipline).
        audit = self._run()
        admitted_ids = {label["entry_id"].split(":", 1)[1] for label in audit["applied_labels"]}
        self.assertNotIn("SUCC001", admitted_ids)
        off_target_ids = {s["entry_id"].split(":", 1)[1] for s in audit["off_target_fingerprint_matches_sample"]}
        self.assertNotIn("SUCC001", off_target_ids)

    def test_admitted_labels_are_bronze_and_leakage_safe(self):
        audit = self._run()
        for label in audit["applied_labels"]:
            self.assertEqual(label["fingerprint_id"], "acid_coa_ligase")
    ...
    def test_floor_projection_and_deploy_context(self):
        audit = self._run()
        proj = audit["floor_projection"]["acid_coa_ligase"]
        self.assertEqual(
            proj["deploy_missing_active_site_context"],
            "anl_atp_acyl_adenylate_coa_thioester_ligation_context",
        )
    ...
    def test_guardrails_non_destructive(self):
        ...
        self.assertTrue(g["acid_coa_ligase_handles_scope_admission_only"])
        self.assertTrue(g["ec_never_a_counted_corroborator"])
        self.assertTrue(g["coa_transferase_carboxylase_thiolase_dehydrogenase_boundary_guard"])
