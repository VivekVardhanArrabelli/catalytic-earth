"""Offline validation of ATP amide ligase broadened-handle sourcing."""

from __future__ import annotations

import unittest

from catalytic_earth.atp_amide_ligase_sourcing import (
    FAMILIES,
    build_atp_amide_ligase_sourcing,
)
...
_ROWS = {
    ...
    "AL0002": (
        ...
        "Active site",
        "amide ligase catalytic base",
    ),
    # EC + Ligase only: hold because EC/keyword alone is insufficient.
    "NX0001": (
        ["6.3.2.4"],
        ["Ligase"],
        [],
    ...
    # Biotin carboxylases are an explicit boundary lane.
    "BT0001": (
        ["6.3.4.14"],
        ["Ligase", "Biotin"],
        ["biotin", "Mg(2+)"],
        "Biotin carboxylase",
        "ATP + hydrogencarbonate + protein-bound biotin = ADP + phosphate + carboxybiotin",
        "Binding site",
        "biotin binding",
    ),
    # Kinase/transferase and hydrolase side rows are boundary rows.
    "KN0001": (
        ["6.3.2.4", "2.7.1.1"],
        ["Ligase", "Kinase"],
        ["Mg(2+)"],
        "Dual-function ligase kinase",
        "ATP + amino acid = ADP + phosphate + product",
        "Binding site",
        "ATP binding",
    ),
    "HD0001": (
        ...
        ["Ligase"],
        ["Mg(2+)"],
        "Dual-function ligase hydrolase",
        "ATP + carboxylate + amine = ADP + phosphate + amide",
        "Binding site",
        "ATP binding",
    ),
}
...
class AtpAmideLigaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_atp_amide_ligase_sourcing(
    ...
    def test_family_is_atp_amide_ligase(self):
        self.assertEqual(FAMILIES, ("atp_amide_ligase",))
    ...
    def test_admitted_labels_are_bronze_and_leakage_safe(self):
        ...
        for label in audit["applied_labels"]:
            self.assertEqual(label["fingerprint_id"], "atp_amide_ligase")
    ...
    def test_axes_are_mechanism_not_ec(self):
        ...
        self.assertEqual(set(labels), {"uniprot:AL0001", "uniprot:AL0002"})
        axes = labels["uniprot:AL0001"]["evidence"]["source_trust_tier"][
    ...
    def test_ec_only_biotin_kinase_and_hydrolase_controls_do_not_enter_family(self):
        audit = self._run()
        admitted_ids = {label["entry_id"] for label in audit["applied_labels"]}
        for accession in ("NX0001", "BT0001", "KN0001", "HD0001"):
            self.assertNotIn(f"uniprot:{accession}", admitted_ids)

    def test_sequence_provenance_and_floor_projection(self):
        audit = self._run()
        proj = audit["floor_projection"]["atp_amide_ligase"]
        self.assertEqual(
            proj["deploy_missing_active_site_context"],
            "atp_mg_acyl_phosphate_amide_ligation_context",
        )
