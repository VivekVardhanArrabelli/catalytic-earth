...
from catalytic_earth.lever2_mechanism_incremental_readout import (
    ...
    build_lever2_source_free_electron_flow_approval_import_delta_package_contract_readout,
    ...
    build_lever2_source_free_electron_flow_protected_train_cal_approved_sidecar_import_readout,
...
class Lever2MechanismIncrementalReadoutTests(unittest.TestCase):
    ...
    def test_electron_flow_approval_import_smoke_materialization_readout(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            smoke_review_path = root / "smoke_review.json"
            sidecar_path = root / "sidecar.json"
            ...
            smoke_review_path.write_text(
                json.dumps(
                    {
                        "review_contract": {
                            "review_id": "smoke_review",
                            "axis_id": "source_free_electron_flow",
                            "feature_fields": feature_fields,
                        },
                        "feature_rows": {
                            "smoke_tranche": smoke_rows,
                            "full_current_split": full_rows,
                        },
                        "measured_readout": {
                        ...
                        "counts": {
                            "full_current_split_rows": 5,
                            "full_current_retained_oos_positive_entry_ids": [
                                "m_csa:104",
                                "m_csa:119",
                                "m_csa:464",
                            ],
                            "full_incremental_oos_abstain_recall_vs_current_geometry_fold": 0.6,
                        },
                    }
                ),
                encoding="utf-8",
            ...
            readout = build_lever2_source_free_electron_flow_approval_import_smoke_materialization_readout(
                approval_import_smoke_review_readout_path=smoke_review_path,
                train_cal_feature_sidecar_path=sidecar_path,
                artifact_id="test_smoke_materialization",
            )
        ...
        self.assertEqual(
            readout["result_class"],
            (
                "research_only_approved_sidecar_shaped_smoke_materialization_"
                "gate_positive_pending_protected_import"
            ),
        )
        self.assertEqual(
            readout["counts"]["existing_approved_sidecar_rows_before"], 2
        )
        self.assertEqual(
            readout["counts"]["simulated_approved_sidecar_rows_after_smoke"], 4
        )
        self.assertEqual(readout["counts"]["smoke_materialization_new_rows"], 2)
        self.assertEqual(
        ...
        self.assertEqual(
            readout["counts"]["remaining_current_split_rows_after_smoke"], 2
        )
        self.assertEqual(
    ...
    def test_electron_flow_approval_import_delta_package_contract_detects_stale_sidecar(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            delta_package = build_lever2_source_free_electron_flow_approval_import_delta_package_readout(
                approval_import_candidate_sidecar_readout_path=candidate_path,
                train_cal_feature_sidecar_path=sidecar_path,
                artifact_id="test_delta_package",
            )
            delta_package_path.write_text(
                json.dumps(delta_package),
                encoding="utf-8",
            )

            contract = build_lever2_source_free_electron_flow_approval_import_delta_package_contract_readout(
                approval_import_delta_package_readout_path=delta_package_path,
                train_cal_feature_sidecar_path=sidecar_path,
                artifact_id="test_delta_contract",
            )
            ...
            stale_contract = build_lever2_source_free_electron_flow_approval_import_delta_package_contract_readout(
                approval_import_delta_package_readout_path=delta_package_path,
                train_cal_feature_sidecar_path=sidecar_path,
                artifact_id="test_delta_contract",
            )

        self.assertEqual(contract["artifact_id"], "test_delta_contract")
        self.assertEqual(
            contract["result_class"],
            "research_only_import_contract_verified_pending_protected_import",
        )
        self.assertTrue(contract["decision"]["acceptance_contract_verified"])
        self.assertTrue(
            contract["counts"]["current_sidecar_sha256_matches_source_package"]
        )
        self.assertEqual(contract["counts"]["contract_full_delta_rows_after_smoke"], 5)
        self.assertEqual(contract["counts"]["contract_full_delta_complete_rows"], 5)
        self.assertEqual(contract["counts"]["contract_full_delta_field_cell_writes"], 40)
        self.assertEqual(
            contract["counts"]["contract_component_positive_retained_oos_rows"], 3
        )
        self.assertEqual(
            contract["counts"]["contract_pqq_positive_retained_oos_entry_ids"],
            ["m_csa:104"],
        )
        self.assertEqual(
            contract["counts"]["contract_nad_family_positive_retained_oos_entry_ids"],
            ["m_csa:464"],
        )
        self.assertEqual(
            contract["counts"][
        ...
        self.assertEqual(
            stale_contract["result_class"],
            "research_only_import_contract_incomplete_or_stale",
        )
        self.assertFalse(
            stale_contract["decision"]["acceptance_contract_verified"]
        )
        self.assertFalse(
            stale_contract["counts"][
                "current_sidecar_sha256_matches_source_package"
            ]
        )
    ...
    def test_electron_flow_protected_train_cal_approved_sidecar_import_smoke_then_full(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            def row(
                ...
                role: str,
                stage: str,
                row_action: str = "add_new_approved_row",
                *,
                pqq: int = 0,
                nad: int = 0,
                fe_s: int = 0,
            ...
            smoke_rows = [
                row(
                    ...
                    "protected_smoke_tranche",
                    (
                        "update_existing_approved_row"
                        if index == 2
                        else "add_new_approved_row"
                    ),
                )
                for index in range(1, 35)
            ]
            ...
            smoke_gate = {
                ...
                "union_or_gate_oos_abstain_recall": 0.48,
                "current_geometry_fold_oos_rows": 75,
            }
            full_gate = {
                "rows": 74,
                "complete_rows": 74,
                "primary_rows": 34,
                "retained_oos_rows": 40,
                "primary_positive_rows": 0,
                "retained_oos_positive_rows": 3,
                "primary_positive_entry_ids": [],
                "retained_oos_positive_entry_ids": [
                ...
                "union_or_gate_oos_abstain_recall": 0.506667,
                "current_geometry_fold_oos_rows": 75,
            }
            delta_package_path.write_text(
                json.dumps(
                    {
                        ...
                        "measured_readout": {
                            "delta_smoke_gate_rerun": {
                                "fixed_gate_readout": smoke_gate,
                            },
                            "delta_full_74row_gate_rerun": {
                                "fixed_gate_readout": full_gate,
                            },
                        },
                    }
            ...
            readout = build_lever2_source_free_electron_flow_protected_train_cal_approved_sidecar_import_readout(
                protected_import_sequence_preflight_readout_path=preflight_path,
                train_cal_feature_sidecar_path=sidecar_path,
                artifact_id="test_protected_import",
            )

        self.assertEqual(readout["artifact_id"], "test_protected_import")
        ...
        smoke_sidecar = readout["materialized_approved_sidecars"][
            "smoke_tranche_first"
        ]
        imported_existing = {
            row["entry_id"]: row
            for row in smoke_sidecar["feature_rows"]
            if row["entry_id"] == "m_csa:2"
        }["m_csa:2"]
        self.assertEqual(
            imported_existing["row_specific_event_features"][
                "has_electron_transfer_event"
            ],
            True,
        )
