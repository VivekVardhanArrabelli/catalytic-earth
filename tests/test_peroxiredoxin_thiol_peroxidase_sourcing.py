"""Offline validation of peroxiredoxin / thiol-peroxidase sourcing."""
...
from catalytic_earth.peroxiredoxin_thiol_peroxidase_sourcing import (
    FAMILIES,
    build_peroxiredoxin_thiol_peroxidase_sourcing,
)
...
_ROWS = {
    "PRX001": (
        ...
        "Active site",
        "Cysteine sulfenic acid intermediate; peroxidatic cysteine",
    ),
    "GPX001": (
        ["1.11.1.9"],
        "Glutathione peroxidase 1",
        ["Selenocysteine", "Antioxidant"],
        [],
        "2 glutathione + H2O2 = glutathione disulfide + 2 H2O",
        "Active site",
        "Selenocysteine peroxidatic residue",
    ),
    # Peroxiredoxin/GPx family name only, no peroxide-reduction reaction: held.
    "EC0001": (
        ["1.11.1.24"],
    ...
    # Heme catalase (no thiol family text): routes off-target to heme_peroxidase_oxidase.
    "CAT001": (
        ["1.11.1.6"],
        "Catalase",
        ["Heme", "Hydrogen peroxide"],
        ["heme b"],
        "2 H2O2 = O2 + 2 H2O",
        "Active site",
        "heme-binding catalase active site",
    ),
    # FAD-dependent NADH peroxidase (flavin boundary): held, not peroxiredoxin.
    "NPX001": (
        ["1.11.1.1"],
        "NADH peroxidase",
        ["FAD", "Flavoprotein"],
        ...
        "Binding site",
        "FAD binding site of the NADH peroxidase",
    ),
}

_QUERY_ORDER = ("EC0001", "PRX001", "GPX001", "CAT001", "NPX001")
...
class PeroxiredoxinThiolPeroxidaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_peroxiredoxin_thiol_peroxidase_sourcing(
            max_records_per_lane=10,
            current_manifest_payload={"rows": []},
            frozen_benchmark_payload=[],
            expansion_payload=[],
            created_utc="2026-06-18T15:00:00Z",
    ...
    def test_family_is_peroxiredoxin_thiol_peroxidase(self):
        self.assertEqual(FAMILIES, ("peroxiredoxin_thiol_peroxidase",))
    ...
    def test_floor_projection_and_deploy_context(self):
        audit = self._run()
        proj = audit["floor_projection"]["peroxiredoxin_thiol_peroxidase"]
        self.assertEqual(
            proj["deploy_missing_active_site_context"],
            "peroxidatic_cysteine_selenocysteine_thiol_peroxide_reduction_context",
        )
    ...
    def test_guardrails_non_destructive(self):
        ...
        self.assertTrue(g["peroxiredoxin_thiol_peroxidase_handles_scope_admission_only"])
        self.assertTrue(g["ec_never_a_counted_corroborator"])
        self.assertTrue(g["heme_peroxidase_haloperoxidase_nadh_peroxidase_sod_boundary_guard"])
