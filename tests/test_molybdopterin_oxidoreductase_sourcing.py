"""Offline validation of molybdopterin oxidoreductase broadened-handle sourcing."""
...
from catalytic_earth.molybdopterin_oxidoreductase_sourcing import (
    FAMILIES,
    build_molybdopterin_oxidoreductase_sourcing,
)
...
_ROWS = {
    "MO0001": (
        ...
        "nitrate + a quinol = a quinone + nitrite + H2O",
        "Binding site",
    ...
    "MO0002": (
        ...
        "",
        "Active site",
        "catalytic selenocysteine near molybdopterin cofactor",
    ),
    # EC + keyword without a Mo-cofactor or reaction/active-site mechanism handle: hold.
    "NX0001": (
        ["1.7.5.1"],
        ["Molybdenum", "Oxidoreductase"],
        [],
    ...
    # Hydrolase side EC guard must hold even with Mo-cofactor evidence.
    "HD0001": (
        ["1.7.5.1", "3.1.1.1"],
        ["Molybdenum", "Oxidoreductase"],
        ["Mo-bis(molybdopterin guanine dinucleotide)"],
    ...
    "FD0001": (
        ...
        ["Flavoprotein"],
        ["FAD"],
        "glutathione + NADP(+) = glutathione disulfide + NADPH",
        "Binding site",
        "FAD binding",
    ),
}
...
def _search_record(accession):
    ...
    return {
        ...
        "protein_name": f"Test molybdo enzyme {accession}",
...
class MolybdopterinOxidoreductaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_molybdopterin_oxidoreductase_sourcing(
            max_records_per_lane=10,
            current_manifest_payload={"rows": []},
            frozen_benchmark_payload=[],
            expansion_payload=[],
            created_utc="2026-06-13T02:25:00Z",
    ...
    def test_family_is_molybdopterin_oxidoreductase(self):
        self.assertEqual(FAMILIES, ("molybdopterin_oxidoreductase",))
    ...
    def test_admitted_labels_are_bronze_and_leakage_safe(self):
        ...
        self.assertEqual(
            ...
            {"molybdopterin_oxidoreductase": 2},
        )
        for label in audit["applied_labels"]:
            self.assertEqual(label["fingerprint_id"], "molybdopterin_oxidoreductase")
    ...
    def test_molybdopterin_axes_are_mechanism_not_ec(self):
        audit = self._run()
        labels = {label["entry_id"]: label for label in audit["applied_labels"]}
        self.assertEqual(set(labels), {"uniprot:MO0001", "uniprot:MO0002"})
        axes = labels["uniprot:MO0001"]["evidence"]["source_trust_tier"][
    ...
    def test_ec_only_and_side_ec_controls_do_not_enter_family(self):
        ...
        self.assertNotIn("uniprot:HD0001", admitted_ids)

    def test_sequence_provenance_and_floor_projection(self):
        audit = self._run()
        proj = audit["floor_projection"]["molybdopterin_oxidoreductase"]
        self.assertEqual(
            proj["deploy_missing_active_site_context"],
            "molybdopterin_metal_center_redox_context",
        )
    ...
    def test_record_window_processes_slice_before_entry_fetch(self):
        audit = self._run(record_offset_per_lane=0, record_limit_per_lane=2)
        self.assertEqual(audit["counts"]["fetched_candidate_rows"], 2)
        self.assertEqual(audit["counts"]["record_offset_per_lane"], 0)
    ...
    def test_guardrails_non_destructive(self):
        ...
        self.assertTrue(g["non_oxidoreductase_side_ec_guard"])
