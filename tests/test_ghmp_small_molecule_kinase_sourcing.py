"""Offline validation of GHMP small molecule kinase broadened-handle sourcing."""

from __future__ import annotations

import unittest

from catalytic_earth.ghmp_small_molecule_kinase_sourcing import (
    FAMILIES,
    build_ghmp_small_molecule_kinase_sourcing,
)
...
_ROWS = {
    ...
    "GH0002": (
        ["2.7.1.39"],
        "Homoserine kinase",
        "ATP + L-homoserine = ADP + O-phospho-L-homoserine + H(+)",
        "Binding site",
        "ATP",
    ),
    "EC0001": (
    ...
    "PK0001": (
        ["2.7.1.36", "2.7.11.1"],
        "Mevalonate protein kinase boundary",
        "ATP + (R)-mevalonate = ADP + (R)-5-phosphomevalonate + H(+)",
        "Binding site",
        "ATP",
    ),
    "HK0001": (
        ["2.7.1.36", "2.7.13.3"],
        "Mevalonate histidine kinase boundary",
        "ATP + (R)-mevalonate = ADP + (R)-5-phosphomevalonate + H(+)",
        "Binding site",
        "ATP",
    ),
...
class GhmpSmallMoleculeKinaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_ghmp_small_molecule_kinase_sourcing(
    ...
    def test_family_is_ghmp_small_molecule_kinase(self):
        self.assertEqual(FAMILIES, ("ghmp_small_molecule_kinase",))
    ...
    def test_admitted_labels_are_bronze_and_leakage_safe(self):
        ...
        self.assertEqual(
            ...
            {"ghmp_small_molecule_kinase": 2},
        )
        for label in audit["applied_labels"]:
            self.assertEqual(label["fingerprint_id"], "ghmp_small_molecule_kinase")
    ...
    def test_boundary_controls_do_not_enter_family(self):
        ...
        for accession in ("EC0001", "PK0001", "HK0001", "AS0001"):
