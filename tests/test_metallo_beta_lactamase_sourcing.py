"""Offline validation of metallo-beta-lactamase (MBL) sourcing."""
...
from catalytic_earth.metallo_beta_lactamase_sourcing import (
    FAMILIES,
    build_metallo_beta_lactamase_sourcing,
)
...
_ROWS = {
    "MBL001": (
        ...
        ["Antibiotic resistance", "Zinc", "Metal-binding"],
        ["Zn(2+)"],
        "a beta-lactam + H2O = a substituted beta-amino acid",
        "Active site",
        "His/Asp/His zinc-ligand metallo-beta-lactamase active site",
    ),
    "MBL002": (
        ["3.5.2.6"],
        ...
        "a beta-lactam + H2O = a substituted beta-amino acid",
        "Binding site",
        "binuclear zinc binding site of the metallo-beta-lactamase fold",
    ),
    # Zinc/metallo name only, no beta-lactam hydrolysis reaction: held.
    "EC0001": (
        ["3.5.2.6"],
        "Metallo-beta-lactamase-like protein",
        ["Zinc"],
        ["Zn(2+)"],
        "",
        "",
    ...
    # Serine beta-lactamase (no zinc): routes off-target to serine_beta_lactamase.
    "SBL001": (
        ["3.5.2.6"],
        ...
        "a beta-lactam + H2O = a substituted beta-amino acid",
        "Active site",
        "Acyl-ester intermediate; nucleophile serine of the serine beta-lactamase",
    ),
    # Penicillin-binding protein / DD-peptidase boundary: held, not MBL.
    "PBP001": (
        ["3.5.2.6"],
        "Penicillin-binding protein 2 (D-alanyl-D-alanine carboxypeptidase)",
        ["Zinc"],
        ["Zn(2+)"],
        "a beta-lactam + H2O = a substituted beta-amino acid",
        "Active site",
        "penicillin-binding protein transpeptidase site",
    ),
}

_QUERY_ORDER = ("EC0001", "MBL001", "MBL002", "SBL001", "PBP001")
        ...
        "cofactor_comments": [
            {
                "cofactors": [
                    {
                        "name": name,
                        "cross_reference": {"id": "ChEBI:CHEBI:29105" if name.startswith("Zn") else None},
                        "evidence": [{"evidence_code": "ECO:0000269"}],
                    }
...
class MetalloBetaLactamaseSourcingTest(unittest.TestCase):
    def _run(self, **kwargs):
        return build_metallo_beta_lactamase_sourcing(
    ...
    def test_family_is_metallo_beta_lactamase(self):
        self.assertEqual(FAMILIES, ("metallo_beta_lactamase",))
    ...
    def test_floor_projection_and_deploy_context(self):
        audit = self._run()
        proj = audit["floor_projection"]["metallo_beta_lactamase"]
        self.assertEqual(
            proj["deploy_missing_active_site_context"],
            "dizinc_metallo_beta_lactamase_ring_hydrolysis_context",
        )
    ...
    def test_guardrails_non_destructive(self):
        ...
        self.assertTrue(g["metallo_beta_lactamase_handles_scope_admission_only"])
        self.assertTrue(g["ec_never_a_counted_corroborator"])
        self.assertTrue(g["serine_beta_lactamase_pbp_nonbetalactam_amidohydrolase_boundary_guard"])
